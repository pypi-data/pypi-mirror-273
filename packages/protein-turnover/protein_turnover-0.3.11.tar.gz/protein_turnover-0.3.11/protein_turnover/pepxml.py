...
def compute_settings(
    df: pd.DataFrame,
    settings: PeptideSettings,
    abundanceCutoff: float,
) -> pd.DataFrame:
    from .utils import (
        isotopicDistribution,
        peptideFormula,
        NAMES,
        labelledAtomicAbundances2,
    )
    from .utils import eic_mzranges
    from .types.pepxmltypes import PepXMLRunRow

    element = NAMES == settings.labelledElement
    assert element.sum() == 1

    def element_count(peptide: str) -> int:
        return peptideFormula(peptide)[element][0]

    # labelled abundances at maximum Enrichment level
    abundances = labelledAtomicAbundances2(settings)

    # maxIso length of ndist[E] to 0.01
    def max_iso(peptide: str) -> int:
        return len(isotopicDistribution(peptide, abundances, abundanceCutoff)) - 1

    def mzranges(pep: PepXMLRunRow) -> np.ndarray:
        return eic_mzranges(pep, settings)
...
def filter_pepxml(df: pd.DataFrame, job: TurnoverJob) -> pd.DataFrame:
    ...
    df = compute_settings(df, job.settings, abundanceCutoff=ABUNDANCE_CUTOFF)
...
#     protxml_df: pd.DataFrame,
#     settings: PeptideSettings,
# ) -> pd.DataFrame:
#     # TODO: join peptide and protxml
#     return pepxml_df
