...
def protxml_raw(
    ...
    logger.info("reading: %s", protxml.original.name)
    df = ProtXmlDataFrame(
        protxml.original,
        level=level,
        number_of_bg_processes=number_of_bg_processes,
    )
    logger.info("done: %s", protxml.original.name)
    return df
...
def getprotxml(protxml: ProtXMLResourceFile) -> pd.DataFrame:
    if protxml.cache_protxml_ok():
        ...
        if not protxml.exists:
            raise RuntimeError(f"can't find file: {protxml.original}")
