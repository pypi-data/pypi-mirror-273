...
def make_migrations(
    ...
    *,
    ...
    try:
        ...
        print(f'[blue]{warn}[/blue]')
    else:
        print(f'[green]Migration created: {migration.path.name}[/green]')


@sub_app_deprecated.command(name='make', deprecated=True)
def make(
    ctx: typer.Context,
    build_dir: Annotated[Path, typer.Option('--build-dir', '-b')] = Path('.'),
    *,
    ...
    make_migrations(ctx, build_dir=build_dir, name=name, is_data=is_data, config=config)
