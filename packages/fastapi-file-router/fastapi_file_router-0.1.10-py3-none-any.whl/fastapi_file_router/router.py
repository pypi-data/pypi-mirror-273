...
def load_routes(app: FastAPI, directory: Path, auto_tags: bool = True, verbose: bool = False):
    ...
    for root, _, files in walk(directory):
        ...
        for file in files:
            ...
            if "route" in file_path.stem and file_path.stem != "route":
                log(
                    f"Skipping possibly misspelled route file {file_path.stem} in {root}", verbose
                )
                continue
            ...
            if not router:
                log(
                    f"Router {(root / file).absolute().as_posix()} does not contain a router", verbose
                )
                continue
    ...
    log(f"Routes loaded in {time() - start:.2f}s")
