...
class DiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        elif self.mode == MULTICLASS_MODE or self.mode == MULTILABEL_MODE:
...
if __name__=="__main__":
    ...
    input_binary = torch.rand((1, 1, 16, 16))
    target_binary = torch.randint(0, 2, (1, 1, 16, 16))
    ...
    input_multiclass = torch.randn((1, 3, 16, 16))
    target_multiclass = torch.randint(0, 3, (1, 16, 16))
    ...
    # Multilabel Mode
    criterion_multilabel_focal = FocalLoss(mode='multilabel', alpha=1, gamma=2, reduction='mean')
    criterion_multilabel_dice = DiceLoss(mode=MULTILABEL_MODE, reduction='mean')
    criterion_multilabel_iou = JaccardLoss(mode=MULTICLASS_MODE, reduction='mean')
    input_multilabel = torch.randn((1, 3, 16, 16))
    target_multilabel = torch.randint(0, 2, (1, 3, 16, 16))
