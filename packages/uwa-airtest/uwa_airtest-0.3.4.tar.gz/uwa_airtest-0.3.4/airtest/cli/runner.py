    ...
    def tearDown(self):
        if self.args.log and self.args.recording:
            for k, dev in enumerate(G.DEVICE_LIST):
                ...
                try:
                    if isinstance(
                        self.args.recording, six.string_types
                    ) and self.args.recording.endswith(".mp4"):
                        basename = os.path.basename(self.args.recording)
                        output_name = (
                            dev.serialno + "_" + basename
                            if len(G.DEVICE_LIST) > 1
                            else basename
                        )
                    else:
                        output_name = "recording_{serialno}.mp4".format(
...
def setup_by_args(args):
    ...
    try:
        if args.stop_on_error:
            ...
            print("Continue on error by default")
    except Exception:
        # 不一定配置了 stop_on_error
        print("stop_on_error not set, Continue on error by default")
