...
def log(arg, timestamp=None, desc="", snapshot=False):
    ...
    if G.LOGGER:
        # ADD: 同 function 一样记录 info log
        depth = 0
        start = time.time()
        stack = inspect.stack()[1]
        m = inspect.getcallargs(log, arg, timestamp=timestamp, desc=desc, snapshot=snapshot)
        fndata = {
            "call_args": m,
            "start_time": start,
            "end_time": start,
        ...
        if isinstance(arg, Exception):
            ...
            G.LOGGER.log("info", {"name": desc or arg, "traceback": None, "log": arg, **fndata}, depth=depth, timestamp=timestamp)
            G.LOGGING.info(arg)
        else:
            G.LOGGER.log("info", {"name": desc or repr(arg), "traceback": None, "log": repr(arg), **fndata}, depth=depth,
                         timestamp=timestamp)
