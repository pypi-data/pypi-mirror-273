...
@pytest.fixture(scope="function")
def create_source_and_base_model(
    postgres_connection,
    TEST_SOURCE,
    TEST_SCHEMA,
    SOURCE_SCHEMA_PATH,
...
def create_model(
    postgres_connection,
    create_source_and_base_model,
    TEST_TABLE_ACCOUNT,
    TEST_TABLE_ACCOUNT_BASE_MODEL,
):
    """Create a model for tests.

    Creating a model consists of three steps:
    1. Creating the SQL for the model
    2. Materializing it with `dbt run`
    3. Creating the YAML file for the model with `nesso model bootstrap-yaml`
    """
    # Create model SQL.
    ...
    call_shell(f"dbt run -s {TEST_TABLE_ACCOUNT}")
