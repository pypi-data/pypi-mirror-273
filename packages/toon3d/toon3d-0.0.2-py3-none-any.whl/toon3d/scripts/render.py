...
from toon3d.utils.mesh_utils import render_mesh, render_point_cloud
import trimesh
...
class RenderToon3DInterpolated(BaseRender):
    ...
    representation: Literal["splat", "point-cloud", "mesh"] = "splat"
    ...
    def main(self) -> None:
        ...
        for indices in all_pair_indices:
            ...
            elif self.representation == "point-cloud":
                point_cloud_filename = self.nerfstudio_data_path / "plys" / "all.ply"
                point_cloud = trimesh.load(point_cloud_filename)
                vertices = torch.from_numpy(point_cloud.vertices).float().to(device)
                colors = torch.from_numpy(point_cloud.visual.vertex_colors[:, :3]).to(device) / 255.0

                dataparser_scale = datamanager.train_dataparser_outputs.dataparser_scale
                dataparser_transform = datamanager.train_dataparser_outputs.dataparser_transform.to(device)
                vertices = (
                    torch.cat(
                        (
                            vertices,
                    ...
                    @ dataparser_transform.T
                )
                vertices = vertices * dataparser_scale

                writer_frames = []
                for i in tqdm(range(len(camera_path))):
                    c2w = camera_path[i].camera_to_worlds.to(device)
                    focal_length = torch.cat([camera_path[i].fx, camera_path[i].fy]).to(device)
                    principal_point = torch.cat([camera_path[i].cx, camera_path[i].cy]).to(device)
                    image_size = (int(principal_point[1] * 2), int(principal_point[0] * 2))
                    ...
                    writer_frames.append(image.detach().cpu().numpy())

            elif self.representation == "mesh":
                mesh_filename = self.nerfstudio_data_path / "objs" / "all.obj"
                mesh = trimesh.load(mesh_filename)
                vertices = torch.from_numpy(mesh.vertices).float().to(device)
                colors = torch.from_numpy(mesh.visual.vertex_colors[:, :3]).to(device) / 255.0
