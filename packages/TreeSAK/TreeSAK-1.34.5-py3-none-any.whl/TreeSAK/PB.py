...
def PB(args):
    ...
    if num_of_chains == 1:
        ...
        pb_mpi_cmd = 'mpirun -np %s pb_mpi -d %s %s -s %s' % (num_of_threads, msa_to_use, setting_to_use, op_prefix)
        ...
        for chain_index in range(1, (num_of_chains + 1)):
            pb_mpi_cmd = 'mpirun -np %s pb_mpi -d %s %s -s %s_chain%s' % (1, msa_to_use, setting_to_use, op_prefix, chain_index)
            pb_mpi_cmd_list.append(pb_mpi_cmd)
        ...
        for chain_index in range(1, (num_of_chains + 1)):
            pb_mpi_cmd = 'mpirun -np %s pb_mpi -d %s %s -s %s_chain%s' % (cores_per_run, msa_to_use, setting_to_use, op_prefix, chain_index)
            pb_mpi_cmd_list.append(pb_mpi_cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-chain',   required=False, type=int, default=4,    help='num of chains to run in parallel, default: 4')
    parser.add_argument('-t',       required=False, type=int, default=4,    help='num of cores, default: 4')
