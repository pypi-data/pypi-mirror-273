...
def config_to_docker(config_path: pathlib.Path, config: Config):
            ...
            if not resolved.exists():
                ...
                for local_pkg in local_pkgs:
                    if resolved.is_relative_to(local_pkg):
                        module_str = (
                            f"/tmp/{local_pkg.name}/{resolved.relative_to(local_pkg)}"
                        )
                        break
                else:
                    for faux_pkg in faux_pkgs:
                        if resolved.is_relative_to(faux_pkg):
                            module_str = f"/tmp/{faux_pkg.name}/{resolved.relative_to(faux_pkg.parent)}"
                            break
                    else:
                        raise ValueError(
    ...
    faux_pkgs_str = "\n\n".join(
        ...
        + (
            f"\n                RUN pip install -r /tmp/{fullpath.name}/{fullpath.name}/requirements.txt"
