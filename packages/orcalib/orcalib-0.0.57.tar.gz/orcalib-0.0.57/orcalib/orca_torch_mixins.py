...
class CurateSettings:
    ...
    def __init__(
        ...
        self.run_ids: Optional[list[int]] = None
...
class CurateSettingsMixin:
    ...
    @property
    def curate_run_ids(self) -> list[int] | None:
        return self._curate_settings.run_ids

    @curate_run_ids.setter
    def curate_run_ids(self, value: list[int] | None) -> None:
        self._curate_settings.run_ids = value
    ...
    @property
    def curate_metadata(self) -> dict[str, Any] | None:
        return self._curate_settings.metadata

    @curate_metadata.setter
    def curate_metadata(self, value: dict[str, Any] | None) -> None:
        self._curate_settings.metadata = value

    @property
    def curate_tags(self) -> set[str] | None:
        return self._curate_settings.tags

    @curate_tags.setter
    def curate_tags(self, value: set[str] | None) -> None:
    ...
    def last_curate_run_info(self):
        assert (
            self.curate_model_id is not None and self.curate_batch_size is not None and self.curate_run_ids is not None
        ), f"It seems there is no last curate run on this model: {self.curate_model_id}, {self.curate_batch_size}, {self.curate_run_ids}"
        return CurateRunInfo(
            run_ids=self.curate_run_ids,
...
class LookupSettings:
    ...
    def __or__(self, other: "LookupSettings") -> "LookupSettings":
        ...
        return LookupSettings(
            ...
            lookup_column_names=self.lookup_column_names
            if self.lookup_column_names is not None
            else other.lookup_column_names,
...
class LookupSettingsMixin:
    ...
    def update_lookup_settings(
        ...
        self._orca_lookup_settings.lookup_database = database or self._orca_lookup_settings.lookup_database
        self._orca_lookup_settings.memory_index_name = memory_index_name or self._orca_lookup_settings.memory_index_name
        self._orca_lookup_settings.lookup_column_names = (
            lookup_column_names or self._orca_lookup_settings.lookup_column_names
        )
        self._orca_lookup_settings.num_memories = num_memories or self._orca_lookup_settings.num_memories
