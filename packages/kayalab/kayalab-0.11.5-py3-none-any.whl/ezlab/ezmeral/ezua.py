...
def install():
    ...
    for out in execute(
        f"{ezfabricctl_cmd} w g k -n {app.storage.general[UA].get('clustername', 'ezlab')} -i ./.ezlab/clusterConfig.yaml -c ./.ezlab/mgmt-kubeconfig -s ./.ezlab/clusterConfigStatus.txt --save-kubeconfig ./.ezlab/workload-kubeconfig"
    ):
        logger.info(out)
...
def deploy():
    ...
    for out in execute(f"{kubectl_cmd} --kubeconfig=./.ezlab/workload-kubeconfig apply -f ./.ezlab/ezkfWorkloadDeploy.yaml"):
        logger.info(out)
        if f"ezkfworkloaddeploy.ezkfops.hpe.ezkf-ops.com/{app.storage.general[UA].get('clustername', 'ezlab')}" in out and any(
            ...
            logger.info(
                ...
                kubectl --kubeconfig=./.ezlab/workload-kubeconfig get ezkfworkloaddeploy/ -n {app.storage.general[UA].get('clustername', 'ezlab')} -o json | jq '.status.genericaddonsstatus | .[] | select(.installstatus == "INSTALLING")'
                """
            )
...
def write_clusterConfig_yaml():
  ...
  deployEnv: ezkube
  workloadType: ezua
  clusterName: {app.storage.general[UA].get('clustername', 'ezlab')}
  resources:
    vcpu: 96
...
metadata:
  name: airgap
  namespace: {app.storage.general[UA].get('clustername', 'ezlab')}
type: Opaque
...
metadata:
  name: {app.storage.general[UA].get('clustername', 'ezlab')}
  namespace: {app.storage.general[UA].get('clustername', 'ezlab')}
spec:
  deploytarget: pph
  workloadtype: ezua
