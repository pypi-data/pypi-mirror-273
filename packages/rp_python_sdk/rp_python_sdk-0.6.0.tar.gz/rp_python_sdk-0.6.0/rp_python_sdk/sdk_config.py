    ...
    def __post_init__(self) -> None:
        if self.include_uncertified_participants:
            ...
            if self.required_claims:
                logger.info(
                    f"Identity provider list will be filtered for participants that support the following claims: {self.required_claims}")
            if self.required_participant_certifications:
                logger.info(
                    f"Identity provider list will be filtered for participants that support the following certifications: {self.required_participant_certifications}")
