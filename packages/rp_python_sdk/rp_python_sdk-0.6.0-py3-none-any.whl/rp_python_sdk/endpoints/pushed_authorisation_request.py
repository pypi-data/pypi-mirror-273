...
default_claims = {"name", "given_name", "middle_name", "family_name", "phone_number", "email", "address", "birthdate",
                  "txn"}
...
def send_pushed_authorisation_request(config: SdkConfig,
    ...
    log_x_fapi_interaction_id_matches(authorisation_server, "send_pushed_authorisation_request", x_fapi_interaction_id, response)
...
def _check_valid_purpose(purpose) -> None:
    if len(purpose) < 3 or len(purpose) > 50:
        raise RelyingPartyClientSdkException("Purpose must be between 3 and 50 characters")
    ...
    if contains_illegal_char:
        raise RelyingPartyClientSdkException(
            f"Purpose cannot contain any of the following characters: {', '.join(illegal_chars)}, purpose supplied: [{purpose}]")
...
def _generate_claims_request(essential_claims: set[str], voluntary_claims: set[str]):
...
def _partition_claims(claims: set[str]):
    """
...
def _map_claims(claims, claim_map):
    """
