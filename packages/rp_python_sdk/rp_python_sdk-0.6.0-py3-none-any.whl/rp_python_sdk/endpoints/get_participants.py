...
def get_participants(config: SdkConfig) -> list[Participant]:
    ...
    if len(config.custom_config.required_claims) != 0:
        ...
        filtered_participants = filter_auth_servers_for_supported_claims(filtered_participants, config.custom_config.required_claims)
    ...
    if len(config.custom_config.required_participant_certifications) != 0:
        ...
        filtered_participants = filter_for_required_certifications(filtered_participants, config.custom_config.required_participant_certifications)
...
def get_issuer_metadata(authorisation_server: AuthorisationServer):
    auth_server_issuer = authorisation_server.open_id_discovery_document
    ...
    try:
        ...
        return IssuerMetadata.from_json(response.text)
