...
import requests
import yaml
from fm_actor.download import DownloadDelegate
from fm_actor.fmdata import FmData
...
def test_download_with_requests():
    ...
    fm_data.download_and_install_into(
        target, delegate=DownloadDelegate(requests.Session())
    )
