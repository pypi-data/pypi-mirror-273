'''
    ############################################################################
    ######################## Print Tricks & More - version 0.3.179 #############
    ############################################################################
...
class pt:
    ...
    def _print_easy_imports(source, msg=''):
        ...
        import_str = f"{C.t2}  > {'successful' if str(source) in sys.path else 'Did not complete: '}{C.er}" \
            f" - The directory {C.t1}'{source}'{C.er} {'is now' if str(source) in sys.path else 'does not exist and was not'} added to the system path.{C.er}"
    ...
    def easy_imports(source=''):
        '''
            source:
                ...
                located. 
            '''
        if source in pt.easy_imports_sources:
            return
        else:
            pt.easy_imports_sources.add(source)
        ...
        while parent:
            ...
            if root_reached:
                if similar_paths:
                    ...
                    pt._print_easy_imports(source=source,
                        msg=f'{C.t1}Did not find "{source}", but found similar path(s):{C.er}\n'
