...
class ListTool:
    ...
    @staticmethod
    def str_list_diff(
        a: list[str], b: list[str], case_insensitive: bool = True
    ) -> list[str]:
        if case_insensitive:
            a = lw(a)
            b = lw(b)
        return ListTool.diff(a, b)
    ...
    def list_of_str_intersection(
        ...
        return ListTool.list_of_str_intersection_ratio(a, b, case_insensitive) < (
            len(a) + len(b)
        )
    ...
    def list_of_str_intersection_ratio(
        ...
        return len(ListTool.str_list_diff(a, b, case_insensitive))

    @staticmethod
    def str_list_equal(
        a: list[str], b: list[str], case_insensitive: bool = True
    ) -> bool:
        return len(ListTool.str_list_diff(a, b, case_insensitive)) == 0
...
class StringTool:
    ...
    def _comparation_ratio_by_tokens(
        a: str, b: str, token_delimiter: str = " ", case_insensitive: bool = True
    ) -> tuple[int, list[str], list[str]]:
        if case_insensitive:
            a = lw(a)
            b = lw(b)
        a_list: list[str] = a.split(token_delimiter)
        b_list: list[str] = b.split(token_delimiter)
        return (ListTool.list_of_str_intersection_ratio(a_list, b_list), a_list, b_list)
    ...
    def equal_by_tokens(
        ...
        return (
            StringTool._comparation_ratio_by_tokens(
                a, b, token_delimiter, case_insensitive
            )[0]
            == 0
        )
