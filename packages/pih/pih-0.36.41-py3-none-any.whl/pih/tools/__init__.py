...
class ListTool:
    ...
    @staticmethod
    def str_diff(a: list[str], b: list[str], compare_level: int = 2) -> list[Any]:
        compare_function: Callable[[list[str], list[str]], bool] | None = None
        if compare_level == 0:
            compare_function = StringTool.equal
        else:
            compare_function = lambda a, b: StringTool.contains(
                a, b, compare_level == 1
            )
        return [
            i
            for i in a + b
            if not (compare_function(i, j(a)) and compare_function(i, j(b)))
        ]
    ...
    def str_list_diff(
        ...
        a = lw(a)
        b = lw(b)
        return ListTool.str_diff(a, b, compare_level)
    ...
    @staticmethod
    def list_of_str_intersection(
        a: list[str], b: list[str]
    ) -> bool:
        return ListTool.list_of_str_intersection_ratio(a, b) < (
            len(a) + len(b)
        )
    ...
    def list_of_str_intersection_ratio(
        ...
        return len(ListTool.str_list_diff(a, b, compare_level))
...
class StringTool:
    ...
    def _comparation_ratio_by_tokens(
        ...
        return (
            ListTool.list_of_str_intersection_ratio(
                a_list, b_list, token_compare_level
            ),
            a_list,
            b_list,
        )
    ...
    def full_right_intersection_by_tokens(
        ...
        result: tuple[int, list[str], list[str]] = (
            StringTool._comparation_ratio_by_tokens(
                a, b, token_delimiter, token_compare_level
            )
        )
        return result[0] == (len(result[1]) - len(result[2]))
