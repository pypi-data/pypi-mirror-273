...
class Session(SessionBase):
    ...
    def say_hello(self, _, greeting: bool = True) -> None:
...
class Input(InputBase):
    ...
    def choose_file(
        self, use_search_request: bool | str | None = None, allow_choose_all: bool = False
    ) -> File | list[File]:
    ...
    def choose_note(
        ...
        if n(use_search_request):
            use_search_request = self.yes_no("Использвать поиск", enter_for_yes = True)
...
class PIH:
    ...
    class ERROR:
        ...
        class POLIBASE:
            ...
            def create_not_found_error(
                ...
                return NotFound(js((start, "с", title,  escs(value), "не найден!")), value)

        class USER:
            
            @staticmethod
            def create_not_found_error(title: str, active: bool, value: str) -> NotFound:
                start: str | None = None
                if n(active):
                    start = "Пользователь"
                elif active:
    ...
    class RESULT(ResultTool):
        ...
        class SETTINGS:
            ...
            @staticmethod
            def get(settings_item: SETTINGS | StorageVariableHolder | None) -> Result[Any] | Result[list[strdict]]:
        ...
        class WORKSTATION:
            ...
            def by_any(value: int | str | User | None) -> Result[list[Workstation]]:
                ...
                try:
                    ...
                    try:
                        ...
                        raise PIH.ERROR.WORKSTATION.create_not_found_error("параметром поиска", value)
            ...
            def by_user_name(value: str) -> Result[list[Workstation]]:  
                ...
                def action(user: User, fields: FieldItemList) -> None:
                    ...
                    DH.result += PIH.RESULT.WORKSTATION.by_login(
                        user.samAccountName
                    )

                ResultTool.every_with_fields(
                    action, PIH.RESULT.USER.by_name(value)
                )
                return DH.result
        ...
        class MARK:
            ...
            def by_tab_number(value: str | int) -> Result[Mark]:
                ...
                if e(result):
                    raise NotFound(js(("Карта доступа с номером", escs(value), "не найдена")))
                return result
        ...
        class USER:
            ...
            def by_login(
                ...
                try:
                    ...
                    raise PIH.ERROR.USER.create_not_found_error("логином", active, value)
            ...
            def by_workstation_name(value: str) -> Result[User]:
                ...
                if e(user_workstation.samAccountName):
                    raise NotFound(
                        js(("За компьютером", value,  "нет залогиненного пользователя")), value
                    )
            ...
            def set(settings_item: SETTINGS | str, value: Any) -> bool:
                ...
                if isinstance(settings_item_value, VariantableStorageVariable):
                    if value not in settings_item_value.variants:
                        raise NotFound(js((value,"not in", settings_item_value.variants)))
