...
from PyQt6.QtGui import QCursor, QAction, QContextMenuEvent, QTextCharFormat, QColor
from .types_input.SingleValueInputDialog import SingleValueInputDialog
from PyQt6.QtCore import Qt, QObject, QCoreApplication, pyqtSlot
from ..Functions import clear_table_widget, format_dbus_error
...
class _MethodItem(QTreeWidgetItem):
    ...
    def call_method(self) -> None:
        if len(self._method.arguments) != 0:
            args = ArgumentInput().get_argument_values(self._method.arguments)
...
class CentralWidget(QWidget, Ui_CentralWidget):
    def __init__(self, env: "Environment", main_window: "MainWindow", connection: "Connection") -> None:
        ...
        self._update_objects_buttons_enabled()
    ...
    def _services_table_context_menu(self, event: QContextMenuEvent) -> None:
        ...
        copy_name_action = QAction(QCoreApplication.translate("CentralWidget", "Copy name"), self)
        ...
        if service.process_name is not None:
            copy_process_name_action = QAction(QCoreApplication.translate("CentralWidget", "Copy process name"), self)
        ...
        if service.pid is not None:
            copy_pid_action = QAction(QCoreApplication.translate("CentralWidget", "Copy pid"), self)
        ...
        if service.user_name is not None:
            copy_user_action = QAction(QCoreApplication.translate("CentralWidget", "Copy user"), self)
    ...
    def _update_objects_buttons_enabled(self) -> None:
    ...
    def disconnect_signal(self,  signal: "Signal", record_macro: bool) -> None:
        ...
        self.connection.connection.disconnect(signal.interface.service.name, signal.interface.object_path, signal.interface.name, signal.name, self._dbus_signal_callback)
