...
def set_state(name: str, state: _State, state_id: str | None) -> None:
    """Sets a state"""
    body = {"name": name, "state": state}
    if state_id is not None:
        url = API_ROOT + "states/" + state_id + "/"
    ...
    if response.status_code != 200:
        raise errors.InternalSoilError("Error setting state:" + response.text)


type _StateId = str


def get_state(name: str) -> tuple[dict, _StateId]:
    """Gets a state"""
    url = API_ROOT + "states/?name=" + name + "&with-states"
    response = session.get(url, timeout=30)

    if response.status_code == 200:
        state = json.loads(response.text)
        if len(state) == 0:
            raise errors.ObjectNotFound("Error getting state:" + response.text)
        if len(state) > 1:
            logging.warning("Multiple states found with name %s", name)
        return state[0]["state"], state[0]["_id"]
    raise errors.InternalSoilError("Error getting state:" + response.text)
