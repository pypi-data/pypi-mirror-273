        ...
        try:
           for i in range(10):
               urls = [{
                   ...
                   'url': 'https://baijiahao.baidu.com/s?id=1728736904461500633&wfr=spider&for=pc',
                   'type': 1,
                   # 'type': 17,
                   'unique_key':self.get_unique_key()#默认md5 url 字段，不写则表示相同链接只查一次
               }]
               config={"param": {'cu':'https://baijiahao.baidu.com','et':0},"conf_district_id":0}
               untilBaidu = UtilBaiduRelate()
               headers = untilBaidu.getBaiduPcHeader()
               headers['Referer'] = 'https://www.hao123.com/'
               headers['Cookie'] = untilBaidu.get_baijiahao_cookie()
               # headers['Accept-Language'] = 'zh-CN,zh;q=0.9,en-CN;q=0.8,en;q=0.7,ar-XB;q=0.6,ar;q=0.5'
