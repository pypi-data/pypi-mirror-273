...
class KubeflowPlugin:
    ...
    @staticmethod
    def get_model_url(model_name: str, max_wait_time: int = 120):
        """
        ...
        Returns:
            str: URL of the deployed model if available within the timeout, otherwise a timeout message or an error message.
        """
        # Verify plugin activation
        PluginManager().verify_activation(KubeflowPlugin().section)
        ...
        while time.time() - start_time < max_wait_time:
            ...
            address = isvc_resp["status"].get("address")
            if address and "url" in address:
                isvc_url = address["url"]
                return isvc_url
