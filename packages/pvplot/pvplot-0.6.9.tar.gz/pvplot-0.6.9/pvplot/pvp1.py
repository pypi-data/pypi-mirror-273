    ...
    for dataset in MapOfDatasets.dtsDict.values():
        ...
        if dataset.dataChanged:
            try:
                ...
                if dataset.viewBox.statisticsWindow.isVisible:
...
class Dataset():
    ...
    def get_statistics(self):
        ...
        rp = r[parname]
        npt = len(self.data[Y])
        rp['mean'] = np.mean(self.data[Y])#.astype('f4')
        rp['std'] = np.std(self.data[Y])#.astype('f4')
...
class CustomViewBox(pg.ViewBox):
    ...
    def show_statistics(self):
        ...
        if visible:
            ...
            self.statisticsWindow.label.setText(f'Statistics will be shown')
