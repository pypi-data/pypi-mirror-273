    ...
    def update_plot(self, time2plot):
        ...
        if l > 1:
            ...
            if time2plot:
                self.plot(ts)
        try:
            sw = self.viewBox.statisticsWindow
            if sw.isVisible():
                np = 0
                #np = len(self.data[Y])
                #mean = np.mean(self.data[Y])
                #std = np.std(self.data[Y])
                #sw.label.setText(f'Mean,std of {self.adoPar}: {mean,std}')
                sw.label.setText(f'Mean,std[{np}] of {self.adoPars}: {time.time()}')
        except: pass
...
class CustomViewBox(pg.ViewBox):
    ...
    def show_statistics(self):
        try:
            visible = self.statisticsWindow.isVisible()
        except:
            self.statisticsWindow = PopupWindow()
            visible = False
        if visible:
            self.statisticsWindow.hide()
        else:
            #self.statisticsWindow.label.setText(f'Statistics: {time.time()}')
