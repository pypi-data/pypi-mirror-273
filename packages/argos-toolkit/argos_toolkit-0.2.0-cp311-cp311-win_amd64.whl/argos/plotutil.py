...
def read_frame(path, frame_no):
    ...
    ret, frame = vid.read()
    if frame is None:
        print('Empty frame', path, ', frame #', frame_no)
    vid.release()
    return frame


def interpolate_tracks(tracks, frame_ts=None):
    """Interpolate the track positions for missed frames. Tracks must have
    a ``timestamp`` column with timestamp for each frame. If `tracks`
    does not contain ``timestamp`` column or ``frame_ts`` is `None`,
    then the frame no. is used as timestamp equivalent.

    Parameters
    ----------
    tracks: pd.DataFrame
        Track data with at least ``frame``, ``trackid``, ``x``, ``y``, ``w``, 
        ``h`` columns.
    frame_ts: pd.DataFrame or None
        Timestamp for each frame.

    Returns
    -------
    pd.DataFrame: 
        New track data where positions have been interpolated for
        every frame with two added columns ``tdelta`` and ``stay``
        described below.

        ``tdelta`` column is the total seconds from the start of
        recording.
    
        ``stay`` columns indicates how many seconds ellapsed between
        successive frames. (If we did not interpolate through frames,
        it would have been the time interval between successive
        detected positions).

    """
    tracks.sort_values(by=['frame', 'trackid'], inplace=True)
    itracks = []
    # we don't have real timestamps, use frame no.
    has_ts = (frame_ts is not None) and ('timestamp' in tracks.columns)
    tstart = tracks.timestamp.min() if has_ts else 0
    for tid, tgrp in tracks.groupby('trackid'):
        frames = pd.Series(data=np.arange(tgrp.frame.min(),
                                          tgrp.frame.max() + 1),
                           dtype=int,
                           name='frame')
        # print('Number of frames', len(frames))
        # get the actual timestamps for entire range of frames with this track
        if has_ts:
            t = pd.merge(frames, frame_ts, on='frame', how='left')
            # print(t.head())
            dt_all = t.timestamp - tstart   # dt for all frames in this range
            dt_all = dt_all.dt.total_seconds()
            dt = tgrp.timestamp - tstart    # dt for frames with this track detected
            dt = dt.dt.total_seconds()
        else:
            dt_all = frames
            dt = tgrp.frame - tstart
        x = np.interp(dt_all, dt, tgrp.x.values)
        y = np.interp(dt_all, dt, tgrp.y.values)
        w = np.interp(dt_all, dt, tgrp.w.values)
        h = np.interp(dt_all, dt, tgrp.h.values)
        stay = np.r_[np.diff(dt_all), 0]
        # print('Frames', len(frames),
        #       'tdelta', len(dt_all),
        #       'stay', len(stay))
        df = pd.DataFrame({'frame': frames.values,
                           'tdelta': dt_all,
                           'stay': stay,
                           'trackid': [tid] * len(frames),
                           'x': x, 'y': y, 'w': w, 'h': h})
        if 'wx' in tgrp.columns:
            # This can be used for warped position
            df['wx'] = np.interp(dt_all, dt, tgrp.wx.values)
            df['wy'] = np.interp(dt_all, dt, tgrp.wy.values)
            df['wh'] = np.interp(dt_all, dt, tgrp.wh.values)
            df['ww'] = np.interp(dt_all, dt, tgrp.ww.values)
        itracks.append(df)
    itracks = pd.concat(itracks)
    if has_ts:
        itracks['timestamp'] = (pd.to_timedelta(itracks['tdelta'], unit='s') +
                                tstart)
    return itracks
...
def plot_hexbin(tracks, title='Argos: Occupancy histogram', cmap='inferno',
                gridsize=100,
                frame=None):
    """Plot distribution of animal position using 2D histogram in hexbin
    form."""
    fig, ax = plt.subplots()    
    fig.suptitle(title)
    if frame is not None:
        ax.imshow(frame)
    x = tracks['x'].values + tracks['w'].values / 2.0
    y = tracks['y'].values + tracks['h'].values / 2.0
    ax.hexbin(x, y, gridsize=gridsize, cmap=cmap)
    return fig, ax
