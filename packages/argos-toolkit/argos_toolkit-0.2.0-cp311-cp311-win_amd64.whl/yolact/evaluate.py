...
class Detections:
    ...
    def dump_web(self):
        ...
        with open(
            os.path.join(args.web_det_path, '%s.json' % cfg.name), 'w'
        ) as f:
            json.dump(output, f)
...
def evalvideo(net: Yolact, path: str, out_path: str = None):
    ...
    def extract_frame(x, i):
        if x[1][i]['detection'] is None:
            ...
            return (x[0][i].to(x[1][i]['detection']['box'].device), [x[1][i]])
    ...
    with open(fname, 'w') as fd:
        ...
        for bbox_dict in out_bboxes:
            line = ','.join(
                [
                    str(_class) + ',' + ','.join(map(str, bbox))
                    for bbox, _class in bbox_dict.items()
                ]
            )
            fd.write(f'{line}\n')
    cleanup_and_exit()
...
def evaluate(net: Yolact, dataset, train_mode=False):
    ...
    dataset_size = (
        ...
        else min(args.max_images, len(dataset))
    )
...
def calc_map(ap_data):
    ...
    all_maps = {
        k: {j: round(u, 2) for j, u in v.items()} for k, v in all_maps.items()
    }
    return all_maps
...
if __name__ == '__main__':
    ...
    if args.config is None:
        ...
        print(
            'Config not specified. Parsed %s from the file name.\n'
            % args.config
        )
        set_cfg(args.config)
    ...
    with torch.no_grad():
        ...
        tend = datetime.now()
        dt = tend - tstart
        print('Time to evaluate:', dt.seconds + dt.microseconds * 1e-6)
