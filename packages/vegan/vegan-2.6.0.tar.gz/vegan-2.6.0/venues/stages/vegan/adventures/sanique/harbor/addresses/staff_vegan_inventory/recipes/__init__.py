...
def addresses_staff_vegan_inventory_recipes (packet):
	
	blueprint = sanic.Blueprint (
		"staff_vegan_inventory_recipes", 
		url_prefix = "/staff/vegan_inventory/recipes"
	)
	
	@blueprint.route ("/insert_1")
	#@openapi.parameter ("opener", str, "header")
	async def addresses_staff_vegan_inventory_recipes_insert_1 (request):
