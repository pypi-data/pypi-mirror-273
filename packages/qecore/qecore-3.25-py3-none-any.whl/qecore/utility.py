...
def get_application(context, application) -> Application:
    ...
    try:
        ...
        assert context.sandbox.default_application is not None, "\n".join(
            (
                ...
                "This is indication that no application was defined in environment.py file.",  # noqa: E501
            )
        )
    ...
    ), "Application class was not found. Usually indication of not installed application."  # noqa: E501
...
def plain_dump(node) -> None:
    ...
    def dump_std_out(item, depth) -> None:
        print(
            SPACER * depth
            + str(item)
            + f"     [p:{item.position}, s:{item.size}, vis:{item.visible}, show:{item.showing}]"  # noqa: E501
        )
    ...
    def scenario_run_with_retries(scenario_run, *args, **kwargs):
        for attempt in range(1, max_attempts + 1):
            ...
            if attempt < max_attempts + 1:
                ...
                log.debug(f"{logic} scenario attempt '{attempt}'.")
            ...
            if not scenario_run_failed and not stability:
                ...
                log.debug(f"Auto retry scenario passed after '{attempt}' attempts.")
                return False
            ...
            if scenario_run_failed and stability:
                ...
                log.debug(f"The scenario is not stable, fail on attempt '{attempt}'.")
                return True

        # If stability is in effect and we get here, the logic passes.
        if stability:
            # If it got to this point nothing has failed.
            log.debug(f"The scenario is stable after '{attempt}' attempts.")
            return False

        # If stability is not in effect it should end much earlier.
        log.debug(f"Auto retry scenario failed after '{attempt}' attempts.")
        return True
