...
from qecore.utility import run, run_verbose
...
class Matcher:
    ...
    def __init__(self, context) -> None:
        ...
        if run_verbose("which fnome-screenshot")[1] == 0:
            self.capture_image_cmd = f"gnome-screenshot -f "
        else:
    ...
    def capture_image(self, screen_path=None) -> None:
        ...
        remove_screen = run_verbose(f"rm -rfv {screen_path}")
        log.debug(f"Image matching remove older image: '{remove_screen}'")

        capture_image = run_verbose(self.capture_image_cmd + screen_path, timeout=5)
        if capture_image[0].startswith("file:"):
            screen_captured = capture_image[0].replace("file://", "").strip("\n")
            copy_screen = run_verbose(f"cp {screen_captured} {screen_path}")
            log.debug(f"Image matching copy captured image: '{copy_screen}'")
...
def debug_click_via_image(context, atspi_object, click_button=1):
    ...
    log.debug(f"File for grid debug: '{grid_debug_image}'")
    ...
    if not hasattr(context, "grid_shown") or (hasattr(context, "grid_shown") and not context.grid_shown):
        log.debug("Writing grid_debug_image")
    ...
    log.debug(f"File for click debug: '{click_point_debug_image}'")
    ...
    log.debug("Writing click_point_debug_image")
    cv2.imwrite(click_point_debug_image, click_point_image_target)

    log.debug(f"Clicking at x:'{x}', y:'{y}' to obj {atspi_object.name}")
    ...
    context.embed(mime_type="image/png", data=grid_debug_image, caption="Grid Image",)
    cap = f"Click ({x}, {y})"
    context.embed(mime_type="image/png", data=click_point_debug_image, caption=cap,)
    sleep(1)