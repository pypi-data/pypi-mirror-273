...
class DynamodbStorageClient(AbstractStorageClient):
    ...
    def list_for_partition_key(self, partition_key_name: str, partition_key_value: str, item_model_type: BaseDynamodbModel | None = None):
        key_condition_expression = Key(partition_key_name).eq(partition_key_value)
        resp = self.table.query(KeyConditionExpression=key_condition_expression)
        table_items = resp["Items"]
        if item_model_type:
            return list(self.iterate_table_item_models(table_items=table_items, item_model_type=item_model_type))
        return table_items
