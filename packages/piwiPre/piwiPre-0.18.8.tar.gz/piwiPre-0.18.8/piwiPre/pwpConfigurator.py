...
# FIXME REQ 6118: DirChooser long directories are managed  with a vertical scrollbar
...
# FIXME REQ 6123: MINOR bug: when --debug is changed by the GUI, it is not taken into account when running piwiPre
# REQ 6124: MINOR bug: messenger is not completely readable through scroll-bar & mouse events
# FIXME REQ 6125: when piwiPre is running: run a spinner, disable SAVE, RUN, CANCEL, UNDO

# TODO: test Configurator and piwiPre GUI without HOME/.piwiPre.ini
# TODO: test  "pwpInstaller --gui false --mode install --piwipre --ffmpeg "
...
class PwpEditorUI(PwpGui):
    ...
    def __init__(self, father: "PwpConfigurator", config: PwpConfig or None = None):
        ...
        # TODO: Change these URLs to actual doc
        ...
        self.help_button = GuiButton(self, self.frm, column=6, row=row, text="Help", fr_text="Aide",
                                     background="blue",
                                     command=lambda: webbrowser.open(self.en_url),)
        ...
        self.save_button = GuiButton(self, self.frm, column=5, row=row, text="Save .ini", fr_text='Écrit .ini',
                                     command=self.save)
        ...
        if self.test_scenario:
            ...
            if self.test_scenario:
                ...
                if "exit" in self.test_scenario and self.test_scenario["exit"] == 'true':
                    self.exit()
        elif not self.do_gui:  # TODO: verify we Need to keep this for test scenario with --test-gui
            self.run()
        else:
            self.setup_has_changed = True
            self.compute_constraints()
...
"""
        ...
        if self.bat:
            ...
            # filename = base + ("\\pwpInstaller.lnk" if platform.system() == "Windows" else '/pwpInstaller.sh')
            # build_file(filename, "pwpInstaller", True)
