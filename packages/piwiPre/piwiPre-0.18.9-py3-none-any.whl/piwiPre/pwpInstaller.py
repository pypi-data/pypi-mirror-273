...
class Installer:
    def __init__(self, arguments=None):
        ...
        if not self.args.user:
            ...
            self.msg(f"get user from environment ")
        ...
        if self.args.elevation == "true":
            try:
                ...
                print(f"exception {exx}")

        self.cur_dir = os.path.abspath(self.args.chdir or (os.path.expanduser('~') + r'\Downloads'))
    ...
    def launch_piwi_pre(self):
        ...
        if not os.path.isfile(target):
            ...
            return
        try:
            subprocess.run([target, "--gui", "true", "--home", self.user_home],
                           shell=True,
    ...
    def compute_default_values(self):
        ...
        self.msg(f"PRGF = {self.user_program_files}")
    ...
    def install_piwipre(self):
        def build_lnk(source, dst, arguments='--gui true'):
        ...
        build_lnk(piwipre_abs, piwipre_lnk)
        ...
        build_lnk(installer_abs, installer_lnk)
        ...
        build_lnk(piwipre_abs, menu + '/piwiPre.lnk', arguments=f" --gui true --home {self.user_home}")
        build_lnk(installer_abs, menu + '/pwpInstaller.lnk', arguments=f" --gui true --home {self.user_home}")
