...
class PwpInstallerUi(PwpGui):
    def __init__(self, args, installer):
        ...
        GuiButton(self, self.frm, text="Install", fr_text="Installer", command=self.run, width=8,
                  background="green", column=3, row=row)

        GuiButton(self, self.frm, text="Help", fr_text="Aide", command=lambda: webbrowser.open(self.en_url),
...
class Installer:
    def __init__(self, arguments=None):
        ...
        cur_dir = self.args.chdir or (os.path.expanduser('~') + r'\Downloads')
        self.makedirs(cur_dir)
        os.chdir(cur_dir)
    ...
    def launch_piwi_pre(self):
        ...
        if not os.path.isfile(target):
            ...
            return
        try:
            subprocess.run([target, "--gui", "true"],
                           shell=True,
    ...
    def install_piwipre(self):
        def build_lnk(source, dst):
            if not os.path.isfile(dst):
                pylnk3.for_file(source, dst, arguments='--gui true', window_mode="Minimized")
            self.msg(f"build menu item '{os.path.basename(source)}'")
        ...
        menu = os.environ['APPDATA'] + r"\Microsoft\Windows\Start Menu\Programs" if self.args.mode != "test" else "."
        menu += r'\piwiPre'
        ...
        build_lnk(r'C:\Program Files (x86)\piwiPre\piwiPre.exe', menu + '/piwiPre.lnk')
        build_lnk(r'C:\Program Files (x86)\piwiPre\pwpInstaller.exe', menu + '/pwpInstaller.lnk')
