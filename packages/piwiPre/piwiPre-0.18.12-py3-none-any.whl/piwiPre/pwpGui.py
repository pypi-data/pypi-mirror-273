...
class GuiMessager:
    def __init__(self, root_gui: PwpGui, frm, row, title, fr_title, height=5, width=1400):  # width is in pixel !
        ...
        self.feedback = tkinter.Text(self.scroller.frm, background="#e00e00e00",
                                     padx=3, pady=3,
                                     font=font,
                                     #  height=height,
                                     width=chars,)
        self.feedback.pack()
        self.feedback.tag_config('orange', foreground="#ff5500")
...
class GuiButton(tkinter.Button):
    ...
    def disable(self):
        self.configure(background="#cccCCCccc")
        self["state"] = 'disabled'
...
class _DirChooserField:
    def __init__(self, root_gui: 'GuiDirChooser', path, abspath, row, max_width1, max_width2, _choice):
        self.root_gui = root_gui
        self.label = GuiLabel(root_gui, self.root_gui.frm,
                              text=path, fr_text=path,
                              column=0, row=row, width=max_width1)
        self.abspath = GuiLabel(root_gui, self.root_gui.frm,
        ...
        # if choice:
        if True:
            self.choose = GuiButton(root_gui, self.root_gui.frm, text="Choose", fr_text="Choisir",
                                    command=lambda: self.root_gui.select(path), sticky="N",
                                    background="green", column=3, row=row)

        # else:
        if True:
            self.enter = GuiButton(root_gui, self.root_gui.frm, text="Enter", fr_text="Entrer",
...
class GuiDirChooser(PwpGui):
    ...
    def __init__(self, father, dir_name, name, called,
                 self_test=False, x=None, y=None, language="en"):
        """
        class PwpDirChooser(PwpGui):
        :param father:  calling objet. must implement called(path)
        ...
        self.max_width = int(len(title) * 1.5)
    ...
    def build_list(self):
        ...
        first = True
        for line in all_lines:
            self.folders.append(_DirChooserField(self, path=line[0], abspath=line[1], row=row, _choice=first,
...
class GuiSeparator:
    def __init__(self, root_gui, frm, row, text, fr_text):
        self.frm = frm
        self.frame = tkinter.Frame(self.frm, width=1300, height=10, background="#dddDDDeee")
        self.frame.grid(column=0, row=row, columnspan=9, sticky="W")  # noqa
        self.label = GuiLabel(root_gui, frm, column=0, row=row, background="#dddDDDeee",
...
class GuiScrollable(ttk.Frame):
    ...
    def on_mouse_wheel(event):
        ...
        # v
        # Y
        for canvas in GuiScrollable.canvases:
