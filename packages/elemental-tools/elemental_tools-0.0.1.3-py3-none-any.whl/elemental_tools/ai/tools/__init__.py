...
__logger__ = Logger(app_name="ai", owner=get_package_name(__name__), destination=None).log
...
def get_random_syn(word: str):
    syn_list = get_syn_list(word)
    return random.choice(syn_list)


def get_currency_symbols(currency: str):
    c = currency.lower()
    s = ['$']
    if c == "usd":
        s = ['usd', '$']
...
def to_nlp(models, _ndarray: np.ndarray):
    unique_values = []
    seen_values = set()
    for item in _ndarray:
...
class Tools:
    # import statics to class

    to_nlp = to_nlp

    beautify = beautify
    clean_phone = clean_phone
    codes_and_languages = codes_and_languages

    retrieve_amounts = retrieve_amounts
    retrieve_dates = retrieve_dates

    generate_combinations = generate_combinations

    get_syn_list = get_syn_list
    get_random_syn = get_random_syn
    ...
    def get_amounts(self, currency: str):
        result = []
        currency_symbol = get_currency_symbols(currency)
    ...
    def is_junk(self, word: str):
        # __logger__('error', f'Searching for {word} in junk: {self.junk}')

        if len(np.where(self.junk == word)[0]):
            # __logger__('error', f'Word {word} is junk!')
            return True
        # __logger__('error', f'Word {word} is not junk...')
        return False

    def add_junk(self, word: str):
        # __logger__('error', f"adding {word} with type: {type(word)} to junk : {self.junk}")
        _array = np.array([word])
        self.junk = np.concatenate((_array, self.junk), axis=0)
        # __logger__('error', f"junk after addition {self.junk}")
        return self.junk
