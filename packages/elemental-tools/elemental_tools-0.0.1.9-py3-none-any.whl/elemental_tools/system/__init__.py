...
def run_cmd(command, debug: bool = False, supress: bool = True, expected_status: int = 0):
    ...
    """
    __logger__ = Logger(app_name=os.getenv('APP_NAME', 'elemental_tools'), owner='cmd').log
...
class Cache:
    __logger__ = Logger(app_name=os.getenv('APP_NAME', 'elemental_tools'), owner='cache').log
    ...
    def __init__(self, file: str = cache_file):
        ...
        self.cache_file_content = open(self.__cache_file__, 'a+')
    ...
    def destroy(self):
        try:
            ...
            shutil.rmtree(os.path.dirname(self.__cache_file__))
        except:
            self.__logger__("alert", f"Failed to Destroy: {self.__cache_file__}")
