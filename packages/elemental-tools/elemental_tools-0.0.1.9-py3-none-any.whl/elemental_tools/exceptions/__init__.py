...
class ScriptModuleMissing:
    ...
    def __init__(self, title: str):
        __logger__("error", f"Script module missing when validating: {title}", origin=__task_origin__)


class ParameterMissing(HTTPException):
    def __init__(self, title):
        super().__init__(detail=f"The Argument: {title} is Missing.", status_code=500)
        __logger__('error', f"The Argument: {title} is Missing.", origin=__http_origin__)
    ...
    def __init__(self, title):
        Exception.__init__(self, f"The Setting: {title} cannot be found on the database\n"
                                ...
                                """
                                 ...
                                 f"\n\n\tIf it was not your intent, try to run: pip3 install elemental-tools && elemental -i -e /path/to/.env")
...
class InvalidCSV(Exception):
    ...
    def __init__(self, link):
        ...
        __logger__("error", f"Cannot find csv on the provided link: {link}")
...
class Unauthorized(HTTPException):
    ...
    def __init__(self, item=None, message=None, retry_times: int = None, detail: str = None, status_code: int = status_code):
...
class UnauthorizedDevice(HTTPException):
    ...
    def __init__(self):
        super().__init__(detail="Unauthorized Device. Please check your e-mail box.", headers={"message": self.message}, status_code=self.status_code)
