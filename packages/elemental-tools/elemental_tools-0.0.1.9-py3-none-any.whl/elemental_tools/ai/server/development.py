...
from elemental_tools.ai.brainiac import Brainiac, NewRequest
from deep_translator import GoogleTranslator
from langdetect import detect_langs

from elemental_tools.ai.exceptions import Error, Unauthorized, InvalidOption
...
class DevServer:
	...
	def __init__(self, brainiac: Brainiac, debug: bool = False, bypass_translator: bool = False, bypass_language_adaptation: bool = False):
		...
		self.__logger__ = Logger(app_name='server', owner='development').log
		...
		while True:
			...
			if not self.bypass_translator:
				self.__logger__('info', f"The detected language is {_language}")
				try:
					self.__logger__('info', "Translating input information")
					processable_text = GoogleTranslator(source='auto', target='en').translate(
						_user_input.lower())

					if processable_text.lower() != _user_input.lower():
						self.__logger__('success', "The message was translated")
					else:
						self.__logger__('alert', "The message stays untranslated")
	...
	def process_request(self, _request):
		...
		if not self.bypass_translator:
			...
			try:
				...
				self.__logger__('info', f"Translating output from 'en' to {_to_lang}")
