...
from elemental_tools.ai.brainiac.prediction import max_similarity, quick_look
from elemental_tools.ai.exceptions import Unauthorized, InvalidOption, Error
from elemental_tools.ai.tools import BrainiacRequest, MessageStats, nlp_models, Tools
from elemental_tools.ai.brainiac.decorators import ExamplesModel, Grimoire, GrimoireClass, GrimoireSkillsLoader
...
class Brainiac:
    ...
    def __init__(self, *grimoires: Grimoire, threshold_skill: float = 0.50, threshold_args: float = 0.50,
        ...
        self.__logger__ = Logger(app_name=self.config.app_name, origin='brainiac', owner=get_package_name(__name__)).log

        self.__logger__('start', f"Initializing Brainiac")
        ...
        self.__logger__('success', f"Loaded Successfully. Here's your Brainiac Skill List: {[self.grimoire_skills.keys()]}",
                    origin='initialization')
        self.__logger__('success', f"Here's your Documented Brainiac Skill List: {[self._available_skills.keys()]}",
                    origin='initialization')
    ...
    def execute_function(self, request):
        ...
        if request.skill.exception is None:
            self.__logger__('info', f"Starting Skill Function: {request.skill.function}")
            if len(request.skill.parameters):
                ...
                for param, param_default in request.skill.parameters.items():
                    self.__logger__('info', f"Attempting to Retrieve: {param} with {param_default.default}")
            ...
            if not len(request.skill.args):
                ...
                self.__logger__('info', f"without parameters")

                request.skill.result = request.skill.function()
            else:

                self.__logger__('info', f"with parameters and args: {request.skill.args}")

                request.skill.result = request.skill.function(**request.skill.args)

            if request.output_function is not None:
                self.__logger__('info', f"Running output function: {request.output_function}")
    ...
    def retrieve_function(self, request: BrainiacRequest):
        ...
        self.__logger__('info', "Retrieving function for request message...")

        def process_request():

            self.__logger__('info', "Processing request message...")
            ...
            self.__logger__(f'info', f'Checking Depends for {request.skill.name}')

            if request.skill.authorized:
                self.__logger__(f'success', f'Depends returns True to {request.skill.name}')
                return request
            else:
        ...
        self.__logger__('info', "Iterating over skill list to find the right option with quick look")
        ...
        for skill_id, skill_request_model in self.grimoire_skills.items():
            ...
            if current_max_similarity >= 1.0:
                self.__logger__('success', "Quick look works awesome! Processing the perfect matched skill...")
                return process_request()

        if current_max_similarity > self.threshold_skills:
            self.__logger__('success', "Quick look find a most similar function!")
            return process_request()

        self.__logger__('alert', "Cannot find what is not there! (Not by quick looking)")

        self.__logger__('info', "We must iterate over the skills, to find the right one...")
        # iterate over skills to find the correct option
        for skill_id, skill_request_model in self.grimoire_skills.items():
            self.__logger__('info', f"Processing skill: {str(skill_id)}")
            request.skill = skill_request_model
            self.__logger__('info', f"Checking for Max Similarity")
            ...
            if request.skill.confidence is not None:
                if request.skill.confidence >= 1.0:
                    self.__logger__('success', "Max similarity works awesome!")
                    return process_request()
        ...
        self.__logger__('alert', f"Reiteration reached the end")

        self.__logger__('info', f"Checking dependencies for the similar options.")
        ...
        try:
            if all([dp() for dp in [pf.depends for pf in self.possible_functions if pf.confidence == max(
                ...
                self.__logger__('error', f"One dependency blocks the suggested options.")
    ...
    def process_message(self, request):
        ...
        self.__logger__('info', "Building input nlp models...")
        _request.message_model = nlp_models(_request.message)
        self.__logger__('success', "Nlp models successfully built!")
        self.__logger__('info',
        ...
        if _request.message_model_lemma:
            self.__logger__('success', "Lemmas were generated for the message to be processed!")
        else:
            self.__logger__('alert', "Failed to generate lemmas for the message")
