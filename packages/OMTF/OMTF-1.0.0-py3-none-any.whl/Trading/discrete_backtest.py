...
class DiscreteBackTest(Backtest):
    ...
    config: BtConfig = BtConfig((dt.date.today() - dt.timedelta(days=365*2)).strftime('%Y-%m-%d'), 
                      ...
                      continue_onecandle=True, allocate=False)
    open_trades: list = []
    closed_trades: list = []
    balance: list = []
    ...
    def backtest(self, df:pd.DataFrame=None) -> pd.DataFrame:
        ...
        for g in self.data_df.groupby('DateTime'):
            ...
            for i in g[1].index:
                ...
                candle = copy.deepcopy(g[1].loc[i])
                        ...
                        if candle[f'{strat}Entry'] != 0 and trades_qty < self.config.max_trades:
                            ...
                            if side != None:
                                ...
                                if not self.isEntered(open_trades=self.open_trades, entry=entry, candle=candle, 
                                    ...
                                    risk: list = []
                                    filtered_trades: list = self.getFilteredTrades(self.closed_trades, candle['Ticker'], 
                                                                                strategy.name)
                                    filtered_trades: pd.Series = self.tradesDF(filtered_trades)['PctRet'] \
                                                            if len(filtered_trades) > 0 else pd.Series()
                                    ...
                                    if len(risk) == 0:
                                        raise ValueError(f'There is no type of risk defined for the asset ({asset.name}) or the strategy {strat}')
                                    ...
                                    current_capital: float = self.currentCapital(self.balance[-1], open_trades=self.open_trades, 
                                                                                capital_type=self.config.capital_type)
                                    trade: Trade = Trade(candle, side, strategy, entry, current_capital, risks=risk, 
    ...
    def weekDayKPI(self, trades:pd.DataFrame=None, df:bool=False):
        ...
        trades['WeekDay']: pd.Series = trades['Date'].dt.day_name()
        day_stats: dict = {}
...
if __name__ == '__main__':
    ...
    config: BtConfig = BtConfig('2010-01-01', dt.date.today().strftime('%Y-%m-%d'), 
    ...
    # bbRsiStrat, stochDip
    strategies: dict = {
        'bbRsiStrat': StrategyConfig(name='bbRsiStrat', use_sl=True, use_tp=True, time_limit=100, 
                                    timeframe='H1', allocation=0.5, 
                                    assets={
                                        'QQQ':AssetConfig(name='QQQ', sl=4.0, tp=10.0, order=OrderType.LIMIT, 
                                                        leverage=Leverage(Leverage.Type.SIZE, 1),
                                                        min_size=1, max_size=5000, risk=None, allocation=0.5,
                                                        account_currency='EUR', asset_currency='USD',
                                                        commissions=etf_com),
                                    ...
                                    risk=RiskCalculation(risk_type=RiskCalculation.Type.EQUAL, 
                                                    default_risk=0.01, bounded=False, scale=True)),
        'stochDip': StrategyConfig(name='stochDip', use_sl=True, use_tp=True, time_limit=100, 
                                    timeframe='H1', allocation=0.5, 
                                    assets={
                                        'QQQ':AssetConfig(name='QQQ', sl=4.0, tp=10.0, order=OrderType.LIMIT, 
                                                        leverage=Leverage(Leverage.Type.SIZE, 1),
    ...
    strategies: dict = {
        'atrExt': StrategyConfig(name='atrExt', use_sl=True, use_tp=True, time_limit=100, 
                                     timeframe='H1', allocation=0.5, assets=assets,
                                     risk=RiskCalculation(risk_type=RiskCalculation.Type.EQUAL, 
    ...
    signals: DiscreteSignals = DiscreteSignals(backtest=True, side=DiscreteSignals.Side.LONG, errors=False)
    ...
    # Backtest
    bt: DiscreteBackTest = DiscreteBackTest(strategies=strategies, config=config)
