...
from gdmty_django_defender.data import get_approx_account_lockouts_from_login_attempts
...
class AccessAttemptTest(DefenderTestCase):
    ...
    @patch("gdmty_django_defender.config.USERNAME_FAILURE_LIMIT", 3)
    ...
    @patch("gdmty_django_defender.config.IP_FAILURE_LIMIT", 3)
    ...
    @patch("gdmty_django_defender.config.BEHIND_REVERSE_PROXY", True)
    @patch("gdmty_django_defender.config.REVERSE_PROXY_HEADER", "HTTP_X_FORWARDED_FOR")
    ...
    @patch("gdmty_django_defender.config.LOCKOUT_URL", "http://localhost/othe/login/")
    ...
    @patch("gdmty_django_defender.config.LOCKOUT_URL", "/o/login/")
    ...
    @patch("gdmty_django_defender.config.LOCKOUT_TEMPLATE", "gdmty_django_defender/lockout.html")
    def test_failed_login_redirect_to_template(self):
        ...
        response = self._login()
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "gdmty_django_defender/lockout.html")
    ...
    @patch("gdmty_django_defender.config.COOLOFF_TIME", 0)
    @patch("gdmty_django_defender.config.LOCKOUT_COOLOFF_TIMES", [0])
    ...
    @patch("gdmty_django_defender.config.DEFENDER_REDIS_NAME", "default")
    def test_get_redis_connection_django_conf(self):
        """ get the redis connection """
        redis_client = get_redis_connection()
        self.assertIsInstance(redis_client, Redis)

    @patch("gdmty_django_defender.config.DEFENDER_REDIS_NAME", "bad-key")
    ...
    @patch("gdmty_django_defender.config.BEHIND_REVERSE_PROXY", True)
    @patch("gdmty_django_defender.config.REVERSE_PROXY_HEADER", "HTTP_X_PROXIED")
    ...
    @patch("gdmty_django_defender.config.BEHIND_REVERSE_PROXY", True)
    @patch("gdmty_django_defender.config.REVERSE_PROXY_HEADER", "HTTP_X_REAL_IP")
    ...
    @patch("gdmty_django_defender.config.USE_CELERY", True)
    ...
    @patch("gdmty_django_defender.config.LOCKOUT_BY_IP_USERNAME", True)
    ...
    @patch("gdmty_django_defender.config.DISABLE_IP_LOCKOUT", True)
    ...
    @patch("gdmty_django_defender.config.DISABLE_USERNAME_LOCKOUT", True)
    ...
    @patch("gdmty_django_defender.config.ATTEMPT_COOLOFF_TIME", "a")
    ...
    @patch("gdmty_django_defender.config.LOCKOUT_COOLOFF_TIMES", "a")
    def test_bad_lockout_cooloff_configuration(self):
        self.assertRaises(Exception)

    @patch("gdmty_django_defender.config.LOCKOUT_COOLOFF_TIMES", [300, "a"])
    def test_bad_list_lockout_cooloff_configuration(self):
        self.assertRaises(Exception)

    @patch("gdmty_django_defender.config.LOCKOUT_COOLOFF_TIMES", [300, dict(a="a")])
    def test_bad_list_with_dict_lockout_cooloff_configuration(self):
        self.assertRaises(Exception)

    @patch("gdmty_django_defender.config.LOCKOUT_COOLOFF_TIMES", [3, 6])
    @patch("gdmty_django_defender.config.IP_FAILURE_LIMIT", 3)
    ...
    @patch("gdmty_django_defender.config.STORE_ACCESS_ATTEMPTS", False)
...
class TestUtils(DefenderTestCase):
    ...
    def test_remove_prefix(self):
        ...
        self.assertEqual(utils.remove_prefix(
            "gdmty_django_defender:blocked:ip:192.168.24.24", "gdmty_django_defender:blocked:"), "ip:192.168.24.24")
        self.assertEqual(utils.remove_prefix(
            "gdmty_django_defender:blocked:username:johndoe", "gdmty_django_defender:blocked:"), "username:johndoe")
        self.assertEqual(utils.remove_prefix(
            "gdmty_django_defender:blocked:username:johndoe", "blocked:username:"),
            "gdmty_django_defender:blocked:username:johndoe")
...
class TestRedisConnection(TestCase):
    ...
    @patch("gdmty_django_defender.config.DEFENDER_REDIS_URL", REDIS_URL_PLAIN)
    @patch("gdmty_django_defender.config.MOCK_REDIS", False)
    ...
    @patch("gdmty_django_defender.config.DEFENDER_REDIS_URL", REDIS_URL_PASS)
    @patch("gdmty_django_defender.config.MOCK_REDIS", False)
    ...
    @patch("gdmty_django_defender.config.DEFENDER_REDIS_URL", REDIS_URL_NAME_PASS)
    @patch("gdmty_django_defender.config.MOCK_REDIS", False)
