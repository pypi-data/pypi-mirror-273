...
import sys
import click
from poonia.utils import log, get_in, sliding_window
...
@io.command(help='Enumerate range', name='range')
@click.argument('int_range', type=str)
@click.option('--zero-pad', '-z', type=int, default=0)
def range_enumerate(int_range, zero_pad):
    for i in expand_range(int_range):
        s = str(i).zfill(zero_pad)
        click.echo(s)
