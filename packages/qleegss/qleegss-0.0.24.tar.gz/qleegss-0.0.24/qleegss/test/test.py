...
if __name__ == '__main__':
    ...
    try:
        ...
        if data.device == 101:
            ...
            print(f"Current memory usage is {current / 10 ** 6}MB; Peak was {peak / 10 ** 6}MB")
            # 慢波图绘制
            if data.sti is not None:
                if data.isHighFreq:
                    plot_sw_hf(data.eeg, data.start_time, data.sf_eeg, data.eeg_path, data.sti)
                else:
                    plot_sw(data.eeg, data.start_time, data.sf_eeg, data.eeg_path, data.sti)
            else:
                pass
