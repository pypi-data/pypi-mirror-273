...
class TestCosimPyCoSim(unittest.TestCase):
    ...
    def test_add_nsti_to_file_name(self):
        ...
        # arrange
        sim_number = 25
        path_cosim_data = os.path.join(os.getcwd(), 'local_PyCoSim_folder', 'CCT_FiQuS_LEDET', f'CCT_FiQuS_LEDET_{sim_number}.yaml')
        verbose = True
    ...
    def test_run_PyCoSim_CCT_FiQuS_LEDET(self):
        ...
        output_path_common = Path(os.path.join(os.getcwd(), self.settings.local_PyCoSim_folder, cosim_data.GeneralParameters.cosim_name)).resolve()
        delete_if_existing(output_path_common)
        ...
        # output files the same
        assert_equal_yaml(output_file_path_FiQuS_B_output_brep, reference_file_path_FiQuS_B_output_brep, max_relative_error=max_relative_error)
        ...
        self.assertTrue(os.path.isfile(os.path.join(output_path_common, 'LEDET', str(nsti_LEDET_A.n),
                                                    cosim_data.Simulations[LEDET_set_name].modelName,
                                                    cosim_data.Simulations[
                                                        FiQuS_set_name].files_to_copy_after.pre_cosim[
                                                        0].new_file_name_relative_path)))  # this entry matches the entry from the FiQuS model (not LEDET)
