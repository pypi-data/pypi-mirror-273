...
class TestDataModel(unittest.TestCase):
    ...
    def test_writeToFile_rutherford(self):
        """
            Check that DataModelMagnet generates a structure with the same keys as a reference file
        """
        # arrange
        generated_file = os.path.join('output',      'data_model_magnet_TEST_rutherford.yaml')
        reference_file = os.path.join('references', 'data_model_conductor_REFERENCE_rutherford.yaml')
        ...
        dict_to_yaml(data.Conductors[0].dict(by_alias=True), generated_file, list_exceptions=['Conductors'])
        # assert
    ...
    def test_writeToFile_ribbon(self):
        ...
        """
        # arrange
        generated_file = os.path.join('output',      'data_model_magnet_TEST_ribbon.yaml')
        reference_file =  os.path.join('references', 'data_model_conductor_REFERENCE_ribbon.yaml')
    ...
    def test_writeToFile_mono(self):
        ...
        """
        # arrange
        generated_file = os.path.join('output', 'data_model_magnet_TEST_mono.yaml')
        reference_file = os.path.join('references', 'data_model_conductor_REFERENCE_mono.yaml')
        ...
        data: DataModelMagnet = DataModelMagnet()
        data.Conductors[0] = Conductor(cable={'type': 'Mono'}, strand={'type': 'Rectangular'}, Jc_fit={'type': 'CUDI1'})
        data.Power_Supply.I_control_LUT = [0, 0]  # example of list
        data.CoilWindings.electrical_pairs.group_together = [[1, 2], [3, 4]]  # example of list of lists
