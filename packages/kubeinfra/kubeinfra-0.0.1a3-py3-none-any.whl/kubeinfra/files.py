...
class Files:
    ...
    def list(self) -> Dict[str, List[Dict[str, Union[str, int, float]]]]:
        # send request
        response = create_get_request(kubeinfra.api_base_files)
    ...
    def upload(
        ...
        if check:
            ...
            if not report_dict["is_check_passed"]:
                raise kubeinfra.FileTypeError(
                    f"Invalid file supplied. Failed to upload.\nReport:\n {report_dict}"
                )
        else:
            logger.warning(
                "Caution: File check is disabled. KubeInfra.ai will not be able to detect errors in your file."
            )
            report_dict = {}
        ...
        init_endpoint = kubeinfra.api_base_files[:-1]
        ...
        try:
            ...
            if response.status_code == 401:
                logger.critical(
                    "This job would exceed your free trial credits. Please upgrade to a paid account through Settings -> Billing on api.kubeinfra.ai to continue."
                )
                raise kubeinfra.AuthenticationError(
                    "This job would exceed your free trial credits. Please upgrade to a paid account through Settings -> Billing on api.kubeinfra.ai to continue."
                )
            ...
            file_id = response.headers["X-KubeInfra-File-Id"]
            ...
            preprocess_url = urllib.parse.urljoin(
                kubeinfra.api_base_files, f"{file_id}/preprocess"
            )
    ...
    def delete(self, file_id: str) -> Dict[str, str]:
        delete_url = urllib.parse.urljoin(kubeinfra.api_base_files, file_id)
    ...
    def retrieve(self, file_id: str) -> Dict[str, Union[str, int]]:
        retrieve_url = urllib.parse.urljoin(kubeinfra.api_base_files, file_id)
    ...
    def retrieve_content(self, file_id: str, output: Union[str, None] = None) -> str:
        ...
        retrieve_url = urllib.parse.urljoin(kubeinfra.api_base_files, relative_path)
...
def check_json(
    ...
    try:
        ...
        report_dict["utf8"] = (
            ...
            f"See https://docs.kubeinfra.ai/docs/fine-tuning for more information."
        )
        report_dict["is_check_passed"] = False
        return report_dict
    ...
    with open(file) as f:
        ...
        try:
            for idx, line in enumerate(f):
                ...
                if not isinstance(json_line, dict):
                    report_dict["line_type"] = (
                        ...
                        "see https://docs.kubeinfra.ai/docs/fine-tuning."
                ...
                if "text" not in json_line:
                    report_dict["text_field"] = (
                        ...
                        "see https://docs.kubeinfra.ai/docs/fine-tuning for more information."
                        f"{str(line)}"
                    )
                    report_dict["is_check_passed"] = False
                else:
            ...
            if idx + 1 < kubeinfra.min_samples:
                report_dict["min_samples"] = (
                    f"Processing {file} resulted in only {idx+1} samples. "
                    f"Our minimum is {kubeinfra.min_samples} samples. "
                )
                report_dict["is_check_passed"] = False
            else:
