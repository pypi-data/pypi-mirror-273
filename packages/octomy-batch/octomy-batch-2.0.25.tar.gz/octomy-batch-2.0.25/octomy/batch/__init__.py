...
class Processor:
	...
	async def create_tables(self):
		await self.dbc.query_none(f"{self.batch_sql_base}.create_batch_status_enum", prepare=False)
		await self.dbc.query_none(f"{self.batch_sql_base}.create_batch_items", prepare=False)
		await self.dbc.query_none(f"{self.batch_sql_base}.create_gcra_throttle", prepare=False)
...
'''
	...
	async def insert_batch_item(self, type="test", data=None, ttl_seconds=None, result=None, priority=50, source=None, throttle_key=None, throttle_limit=1, throttle_period=1):
		...
		# fmt: on
		return await self.dbc.key_query(query_key = "{db_base}.insert_batch_item", params=batch_item, mode = "one", item_type = dict)
	...
	async def _update_hang_status(self):
		...
		db_items, db_err = await self.dbc.key_query(query_key = f"{self.batch_sql_base}.update_batch_hang_status", params=updated_status, mode = "none", do_debug = self.do_debug)
		if db_err:
	...
	async def retry_hung_jobs(self):
		...
		db_items, db_err = await self.dbc.key_query(query_key = f"{self.batch_sql_base}.bump_batch_items", params=updated_status, mode = "none", do_debug = self.do_debug)
		if db_err:
	...
	async def bump_job_by_id(self, id, status):
		...
		return await self.dbc.key_query(query_key = f"{self.batch_sql_base}.bump_batch_item_status", params=updated_status, mode = "none", do_debug = self.do_debug)
