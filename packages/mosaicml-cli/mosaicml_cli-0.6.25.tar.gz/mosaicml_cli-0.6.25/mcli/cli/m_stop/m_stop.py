...
def get_runs_to_stop(runs, finetuning_runs=False):
    ...
    already_stopping = [r for r in runs if r.status == RunStatus.TERMINATING]
    filtered_runs = [r for r in runs if not r.status.is_terminal() and not r.status == RunStatus.TERMINATING]
    run_label = 'finetuning run' if finetuning_runs else 'run'

    # If there are runs that are already stopped or being stopped, print an error
    ...
    if len(already_stopping) == 1:
        logger.error(f'{WARN} {already_stopping[0].name} is already stopping. '
                     f'Skipping stop request for this {run_label}')
    elif len(already_stopping):
        logger.error(f'{WARN} {len(already_stopping)} runs are already stopping. '
                     f'Skipping stop request for these {run_label}s')
