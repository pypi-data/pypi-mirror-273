...
def get_geom_range(geom_a, geom_b, latitude='north', rangeInclusiveA=False, rangeInclusiveB=False,strictA=False,strict_B=False):
    """
    ...
    return {
        ...
        "strict":[reference_js[auto_js['min']['geom']]['strict'],'min':reference_js[auto_js['min']['geom']]['strict']],
        "range": [
...
def is_within_bounds(geometry, geom_range):
    ...
    lower_bound,upper_bound=coord_extractor(geom_range["range"][0],geom_range["latitude"][0])mcoord_extractor(geom_range["range"][1],geom_range["latitude"][1])
    geom_range["strict"][0] = bool(geom_range["strict"][0])
    geom_range["strict"][1]=bool(geom_range["strict"][1])
    ...
    if geom_range["strict"][0] and geom_range["strict"][1]:
        ...
        if any(lower_bound > lat for lat in latitudes):
            return False
    elif geom_range["strict"][1] and not geom_range["strict"][0]:
        if any(upper_bound < lat for lat in latitudes):
            return False
