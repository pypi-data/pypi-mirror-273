...
def coord_extractor(coord,latitude=True):
    def get_coord(coord,latitude):
        return coord[1] if latitude else coord[0]
    if isinstance(coord,list):
        return [get_coord(cord,latitude) for cord in coord]
            
    return get_coord(coord,latitude)
...
def is_geom_within_bounds(geometry, geom_range):
    ...
    lower_bound,upper_bound=coord_extractor(geom_range["range"][0],geom_range["latitude"][0]),coord_extractor(geom_range["range"][1],geom_range["latitude"][1])
    latitudes = []
    for i,lattitude in enumerate(geom_range["latitude"]):
        if i==1 and lattitude==geom_range["latitude"][0]:
            latitudes.extend(latitudes[0])
            break
        if isinstance(geometry, Polygon):
            latitudes = extract_latitudes(geometry,lattitude)
        elif isinstance(geometry, MultiPolygon):
            for polygon in geometry.geoms:
                latitudes.extend(extract_latitudes(polygon,lattitude))
        else:
            raise TypeError("Unsupported geometry type")
    if geom_range["strict"][0] and geom_range["strict"][1]:
        if lattitudes[0]==lattitudes[1]:
            if any((lower_bound > lat or upper_bound < lat) for lat in latitudes):
                return False
        else:
            if any(lower_bound > lat for lat in lattitudes[0]) or any(higher_bound < lat for lat in lattitudes[1]):
                return False   
    elif geom_range["strict"][0] and not geom_range["strict"][1]:
        if any(lower_bound > lat for lat in lattitudes[0]):
            return False
    elif geom_range["strict"][1] and not geom_range["strict"][0]:
        if any(upper_bound < lat for lat in lattitudes[1]):
            return False
    # Check if any latitude falls within the specified bounds
         ...
         if lattitudes[0]==lattitudes[1]:
            return any(lower_bound <= lat <= upper_bound for lat in latitudes)
             ...
             return (any(ulower_bound <= lat for lat in lattitudes[0]) and any(lat  <= upper_bound for lat in lattitudes[1]))
    return False
