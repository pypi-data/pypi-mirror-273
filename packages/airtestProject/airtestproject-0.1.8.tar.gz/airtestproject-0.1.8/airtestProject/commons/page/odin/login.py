...
def r_user_name():
    # 生成一个包含所有字母和数字的字符集
    chinese_chars = ''.join(chr(i) for i in range(0x4e00, 0x9fa6))
    # 使用random.choice随机选择字符，生成指定长度的字符串
    random_string = ''.join(random.choice(chinese_chars) for _ in range(6))
...
class LoginPage:
    ...
    @log.wrap('创建角色or选择角色')
    def create_role(self, create_role_page, select_role_page, role, gender, input_name_pos, enter_game_pos, create_pos,
