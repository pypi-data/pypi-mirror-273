...
from midas.modules.powergrid.model.constrainted import ConstraintedGrid
from midas.modules.powergrid.model.static import PandapowerGrid
...
class TestConstraintedGrid(unittest.TestCase):
    ...
    def test_line_constraints(self):
        self.grid = ConstraintedGrid([["line", 100]])
        gparams = {
    ...
    def test_voltage_constraints(self):
        self.grid = ConstraintedGrid(
            [["sgen", 0.05], ["load", 0.02], ["bus", 0.1]]
        )
        self.ugrid = PandapowerGrid()
        ...
        self.grid.setup("midasmv", 0, gparams)
        self.ugrid.setup("midasmv", 0, gparams)
        step_size = 60
        results = []
        uresults = []
        setpoints = []
        ...
        for bus in uvm_pus:
            ...
            if not checks[bus]:
                self.assertLessEqual(
                    vm_pus[bus],
                    high,
                    msg=f"Voltage of bus {bus} is above limit: {vm_pus[bus]} / {high}",
                )
                self.assertGreaterEqual(
                    vm_pus[bus],
                    low,
                    msg=f"Voltage of bus {bus} is below limit: {vm_pus[bus]} / {low}",
                )
        return checks
