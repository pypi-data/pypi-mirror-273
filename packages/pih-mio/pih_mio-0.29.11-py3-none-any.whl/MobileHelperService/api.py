...
def value_is_file(value: str) -> bool:
...
class MobileOutput(MobileOutputBase, Output):
                    ...
                    self.message_send(
                        js(
                            (
                                "",
                                A.CT_V.WAIT,
                                self.italic(j((text or "Идёт загрузка", "..."))),
                            )
                        )
                    )
    ...
    def bold(self, value: Any) -> str:
        return j(("<*>", str(value).strip(), "</*>"))

    def italic(self, value: Any) -> str:
        return j(("<_>", str(value).strip(), "</_>"))
            ...
            self.write_line(
                j(
                    (
                        ...
                        A.D.list_to_string(
                            A.D.map(
                                lambda item: self.italic(item.caption),
                                A.D.filter(
...
class MobileHelper(OutputStub):
    ...
    def __init__(self, pih: PIH, stdin: Stdin):
        ...
        HOLTER_STUDY_COURSE_CONTENT_LIST: list[HelpContentHolder] = [
            ...
            HelpContentHolder(
                "nn5",
                [self.output.italic("Памятка: Расположение датчиков на теле пациента")],
                [
                    HelpImageContent(
        ...
        self.find_note_node: CommandNode = CommandNode(
            ...
            help_text=lambda: js((" (", self.output.italic("Название заметки"), ")")),
        )
        ...
        self.polibase_person_card_add_to_card_registry_folder_node: CommandNode = (
            CommandNode(
                ...
                help_text=lambda: f" {self.output.italic('название папки')} {self.output.italic('запрос для поиска пациента')}.\nНапример, {self.current_pih_keyword} + п1к {A.CT.TEST.PIN}",
                filter_function=lambda: self.is_not_all or self.in_main_menu,
            )
        )
        ...
        self.address_node: CommandNode = self.create_command_link(
            ...
            self.output.italic("Адресовать команду"),
            ADMIN_GROUPS,
            True,
        )
        ...
        self.address_as_link_node: CommandNode = self.create_command_link(
            ...
            self.output.italic("Адресовать ссылку на команду"),
            ADMIN_GROUPS,
            True,
        )
        ...
        self.all_commands_node: CommandNode = self.create_command_link(
            ...
            [self.output.italic("Все команды")],
            None,
            True,
        )
        ...
        self.exit_node: CommandNode = CommandNode(
            ...
            [None, self.output.italic(A.D.capitalize(self.keywords.command.EXIT[1]))],
            self.session.exit,
            text_decoration_after=lambda: (
                ""
        ...
        self.command_node_tree = {
            ...
            CommandNode(
                ...
                text_decoration_before="🎯 ",
                help_text=lambda: js(
                    (" (", self.output.italic("Название действия"), ")")
                ),
    ...
    def arg_list_exclude_file(self) -> list[str]:
        return A.D.filter(
            lambda item: not value_is_file(item), self.arg_list
        )
        
    @property
    def arg_list_only_file(self) -> list[str]:
        return A.D.filter(
            value_is_file, self.arg_list
        )
    ...
    def run_command_handler(self, command_type: A.CT_CMDT | None = None) -> None:
        ...
        arg: Any = None
        self.arg_list = A.D.sort(lambda item: int(not value_is_file(item)), self.arg_list)
        if n(command_type):
                ...
                for index, content_item in enumerate(content):
                    ...
                    if nn(content_link):
                        ...
                        if content_item.show_loading:
                            ...
                            loading_text += "..."
                            self.write_line(self.output.italic(loading_text))
                        if is_video:
    ...
    def polibase_persons_by_card_registry_folder_handler(self) -> None:
        ...
        self.output.write_result(
            ...
            empty_result_text=self.output.italic("Папка с картами пациентов пуста"),
            use_index=False,
            title=(
                js(
                    (
                        ...
                        self.write_line(
                            j(
                                (
                                    ...
                                    " ",
                                    self.output.italic("для завершения"),
                                )
                            )
                        )
                    ...
                    if_else(
                        ...
                        js(
                            (
                                ...
                                A.CT_V.WARNING,
                                self.output.bold(
                                    self.output.italic(A.CT_FC.POSITION.default_value)
                                ),
                            )
                        ),
                        lambda: j(
