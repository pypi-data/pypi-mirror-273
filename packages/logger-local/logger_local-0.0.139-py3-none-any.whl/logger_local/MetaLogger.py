    ...
    def wrap_function(func: callable, logger: Logger) -> callable:
        ...
        def wrapper(*args, **kwargs) -> any:
            ...
            try:
                ...
                logger.end(full_function_name, object={get_return_variable(func): str(result),
                                                       "extra_kwargs": kwargs_updated})
            return result
...
def get_return_variable(func: callable) -> str:
    ...
    return_pattern = re.compile(r'^\s*return\s+')  # cached by re
    return_variable = next(
        (line.split("return ")[1].split("#")[0].strip() for line in source_lines if
         return_pattern.match(line) and line.split("#")[0].strip() not in ["return", "return None"]), "result")
