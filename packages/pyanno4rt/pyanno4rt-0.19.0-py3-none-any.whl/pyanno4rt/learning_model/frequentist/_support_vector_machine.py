...
class SupportVectorMachineModel():
    ...
    ----------
    model_label : string
        Label for the support vector machine model to be used for file naming.
    ...
    preprocessing_steps : tuple
        ...
        make up the preprocessing pipeline for the support vector machine \
        model. Current available algorithm labels are:
    ...
    oof_splits : int
        ...
        out-of-folds evaluation step of the support vector machine model.
    ...
    prediction_model : object of class `SVC`
        Instance of the class `SVC`, which holds methods to make predictions \
        from the support vector machine model.
    ...
    def __init__(
        ...
        # Get the support vector machine model and its hyperparameters
    ...
    def get_model(
            ...
            labels):
        """
        Get the support vector machine outcome prediction model by reading \
        from the model file path, the datahub, or by training.

        Returns
        -------
        object of class `SVC`
            Instance of the class `SVC`, which holds methods to make \
    ...
    def tune_hyperparameters_with_bayes(
            ...
            labels):
        """
        Tune the hyperparameters of the support vector machine model via \
        ...
        space = {
            'C': hp.uniform(
                'C',
                self.configuration['tune_space']['C'][0],
                self.configuration['tune_space']['C'][1]),
            'kernel': hp.choice(
                'kernel',
                self.configuration['tune_space']['kernel']),
            'degree': hp.choice(
                'degree',
                self.configuration['tune_space']['degree']),
            'gamma': hp.uniform(
                'gamma',
                self.configuration['tune_space']['gamma'][0],
                self.configuration['tune_space']['gamma'][1]),
            'tol': hp.choice(
                'tol',
                self.configuration['tune_space']['tol']),
            'class_weight': hp.choice(
                'class_weight',
                self.configuration['tune_space']['class_weight'])
            }
    ...
    def train(
        ...
        hyperparameters = {'C': tuned_hyperparameters['C'],
                           'kernel': tuned_hyperparameters['kernel'],
                           'degree': tuned_hyperparameters['degree'],
                           'gamma': tuned_hyperparameters['gamma'],
                           'tol': tuned_hyperparameters['tol'],
                           'shrinking': True,
                           'probability': True,
                           'class_weight': tuned_hyperparameters[
                               'class_weight'],
                           'random_state': 3}
    ...
    def read_model_from_file(self):
        """
        Read the support vector machine outcome prediction model from the \
        model file path.
    ...
    def write_model_to_file(
            ...
            prediction_model):
        """
        Write the support vector machine outcome prediction model to the \
        model file path.
    ...
    def read_hyperparameters_from_file(self):
        """
        Read the support vector machine outcome prediction model \
        hyperparameters from the hyperparameter file path.
