"""Random forest outcome prediction model."""
...
class RandomForestModel():
    ...
    random forest model from the input data, inspect the model, make \
    ...
    ----------
    model_label : string
        Label for the random forest model to be used for file naming.
    ...
    preprocessing_steps : tuple
        ...
        make up the preprocessing pipeline for the random forest model. \
    ...
    tune_space : dict
        ...
        - 'bootstrap' : indicator for the use of bootstrap samples to build \
            the trees;
    ...
    oof_splits : int
        ...
        out-of-folds evaluation step of the random forest model.
    ...
    prediction_model : object of class `RandomForestClassifier`
        Instance of the class `RandomForestClassifier`, which holds methods \
        to make predictions from the random forest model.
    ...
    def __init__(
                              ...
                              'tune_space': {
                                  ...
                                  'max_features': tune_space.get(
                                      'max_features', list(range(
                                          1, dataset['feature_values'].shape[1]
                                          + 1))),
                                  'bootstrap': tune_space.get(
                                      'bootstrap', [False, True]),
                                  'warm_start': tune_space.get(
        ...
        # Get the random forest model and its hyperparameters
    ...
    def get_model(
            ...
            labels):
        """
        Get the random forest outcome prediction model by reading from the \
        model file path, the datahub, or by training.

        Returns
        -------
        object of class `RandomForestClassifier`
            Instance of the class `RandomForestClassifier`, which holds \
            methods to make predictions from the random forest model.
        """
    ...
    def tune_hyperparameters_with_bayes(
            ...
            labels):
        """
        Tune the hyperparameters of the random forest model via sequential \
        ...
        space = {
            'n_estimators': hp.choice(
                'n_estimators', self.configuration[
                    'tune_space']['n_estimators']),
            'criterion': hp.choice(
            ...
            'bootstrap': hp.choice(
                'bootstrap', self.configuration['tune_space']['bootstrap']),
            'warm_start': hp.choice(
                'warm_start', self.configuration['tune_space']['warm_start']),
            'class_weight': hp.choice(
                'class_weight', self.configuration[
                    'tune_space']['class_weight']),
            'ccp_alpha': hp.uniform(
                'ccp_alpha',
                self.configuration['tune_space']['ccp_alpha'][0],
    ...
    def predict_oof(
        ...
        def compute_fold_labels(indices):
            ...
            # Initialize the model from the hyperparameter dictionary
            prediction_model = RandomForestClassifier(**self.hyperparameters)
    ...
    def read_model_from_file(self):
        """
        Read the random forest outcome prediction model from the model file \
        path.
    ...
    def write_model_to_file(
            ...
            prediction_model):
        """
        Write the random forest outcome prediction model to the model file \
        path.
    ...
    def read_hyperparameters_from_file(self):
        """
        Read the random forest outcome prediction model hyperparameters from \
