...
def plot_RDTs_along_ring(lattice: CSLattice, RDT_type='f', show=True):
    ...
    for i, ele in enumerate(lattice.elements):
        if isinstance(ele, Sextupole) or isinstance(ele, Octupole):
            ele_slices = ele.slice(ele.n_slices)
        else:
        ...
        for elele in ele_slices:
            ...
            if isinstance(elele, Octupole):
            # if False:
                k2l.append(elele.k3 * elele.length * elele.closed_orbit[0] / (1 + lattice.delta))
                k3l.append(elele.k3 * elele.length / (1 + lattice.delta))
            else:
                k2l.append(elele.k2 * elele.length / (1 + lattice.delta))
                k3l.append(elele.k3 * elele.length / (1 + lattice.delta))
            s.append(elele.s)
