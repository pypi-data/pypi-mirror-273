...
from buildgrid.server.metrics_names import SCHEDULER_QUEUE_ACTION_TIME_METRIC_NAME
from buildgrid.server.metrics_utils import DurationMetric
from buildgrid.server.persistence.sql.impl import SQLDataStore
from buildgrid.server.persistence.sql.models import ClientIdentityEntry
...
class ExecutionInstance(Instance):
    ...
    def _meter_execution(self, client_identity: Optional[ClientIdentityEntry], operation_name: str) -> None:
        ...
        try:
            ...
            LOGGER.exception(f"Failed to publish execution usage, {identity=} {usage=}", exc_info=exc)
