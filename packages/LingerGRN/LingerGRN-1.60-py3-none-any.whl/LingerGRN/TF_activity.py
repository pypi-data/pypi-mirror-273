...
# Apply quantile normalization to 'trans_reg_n'
    ...
    RNA=pd.DataFrame(adata_RNA.raw.X.toarray().T,index=adata_RNA.raw.var['gene_ids'].values,columns=adata_RNA.obs['barcode'].values)
    gene_overlap=list(set(trans_reg.index)&set(RNA.index))  
    RNA=RNA.loc[gene_overlap]
    trans_reg=trans_reg.loc[gene_overlap]
    row=trans_reg.sum(axis=1).values[:,np.newaxis]
    colsum=trans_reg.sum(axis=0).values[:,np.newaxis]
...
def box_comp(TFName,adata_RNA,celltype1,celltype2,datatype,regulon_score,save,outdir):
    ...
    if datatype=='expression':
        data0=pd.DataFrame(adata_RNA.raw.X.toarray().T,index=adata_RNA.raw.var['gene_ids'].values,columns=adata_RNA.obs['barcode'].values)
