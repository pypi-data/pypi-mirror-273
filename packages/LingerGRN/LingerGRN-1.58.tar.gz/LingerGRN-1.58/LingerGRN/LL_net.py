...
## extact the overlapped peaks.
    ...
    return mean_S
def TF_RE_binding(GRNdir,adata_RNA,adata_ATAC,genome,method,outdir,method):
    from tqdm import tqdm
    import numpy as np
    import pandas as pd
    ...
    if method=='LINGER':
        ...
        for i in tqdm(range(23)):
            ...
            TFs = mat.columns
## read the count file.
            TG=pd.DataFrame(adata_RNA.raw.X.T,index=adata_RNA.raw.var['gene_ids'].values,columns=adata_RNA.obs['barcode'].values)
            TFoverlap = list(set(TFs) & set(TG.index))
            mat = mat[TFoverlap]
...
## extact the overlapped peaks.
    ...
    if method=='LINGER':
        ...
        if len(other_RE)>0:
            B_arr = pd.DataFrame(np.zeros((len(other_RE), 3)), columns=mat.columns, index=other_RE)
