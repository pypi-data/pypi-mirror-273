...
class MessageLocal(Item, CompoundMessage, metaclass=MetaLogger, object=object_message):
    ...
    def __init__(self, *, recipients: List[Recipient], api_type_id: int = None, campaign_id: int = None,
        ...
        CompoundMessage.__init__(self, message_id=message_id, campaign_id=campaign_id, is_debug=is_debug,
                                 message_template_id=message_template_id, body=body, form_id=form_id,
                                 subject=subject, recipients=recipients, is_test_data=is_test_data,
    ...
    def get_subject_text_after_template_processing(self, recipient: Recipient, message_channel: MessageChannel = None) -> str:
        seperator = ", "
    ...
    def get_body_text_after_template_processing(self, recipient: Recipient, message_channel: MessageChannel = None) -> str:
        seperator = "\n"
        message_channel = message_channel or self.get_message_channel(recipient)
    ...
    def __can_send_direct(self, sender_profile_id: int, api_data: dict = None) -> bool:
        ...
        try:
            ...
            sleep_after_passing_the_hard_limit = self.api_management_manager.get_seconds_to_sleep_after_passing_the_hard_limit(
                api_type_id=self._api_type_id)
        ...
        try:
            ...
            if arr is None:
                ...
                if api_check != APILimitStatus.GREATER_THAN_HARD_LIMIT:
                    ...
                    x = self.api_management_manager.get_seconds_to_sleep_after_passing_the_hard_limit(
                        api_type_id=self._api_type_id)
                    if x > 0:
