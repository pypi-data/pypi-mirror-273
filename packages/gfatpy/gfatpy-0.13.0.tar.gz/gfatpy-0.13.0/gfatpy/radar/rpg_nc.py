...
class rpg:
    ...
    def __init__(self, path: Path):
        ...
        self.level = int(path.name.split(".")[-2][-1])
        self.raw = enhance_rpgpy_dataset(path)
        self._data = None
        self._band = None
    ...
    def plot_spectra_by_range(
        ...
        **kwargs,
        ...
        ax.set_title(
            f"Time: {target_time:%Y%m%dT%H:%M:%S} | Range: [{range_slice[0]/1e3:.2f} - {range_slice[1]/1e3:.2f}] km"
        )
        ax.legend(ncol=kwargs.get("ncol", 2), loc="upper right", fontsize=8)
    ...
    def plot_zen(self, variable: str | list[str] | None = None, **kwargs) -> tuple[list[Figure], list[Path]]:
        ...
        for var in variables_to_plot:
            ...
            pcm = data[f"{var}"].plot(
                ...
                vmin=vmin,
                vmax=vmax,
                cmap=kwargs.get("colorbar_cmap", "jet"),
            )
    ...
    def plot_ppi(
        self, variable: str | None = None, **kwargs
    ...
    def plot_profile(
        self,
        target_times: datetime | list[datetime] | tuple[datetime, datetime],
        ...
        **kwargs,
        ...
        if isinstance(target_times, tuple):            
            time_list = self.data.sel(time=slice(*target_times)).time.values

            title_str = f"Period: {original_target_times[0]:%Y-%m-%d} {original_target_times[0]:%H:%M:%S} - {original_target_times[-1]:%H:%M:%S}"
            filename = self.path.name.replace(
                ".nc",
                f"_{variable}_profile_{original_target_times[0]:%Y%m%dT%H%M}_{original_target_times[-1]:%Y%m%dT%H%M}.png",
            )
        ...
        data["range"] = data["range"] / 1e3
        for idx, time_ in enumerate(time_list):
            data[variable].sel(time=time_).plot(y='range', ax=ax, color=colors[idx], label=f"{parse_datetime(time_):%H:%M:%S}")
        if 'range_limits' in kwargs:
            range_limits = kwargs.get('range_limits')/1e3
            ax.set_ylim(range_limits)
    ...
    def plot_timeseries(
        self,
        target_ranges: float | list[float] | np.ndarray[Any, np.float64],
        ...
        **kwargs,
        ...
        original_target_ranges = target_ranges        
        if isinstance(target_ranges, tuple):            
            ...
            filename = self.path.name.replace(
                ".nc",
                f"_{variable}_timeseries_{original_time_limits[0]:%Y%m%dT%H%M}_{original_time_limits[-1]:%Y%m%dT%H%M}_{original_target_ranges[0]:.1f}_{original_target_ranges[-1]:.1f}.png",
            )
            ...
            filename = self.path.name.replace(
                ".nc",
                f"_{variable}_timeseries_{original_time_limits[0]:%Y%m%dT%H%M}_{original_time_limits[-1]:%Y%m%dT%H%M}_{original_target_ranges:.1f}.png",
            )
        ...
        data["range"] = data["range"] / 1e3            
        for idx, range_ in enumerate(range_list):
            data[variable].sel(range=range_).plot(x='time', ax=ax, color=colors[idx], label=f"{range_:.3f} km")
