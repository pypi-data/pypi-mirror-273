...
def filter_wildcard(
    directory: Path, wildcard: str = r"\.\d+$", recursive_search=True
) -> list[Path]:
    ...
    if recursive_search:
        return [
            file for file in directory.rglob("*") if re.search(wildcard, file.name)
        ]
    else:
        return [
            file for file in directory.glob("*") if re.search(wildcard, file.name)
        ]    
