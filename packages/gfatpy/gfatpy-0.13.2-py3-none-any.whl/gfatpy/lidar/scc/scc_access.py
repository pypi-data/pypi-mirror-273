...
class SCC:
    ...
    def __init__(self, auth, output_dir: str, base_url: str):
        self.auth = auth
    ...
    def login(self, credentials: list[str, str]):
        """Login to SCC."""
    ...
    def monitor_processing(
        ...
        if measurement.is_being_processed:
            while (run_count <= run_max):
                ...
                measurement, status = self.get_measurement(measurement_id)
                if not measurement.is_being_processed:
    ...
    def get_ancillary(self, file_path, file_type):
        ...
        if object_list:
            ancillary_file = AncillaryFile(
                self.api_base_url, object_list[0]
...
class AncillaryFile(ApiObject):
    ...
    def __str__(self):
        return "%s: %s, %s" % (self.id, self.filename, self.status)
