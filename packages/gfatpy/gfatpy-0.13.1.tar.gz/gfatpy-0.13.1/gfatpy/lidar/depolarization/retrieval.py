...
def add_depolarization_products_of_channel(
    ...
    Args:
        ...
        ghk (Dict[str, Union[float, None]]): `channel_depo` GHK values from `gfatpy.lidar.depolarization.io.ghk_output_reader`.
        calibration (xr.Dataset): Depolarization calibration dataset from `gfatpy.lidar.depolarization.io.eta_star_reader`.
...
def add_depolarization_products(
    dataset: xr.Dataset, ghk_dir: Path, depo_calib_dir: Path
) -> xr.Dataset:

    # Check directories exist
    if not ghk_dir.exists() and not ghk_dir.exists():
        raise FileNotFoundError(f"{ghk_dir} does not exist.")
    ...
    for telescope_ in polarized_channels.keys():
        for wavelength_ in polarized_channels[telescope_].keys():
            ...
            # Get GHK file
            ghk_file = find_nearest_filepath(
                ghk_dir,
                f"*{lidar_nick}*{wavelength_}{telescope_[0]}*.dat",
                6,
                target_date,
                and_previous=True,
            )
