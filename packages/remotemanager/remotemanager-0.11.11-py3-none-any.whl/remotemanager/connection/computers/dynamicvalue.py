...
str_to_nonstring_regex = r"only concatenate str.*to str"
nonstring_to_str_regex = r"unsupported operand.*'str'"
    ...
    def value(self):
        ...
        if self.op == "add":
            try:
                ...
                # use regex to check that the error is _actually_ caused
                # by adding a string and non string object
                str_to_non = re.search(str_to_nonstring_regex, str(ex)) is not None
                non_to_str = re.search(nonstring_to_str_regex, str(ex)) is not None
                # if it is, cast to string and return
                if str_to_non or non_to_str:
                    return str(a) + str(b)
                else:  # if not, raise
                    raise
