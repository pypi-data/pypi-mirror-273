"""
    ...
    In that sense, Pathfinder's model hierarhcy works as follows::
...
ROUTING_FAST = ''
ROUTING_ADVANCED = ''
ROUTING_ADVANCED_MULTI = ''
ROUTING_PYLON_SPOTTING = ''
ROUTING_PYLON_SPOTTING_MULTI = ''
...
class Scenario(base.Model):
    ...
    def _parse_from_dict(self, ddict):
        ...
        for k, v in ddict.items():
            if k in properties:
                ...
                # has its own funciton.
                continue
            if k == 'config':
                v = ScenarioConfig.NewFromDict(v, api=self._api)
            if k == 'project':
    ...
    @classmethod
    def NewScenario(cls, project, name='New', api=None):
        """
        ...
        """
        return api.post_scenario(name, project.id)
    ...
    def get_cost_models(self, **kwargs):
        ...
        for cm_name in self.enabled_cost_models:
            ...
            for param in cm.get_global_parameters():
                param.set_value(cmodel_data.get(param.name, None))
            cmodels.append(cm)
        return cmodels
    ...
    def set_routing_pathfinder_explore(self, angle_weight=None,
        ...
        """
        cfg = self.config
        self.config.routing_model = 'GeneticAlgorithm'
    ...
    def set_routing_gilytics_fast(self):
        ...
        """
        cfg = self.config
        cfg.routing_model = 'GilyticsFastRouting'
    ...
    def set_routing_pylon_spotting(self, angle_cost_f=None, angle_weight=None,
        ...
        """
            Set the routing algorithm to the GilyticsFast Pylon spotting alongside its
        ...
        cfg = self.config
        cfg.routing_model = 'LionPylonSpottingOptimal'
    ...
    def get_changes_from_server(self):
        """
            ...
            :raises PFAPIException: If any error ocurred while connecting to the server
        """
    ...
    def push_changes_to_server(self):
        """
            ...
            :return: No return
            :rtype: :class:`~libpfapi.task.Task`
            :raises PFAPIException: If any problem ocurred while pushing changes
        """
