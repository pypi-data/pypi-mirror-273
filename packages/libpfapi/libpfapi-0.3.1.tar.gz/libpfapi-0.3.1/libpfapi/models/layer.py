...
appropiate functions and finally pushing the changes to the server.  All
...
changes to the api server::
    ...
    if lay.has_errors:  # Check if any errors ocurred and act accordingly
        print(lay.error_msg)
        return
"""
...
class Layer(base.Model):
        ...
        for k, v in ddict.items():
            if k == 'category':
                ...
                if isinstance(v, dict):
                    v = Category.NewFromDict(v, api=self._api)
                else:
                    v = Category.NewFromDict({"id": v}, api=self._api)
            setattr(self, k, v)
    ...
    @classmethod
    def NewFromBaseLayer(cls, baselayer, category, buffer_rings=[], name=None, api=None):
        """
            ...
            :param api: Api to assign and perform appropiate REST calls
    ...
    def __get_geojson_data(self, attribute, path=None):
        ...
        if path:
            raw = self._api.get_json_url(url, raw=True)
            with open(path, "wb") as f:
                f.write(raw)
            return path
        else:
            return self._api.get_json_url(url)
    ...
    def __repr__(self):
        return "Layer ({}): {}. {}. MRING:{}".format(self.id,
                                                     self.name, self.ltype, self.is_multiring)
