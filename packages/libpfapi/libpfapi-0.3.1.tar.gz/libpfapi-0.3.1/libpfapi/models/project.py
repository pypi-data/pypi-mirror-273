"""
    ...
    It's useful as an starting point for any kind of interaction with the
    pathfinder, although you can acces directly by IDs to any required model.
...
class Project(base.Model):
    ...
    def _parse_from_dict(self, ddict):
        ...
        for k, v in ddict.items():
            if k == 'scenarios':
                v = [
                        Scenario.NewFromDict(sdata, api=self._api)
                        for sdata in ddict['scenarios']
                        ]
            if k == 'layers':
                v = [
                        Layer.NewFromDict(sdata, api=self._api)
                        for sdata in ddict['layers']
                        ]
            if k == 'categories':
                v = [
                        Category.NewFromDict(sdata, api=self._api)
    ...
    def add_category(self, name):
        ...
        """
        ncat = Category.NewCategory(name, self.id, api=self._api)
    ...
    def resolution(self):
        """
            Resuolution configured for this project
        """
