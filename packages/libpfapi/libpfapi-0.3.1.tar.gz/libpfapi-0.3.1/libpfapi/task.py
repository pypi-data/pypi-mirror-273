...
class Task():
    ...
    def wait_till_finished(self, cancel_callback=None):
        """
            ...
            param: cancel_callback: function to be called to check cancelation
        ...
        while True:
            if cancel_callback and cancel_callback():
                ...
                break
            self.get_status()
            if self.status not in [P_OK, P_FAIL, P_ERROR]:
    ...
    @property
    def success(self):
        return self.status == P_OK

    @property
    def failure(self):
        return self.status in [P_FAIL, P_ERROR]
