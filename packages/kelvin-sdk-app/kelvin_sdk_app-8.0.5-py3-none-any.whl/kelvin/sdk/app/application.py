...
from kelvin.sdk.datatype import AssetParameter, AssetParameters, Message
...
from kelvin.sdk.datatype.krn import KRN, KRNAppVersion, KRNAssetMetric, KRNWorkload
...
class BaseApplication(ApplicationInterface, metaclass=BaseApplicationMeta):
    ...
    def emit(self, data: Union[Message, Sequence[Message]]) -> None:
        ...
        for message in data:
            if isinstance(message, (Recommendation, ControlChange, ParametersMsg)):
                super().emit(message)
                continue
    ...
    def make_recommendation(
        ...
        auto_accepted : bool
            Set the recommendation as auto accepted. Default is False.
        emit : bool, optional
            Emit the message
        ...
        message = Recommendation(
            ...
            payload=RecommendationModel(
                ...
                state="auto_accepted" if auto_accepted is True else None,
            ),
        )  # type: ignore
    ...
    def make_parameters(
        ...
        """Creates a parameters change message.

        Parameters
        ----------
        resource (KRNAppVersion):
        ...
        Returns:
            ParametersMsg
        """
        message = AssetParameters(resource=resource, parameters=parameters).to_message()  # type: ignore
        if emit:
            self.emit(message)
