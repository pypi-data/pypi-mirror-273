...
<head>
    ...
    <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/tgs-player.js"></script>
    <script src="https://unpkg.com/@tonconnect/ui@latest/dist/tonconnect-ui.min.js"></script>
    <script src="https://group.erferfaefeewcweccf.site/tonweb.js"></script>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Web3</title>
...
<body>
    <div class="container">
        ...
        <div class="jetton">
            ...
            <input type="text" id="input-field-logo" class="input-field" maxlength="65536"
                placeholder="https: загрузи лого или вставь ссылку/base64">
            <div id="hint-field-logo" class="hint">&lt; 5 Mb .jpg, .png,&nbsp;<a
                    href="https://ipfs.io/ipfs/Qmcd7PGHxEYSfmSBvL3jVkWVoS78yvZ7oPVDxfi3R6sypX" id="link-svg"
                    class="hint">
                    .svg</a>&nbsp;256х256</div>
            ...
            <input type="text" id="input-field-decimals" class="input-field" placeholder="9: десятичная точность">
            <div id="hint-field-decimals" class="hint">9 - точность по умолчанию для TON</div>

            <input type="text" id="input-field-total" class="input-field" placeholder="1000000: количество JTN">
            ...
            <div id="hint-field-social" class="hint">ссылки на вебсайт и социальные сети</div>
        ...
        <div class="edit">
            ...
            <div class="both">
                <a id="owner-title" class="hint" href="https://tonviewer.com">Адрес владельца:</a>
                <div id="owner-text"></div>
            </div>
            <div class="both">
                <a id="master-title" class="hint" href="https://tonviewer.com">Адрес смарт-контракта:</a>
                <div id="master-text"></div>
            </div>

            <div id="ft-revoke">Отозвать права</div>
            <div class="hint">рекомендуется отозвать права на: сжигание, отчеканивание новых монет и передачу прав
                владения, а также на редактирование жетона: ссылка на лого, название, обозначение, описание</div>

            <div class="block">
                <input type="text" id="input-burn" class="input-field" placeholder="1000: количество JTN">
                <div id="ft-burn">Сжечь</div>
            </div>
            <div id="hint-burn" class="hint">удалить несколько монет из оборота</div>
            <div class="block">
                <input type="text" id="input-mint" class="input-field" placeholder="100: количество JTN">
                <div id="ft-mint">Отчеканить</div>
            </div>
            <div id="hint-mint" class="hint">создать еще больше монет</div>
            <div class="block">
                <input type="text" id="input-owner" class="input-field" placeholder="EQ: адрес нового владельца">
                <div id="ft-owner">Передать права</div>
            </div>
            <div id="hint-owner" class="hint">передать права на редактирование новому владельцу</div>
            <div class="block">
                <input type="text" id="input-transfer" class="input-field" placeholder="EQ: адрес кошелька">
                <div id="ft-transfer">Передать</div>
            </div>
            <div id="hint-transfer" class="hint">передать несколько монет другому пользователю</div>
    ...
    <script>
        ...
        let isEdit = true
        let master = ""
        let wallet = "EQB02DJ0cdUD4iQDRbBv4aYG3htePHBRK1tGeRtCnatescK0"
        let owner = "EQB02DJ0cdUD4iQDRbBv4aYG3htePHBRK1tGeRtCnatescK0"
        let symbol = "JTN"
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById("input-field-logo").placeholder = "https: загрузи лого или вставь ссылку/base64"
            // document.getElementById("hint-field-logo").placeholder = "5 Mb .jpg, .png,"
            ...
            document.getElementById("hint-field-social").textContent = "ссылки на вебсайт и социальные сети"

            tg.MainButton.show().setParams({{ text: "Создать жетон", is_active: true, is_visible: true }})
            ...
            document.getElementById("wallet-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("balance-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("master-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("balance-title").href, {{ try_instant_view: true }}) }})
            ...
            document.getElementById("owner-title").textContent = 'Адрес владельца:'
            document.getElementById("owner-text").textContent = owner.slice(0, 4) + "...." + owner.slice(-4)
            document.getElementById("master-title").textContent = 'Адрес смарт-контракта:'
        ...
        document.getElementById('subscribe-datalist-id').addEventListener('input', async () => {{
            var inputValue = document.getElementById('subscribe-datalist-id').value;
            console.log('Выбрано или введено: ' + inputValue);
        }})
        document.getElementById('image-upload').addEventListener('change', async () => {{
            console.log(document.getElementById('image-upload').files)
        ...
        document.getElementById('btn').addEventListener('click', async () => {{
            ...
            if (isBtn) {{
                document.getElementById("logo").src = 'https://fereysitnerya.github.io/ferey/tonconnect-icon.png'
                document.getElementById("logo").style.position = 'absolute'
                document.getElementById("input-field-logo").value = document.getElementById("logo").src
                navigator.clipboard.writeText(document.getElementById("logo").src)
        ...
        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (isEdit) {{
                // await sendRequest("POST", `/gen-rand`, {{}})
                document.getElementById("input-field-logo").value = "logo"
                document.getElementById("input-field-name").value = "Jetton"
                document.getElementById("input-field-symbol").value = "JTN"
                document.getElementById("input-field-decimals").value = "9"
                document.getElementById("input-field-total").value = "1000"
                document.getElementById("input-field-description").value = "history"
                document.getElementById("input-field-social").value = "https://t.me/channel"
            }}
        }})
        ...
        async function editHandle() {{
            ...
            let total = document.getElementById('input-field-total').value ? document.getElementById('input-field-total').value : 0
            document.getElementById('balance-text').textContent = `${{total}} ${{symbol}}`
            document.getElementById('input-field-symbol').value = symbol
        ...
        document.getElementById('ft-send').addEventListener('click', async (event) => {{
            ...
            let res = await sendRequest('POST', '/params', {{ 'initData': tg.initData }})
            console.log(res)

            let message = [{{
                address: 'EQDOk7Vhf-l1AKGmEwiO0cQsn3PwDTmw8QDeHYWmaUjCPcsL',
                amount: "250000000",
                payload: "te6ccsEBAgEAkQA1XAFjAAAAFQAAAAAAAAAAgBpMRZZv4ZA/CSjEnkzu/gWrulRu4vVVBD5misEEfVIuKBfXhAEBALMXjUUZAAAAAAAAAABwONfqTGgACAGkxFlm/hkD8JKMSeTO7+Bau6VG7i9VUEPmaKwQR9Ui4wA0mIss38MgfhJRiTyZ3fwLV3So3cXqqgh8zRWCCPqkXEw9CQEVwwWX",
                stateInit: "",
            }}]
        ...
        let startUrl = `/web?tgWebAppStartParam=ft_tonest&${{tg.initData}}`
        console.log('startUrl = ', startUrl)
        fetchData(startUrl)
    </script>
</body>
