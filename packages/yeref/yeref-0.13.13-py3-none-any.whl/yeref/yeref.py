...
<body>
    <div class="container">
        ...
        <div class="edit">
            <div class="both">
                <a id="supply-title" class="hint" href="https://tonviewer.com">Общий баланс:</a>
                <div id="supply-text">0</div>
            </div>
            <div class="both">
                <a id="balance-title" class="hint" href="https://tonviewer.com">Баланс кошелька:</a>
                <div id="balance-text">0</div>
            </div>
            <div class="both">
            ...
            <div class="both">
                <a id="minter-title" class="hint" href="https://tonviewer.com">Адрес смарт-контракта:</a>
                <div id="minter-text">....</div>
            </div>

            <div id="ft-revoke">Отозвать права</div>
            <div class="hint">рекомендуется отозвать права на: отчеканивание новых монет и передачу прав
    ...
    <script>
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                ...
                let res = await sendRequest('POST', '/download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                console.log(res)
            }}
        }})
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById("minter-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("balance-title").href, {{ try_instant_view: true }}) }})
            ...
            document.getElementById("minter-title").textContent = 'Адрес смарт-контракта:'
            document.getElementById("minter-text").textContent = minter.slice(0, 4) + "...." + minter.slice(-4)
        }})
        // ----------------------------------------------------
        document.getElementById('subscribe-datalist-id').addEventListener('change', async () => {{
            ...
            if (tonConnectUI.connected) {{
                ...
                let res = await sendRequest('POST', '/download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                console.log(res)
                console.log(res[dlAddress]['name'])
            }}
        }})
        ...
        document.getElementById('minter-text').addEventListener('click', async () => {{
            navigator.clipboard.writeText(minter)
            document.querySelector('.copy-media').classList.add('window--open')
            await sleep(1000).then(() => {{ document.querySelector('.copy-media').classList.remove('window--open') }})
        }})
        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (isEdit) {{                
                ...
                owner = params['owner']
                minter = params['address']

                document.getElementById("wallet-text").textContent = wallet.slice(0, 4) + "...." + wallet.slice(-4)
                document.getElementById("owner-text").textContent = owner.slice(0, 4) + "...." + owner.slice(-4)
                document.getElementById("minter-text").textContent = owner.slice(0, 4) + "...." + minter.slice(-4)
            }}
        }})
