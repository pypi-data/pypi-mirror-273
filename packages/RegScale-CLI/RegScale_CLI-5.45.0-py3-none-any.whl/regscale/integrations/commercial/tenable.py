...
def fetch_assets(ssp_id: int) -> list[TenableIOAsset]:
    ...
    logger.info("Found %i existing assets in RegScale.", len(existing_assets))
    ...
    for asset in client.exports.assets(updated_at=tenable_last_updated):  # Client Exports API
        try:
    ...
    TenableIOAsset.sync_to_regscale(assets=assets, ssp_id=ssp_id, existing_assets=existing_assets)
    return assets
