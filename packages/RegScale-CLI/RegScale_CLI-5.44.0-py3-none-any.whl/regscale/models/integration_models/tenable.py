...
class TenableIOAsset(BaseModel):
    ...
    fqdn: Optional[List[str]] = None
    mac_address: Optional[List[str]] = None
    netbios_name: Optional[List[str]] = None
    ...
    def create_asset_from_tenable(asset: "TenableIOAsset", ssp_id: int, app: Application) -> Asset:
        ...
        return Asset(
            ...
            operatingSystemVersion=(asset.operating_system.pop() if asset.operating_system else None),
    ...
    def prepare_assets_for_sync(
        ...
        insert_assets = []
        update_assets = []
        existing_assets = Asset.find_assets_by_parent(app=app, parent_id=ssp_id, parent_module="securityplans")
    ...
    def update_off_network_assets(
        ...
        :param List[Asset] existing_assets: A list of existing Asset objects to compare against.
        :param List[TenableIOAsset] assets: A list of TenableIOAsset objects to update.
        :param List[Asset] update_assets: A list of Asset objects to update.
        :return: A list of updated Asset objects.
        :rtype: List[Asset]
        """
        for existing in existing_assets:
            if existing.tenableId and existing.tenableId not in {asset.id for asset in assets}:
                existing.status = "Off-Network"
                updated_asset = TenableIOAsset.update_existing_asset(existing, update_assets)
                if updated_asset is not None:
                    update_assets.append(updated_asset)
        return update_assets

    @staticmethod
    def sync_assets_to_regscale(app: Application, insert_assets: List[Asset], update_assets: List[Asset]) -> None:
        ...
        if insert_assets:
            ...
            Asset.bulk_insert(assets=insert_assets, batch_size=100, batch=True)
            logger.info("Done!")
        else:
            logger.info("No new assets need to created in RegScale.")

        if update_assets:
            logger.info("Updating %i existing asset(s) in RegScale...", len(update_assets))
            Asset.bulk_update(assets=update_assets, batch_size=150, batch=True)
            logger.info("Done!")
        else:
            logger.info("No assets need to be updated in RegScale.")

    @staticmethod
    def sync_to_regscale(app: Application, assets: List["TenableIOAsset"], ssp_id: int) -> None:
        ...
        """
        existing_assets = Asset.find_assets_by_parent(app=app, parent_id=ssp_id, parent_module="securityplans")
        insert_assets, update_assets = TenableIOAsset.prepare_assets_for_sync(assets, ssp_id, app)
        update_assets = TenableIOAsset.update_off_network_assets(existing_assets, assets, update_assets)
