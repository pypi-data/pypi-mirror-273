...
class NessusReport(BaseModel):
    ...
    def process_nessus_files(regscale_ssp_id: int, folder_path: Path) -> None:
        ...
        def lookup_asset(assets: List[Asset], issue: Issue) -> dict:
            ...
            if matched_asset:
                try:
                    return {matched_asset[0].id: vulnerabilities_dict[matched_asset[0].id]}
                except KeyError:
    ...
    def create_regscale_vuln(**kwargs: dict) -> Tuple[List["NessusReport"], List[ScanHistory]]:
        ...
        if scans := [
            ...
            regscale_vuln = Vulnerability(
                ...
                mitigated=None,
                operatingSystem=(report.asset.operating_system[0] if report.asset.operating_system else None),
