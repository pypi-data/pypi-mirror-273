...
class Deli(MorphoPlugin):
    ...
    Segmentation channel: int, default 0
    ...
    def __init__(self):  # PLUGIN DEFINITION
        ...
        self.set_description("This plugin removes all objects that are under a certain volume (in voxel) from the "
                             "segmented images.\n \n"
                              "Parameters : \n \n "
                             "- minimum volume : (float, default 50) : the minimum volume to be considered as an object\n"
                             "- segmentation channel : (int, default 0) : The desired channel to perform the plugin\n"
                             "- time points : List of time range \n"
                             "    Current : Current time point in MorphoNet \n"
                             "    All times : will delete all objects at all time points \n "
                             " - association : Determines how the deleted objects will be modified \n"
                             "    Background :  background value will replace the object label inside the image \n"
                             "    Closest Object : object is fused with the object/background sharing the most contact surface")
    ...
    def remove_under_at(self, t, channel, dataset, associated, voxel_size):
        ...
        cell_volume = dataset.get_regionprop_at("area", t, channel, ordered=True)  # Find the volume of the cell , using scikit properties
        cell_updated = []
        nb_objects_delete=0
        cell_to_remove=[]
        cell_to_remove_id = []
