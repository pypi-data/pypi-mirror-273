...
def process_genome_data(TAXON):
    ...
    if answer.lower() == "y":
        ...
        subprocess.run(["./datasets", "download", "genome", "taxon", TAXON, "--reference", "--include", "protein,gff3,gbff", "--filename", f"../{TAXON}.zip"])
    
    else:
        print("<< Skipping genome data download.")
        print("<< Unzipping files...")
        subprocess.run(["unzip", f"../{TAXON}.zip", "-d", f"../{TAXON}"])
    ...
    if answer.lower() == "y":
        ...
        print("<< Skipping protein clustering.")
...
# find_gene_cluster("Facklamia", "../target.tsv")