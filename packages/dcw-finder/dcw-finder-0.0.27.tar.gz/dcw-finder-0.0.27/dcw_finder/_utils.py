...
def sum_assembly_report(project_info):
    ...
    raw_df = raw_df[~raw_df['Accession'].isin(excluded_list + missclassified_list)]

    print(excluded_list)
    print(missclassified_list)
    ...
    for accession in excluded_list + missclassified_list:
        ...
        if os.path.isdir(accession_dir):
            print(f"Deleting directory: {accession_dir}")
            os.rmdir(accession_dir)
    ...
    print("  *  missclassified species:\t{}".format(len(missclassified_list)))
    print("------------------------------------------")
...
def _filter_and_save_completeness(project_info, raw_df, completeness_filter, contamination_filter):
    ...
    excluded_list = low_completeness['Accession'].tolist() + high_contamination['Accession'].tolist()
...
def _process_and_save_summary(project_info, raw_df):
    ...
    missclassified_list = df[df['Genus'].str.startswith('[')].tolist()
