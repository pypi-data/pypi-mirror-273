...
# TODO: support region query
# TODO: support array input?
# TODO: find parents by name
# TODO: speedup init
...
class NUTSfinder:
    ...
    **Note**: Points-in-polyon tests via `shapely` may suffer from floating-point precision issues for points on the boundary of regions.
    ...
    def _create_region(self, feature): return NUTSregion(feature, self.buffer)
    def _create_regions(self, features): return [NUTSregion(feature, self.buffer) for feature in features]
    ...
    def _filter_regions(self, fc):
        ...
        #     max_workers = 100
        #     filtered_split = np.array_split(np.array(filtered), max_workers, )
        #     print("max_workers", max_workers)
            # regions = [region for regions in list(ProcessPoolExecutor(max_workers=max_workers).map(self._create_regions, filtered_split)) for region in regions]
            # regions = [region for regions in list(ThreadPoolExecutor(max_workers=max_workers).map(self._create_regions, filtered_split)) for region in regions]
            # print("regions:", regions)
            # regions = ThreadPoolExecutor(max_workers=100).map(self._create_regions, filtered_split)
            # with multiprocessing.Pool(processes=num_processes) as pool:
            #     regions = pool.map(self._create_region, filtered)
        # else:
