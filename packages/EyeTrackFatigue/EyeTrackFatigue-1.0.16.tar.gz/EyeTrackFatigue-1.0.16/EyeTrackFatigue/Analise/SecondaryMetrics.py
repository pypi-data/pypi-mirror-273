# класс для хранения количественных характеристик, связанных с фиксациями/саккадами
class SecondaryMetrics:

    def __init__(self):
        self.fix_distance = self.sacc_distance = 0
        self.fix_time = self.sacc_time = 0.00000001
        self.min_f_speed = self.min_s_speed = self.min_s_length = self.min_s_time = self.min_sf_speed = self.min_lf_speed = 1000000
        self.max_f_speed = self.max_s_speed = self.max_s_length = self.max_s_time = self.max_sf_speed = self.max_lf_speed = 0
        ...
        self.fix_l_150_dist = self.fix_g_150_dist = 0
