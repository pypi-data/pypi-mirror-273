...
CFLIB_HTTPX_ACTIVITY_TIMEOUT = os.getenv("CFLIB_HTTPX_ACTIVITY_TIMEOUT") or 30  # Connection timeout in seconds
            ...
            async with AsyncClient() as client:
                while attempt < max_attempts:
                    try:
                        ...
                        self.logger.error(f"{self.correlation_id} Attempt {attempt} failed with error: {str(e)}.")
                        if attempt <= max_attempts:
                            await asyncio.sleep(2 ** (attempt - 1))  # Exponential back-off (1s, 2s, 4s...)
                            continue
                        raise
            ...
            async with AsyncClient() as client:
                while attempt < max_attempts:
                    try:
                        ...
                        self.logger.error(f"{self.correlation_id} Attempt {attempt} failed with error: {str(e)}.")
                        if attempt == max_attempts:
                            raise ConnectionError(f"{self.correlation_id} Unable to update activity: {response.status_code} {response.text}") from None
                        await asyncio.sleep(2 ** (attempt - 1))  # Exponential back-off (1s, 2s, 4s...)
