...
from ..common.utils import raise_for_status, SHOW_DEBUG, Singleton
...
class TraceDataSingleton(Singleton):
    ...
    def upload_log_data(self, lastmile_api_token: str) -> None:
        ...
        for filepath in self.logger_filepaths:
            ...
            raise_for_status(
                upload_response,
                f"Error creating upload object with S3 url {s3_url}",
            )
...
def _upload_to_s3(filepath: str) -> Optional[str]:
    ...
    See documentation on how to upload to S3:
    https://boto3.amazonaws.com/v1/documentation/api/latest/guide/s3-uploading-files.html

    @return url to S3 uploaded file if successful, None if not
    """
    filename: str = _sanitize_filename(os.path.basename(filepath))
    random_int: int = random.randint(0, 10001)
    bucket: str = "lastmileai.traces.public"
    upload_key: str = (
        f"traces/{_get_date_time_string()}/{random_int}/{filename}"
    )
    ...
    try:
        ...
        # TODO: Add logging statement here and pass into logger
        # logging.error(e)
        return None
