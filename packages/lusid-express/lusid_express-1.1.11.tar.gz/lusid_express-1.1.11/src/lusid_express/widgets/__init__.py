...
class TransactionForm:
    ...
    def create_transaction_form(self, txn: lu.TransactionRequest):
        ...
        def get_prop_widget(prop):
            value = None
            if "label_value" in prop.value.attribute_map:
                value = prop.value.label_value
            elif "metric_value" in prop.value.attribute_map:
                value = prop.value.metric_value.value
            return widgets.Text(
                value=value, layout=field_layout, style={"description_width": "initial"}
            )
