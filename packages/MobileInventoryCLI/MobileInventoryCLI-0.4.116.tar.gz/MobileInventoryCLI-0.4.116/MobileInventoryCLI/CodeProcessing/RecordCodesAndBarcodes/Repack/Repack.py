...
class RepackExec(TasksMode):
                ...
                with Session(self.engine) as session:
                    ...
                    for num,i in enumerate(results):
                        ...
                        for num2,item in enumerate(items):
                            m=f"{Fore.grey_70}{num}{Style.reset}/{Fore.cyan}{i.repackCaseBarcode}|{Style.reset}{Fore.orange_red_1}'{rpk.repackDate.month}.{rpk.repackDate.day}.{str(rpk.repackDate.year)[-2:]}{NTE}'{Style.reset}/{num2} -> {item}"
                            print(m)
            ...
            while True:
                try:
                    ...
                    for num,r in enumerate(items):
                        print(f"{Fore.light_red}RepackItem{Style.reset} {Fore.light_yellow}{num}{Fore.orange_red_1}/{ct2-1}{Style.reset} -> {Fore.light_magenta}{r.EntryBarcode}{Style.reset}|{r.EntryCode}|{r.EntryName}!")
                    def mkInt(text,data):
                        try:
                            if text == '':
                                return 0
    ...
    def __init__(self,engine,parent):
        ...
        self.addCmd(cmd=['delete case','del rpk','drpk',],desc=f"delete repacklist by case barcode{Fore.light_red}{Style.reset}",EXEC=self.DeleteRepack)
        self.addCmd(cmd=['remove from case','rf rpk','rfc',],desc=f"remove an item from repack by code|barcode{Fore.light_red}{Style.reset}",EXEC=self.remove_from_case)
        ...
        while True:
            doWhat=Prompt.__init2__(None,func=mkCmd,ptext=f"[{Fore.light_green}Repack{Fore.light_yellow}] Do What",helpText=self.help(self.helpText),data=self)
            if doWhat in [None,]:
                return
            for k in self.cmds:
                c=self.cmds.get(k)
