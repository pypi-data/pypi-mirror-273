...
class ServerProxy:
    ...
    def comm(
        ...
        logger.info(f"The server is RESTServer object")
    ...
    def async_call(
        ...
        '''
        This is really a call in a separate thread.
        ...
        Returns:
            `List[Dict]`: A list that contains the response from the Tomcat server engine.
        '''
        insight_id = insight_id if insight_id is not None else self.server.cur_insight
        epoc = self.get_next_epoc()
        threading.Thread(
            target=self.comm,
            kwargs={
    ...
    def get_partial_responses(self, job_id:str) -> Generator:
        '''
        Use the client connection to make posts to the partial endpoint and create a generator.
        ...
        '''  
        yield from self.server.get_partial_responses(job_id)
