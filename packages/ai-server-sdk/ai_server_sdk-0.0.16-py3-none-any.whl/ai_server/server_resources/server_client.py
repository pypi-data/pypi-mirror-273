...
class ServerClient():
    ...
    def __init__(
        ...
        """
        self.access_key:str = access_key                             # set the access key as an instance attribute
        self.secret_key:str = secret_key                             # set the secret key as an instance attribute
        self.main_url:str = base                                     # set the base url as an instance attribute
        
        if self.main_url.endswith('\/'):
            self.main_url = self.main_url[:-1]

        # TODO provide definitons for all of these attributes
        self.auth_headers:Dict = {}                                  # used to keep track of the authorization header after the user has authenticated
        self.login()
        self.connected:bool = True                                   # boolean flag to make sure the users have authenticated and the cookies are set
        self.open_insights:Set = set()                               # keep track of the the insights created from this connection
        self.cur_insight:str = self.make_new_insight()
        
        self.monitors = {}                                          # map to keep track of requests and responses outside of this class or in separate threads
        
        ServerClient.da_server = self                               # set the instance of this class as the class attribute da_server
    ...
    def login(
        ...
        headers = {
            'Authorization': f"Basic {combined_enc.decode('ascii')}"
        }
        ...
        if 'errorMessage' in json_response:
            if json_response['errorMessage'] == "null principal":
                raise AuthenticationError(url + f" USERID = INVALID could not login using user access key '{self.access_key}' with invalid secret key")
    ...
    def get_partial_responses(self, job_id:str) -> Generator:
        ...
        started_streaming = False       # keep track of when the streams start coming since it could be delayed   
        while True:
    ...
    def drop_insights(
        self, 
        insight_ids: Union[str, List[str]]
    ) -> None:
        '''
    ...
    def import_data_product(
        ...
        response = requests.get(
            dataProductUrl, cookies=self.cookies
        ).json()
