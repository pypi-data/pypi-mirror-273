""" Extracts the gate current parameters for PSP model versus gate potential
...
* Direct on the I_G(V_GS) characteristics at fixed V_DS AND fixed V_BS
...
from DMT.core import specifiers, sub_specifiers, McParameterCollection, DutView, MCard, constants
...
class XIg(XStep):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_function = self.extract_ig
        self.model_function_name = "ige"
        ...
        if to_optimize is None:
            to_optimize = (["IGINV", "GC2", "GC3", "IGOV", "GCO"],)
        ...
        self.col_ig = specifiers.CURRENT + "G"

    @plot()
    @print_to_documentation()
    def main_plot(self):
        main_plot = super(XIg, self).main_plot(
            r"$ " + self.col_ig.to_tex() + r" \left( " + self.col_vgs.to_tex() + r" \right) $",
            x_specifier=self.col_vgs,
            y_specifier=self.col_ig,
    ...
    def get_tex(self):
        ...
        return r"I_G = f( V_{GS}, V_{DS}, V_{BS})"

    def get_description(self):
        doc = Tex()
        doc.append(
            NoEscape(
                r"This extraction step fits the gate current model directly, without circuit simulation."
            )
        )
        return doc
    ...
    def ensure_input_correct_per_dataframe(self, dataframe, dut=None, key=None):
        ...
        dataframe.ensure_specifier_column(self.col_ig)
    ...
    def init_data_reference_per_dataframe(self, dataframe, t_meas, dut=None, key=None):
        ...
        for _i_vb, v_bs_f, data_filtered_bs in dataframe.iter_unique_col(
            ...
            for _i_vg, v_ds_f, data_filtered_ds in data_filtered_bs.iter_unique_col(
                ...
                ig = data_filtered_ds[self.col_ig].to_numpy().real
                try:
    ...
    def fit(self, data_model, paras_model: McParameterCollection, dut: DutView = None):
    ...
    def extract_ig(self, vgs, vds, vbs, vdb, t_dev, width=0.0, length=0.0, **kwargs):
        kwargs["l"] = length
        kwargs["w"] = width
