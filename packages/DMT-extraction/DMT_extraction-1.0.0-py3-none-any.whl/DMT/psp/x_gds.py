...
import numpy as np
from DMT.core import specifiers, sub_specifiers, MCard, DutLib, constants
...
class XGds(XStep):
    ...
    def __init__(
        ...
        relevant_duts=None,
        to_optimize=None,
        specifier_gds=specifiers.OUTPUT_CONDUCTANCE,
        **kwargs,
    ):
        if model is None:
            model = mcard.get_verilogae_model()

        self.model_function = self.extract_gds
        self.model_function_name = "gds"
        ...
        self.col_gds = specifiers.CONDUCTANCE + ["D", "S"]
        self.col_go = specifier_gds
    ...
    def main_plot(self):
        main_plot = super(XGds, self).main_plot(
            r"$ " + self.col_gds.to_tex() + r" \left( " + self.col_vds.to_tex() + r" \right) $",
            x_specifier=self.col_vds,
            y_specifier=self.col_gds,
    ...
    def get_tex(self):
        ...
        return r"g_ds = f( V_{GS}, V_{DS}, V_{BS})"

    def get_description(self):
        doc = Tex()
        doc.append(
            NoEscape(
                r"This extraction step fits the drain-source conductance model directly, without circuit simulation."
            )
        )
        return doc
    ...
    def ensure_input_correct_per_dataframe(self, dataframe, dut=None, key=None):
        ...
        dataframe.ensure_specifier_column(self.col_go)
    ...
    def init_data_reference_per_dataframe(self, dataframe, t_meas, dut=None, key=None):
        ...
        for _i_vb, v_bs_f, data_filtered_bs in dataframe.iter_unique_col(
            ...
            for _i_vg, v_gs_f, data_filtered_gs in data_filtered_bs.iter_unique_col(
                ...
                # g_ds = data_filtered_gs[self.col_go].to_numpy().real
                g_ds = np.gradient(id, v_ds)
                try:
    ...
    def extract_gds(self, vgs, vds, vbs, vdb, t_dev, width=0.0, length=0.0, **kwargs):
        kwargs["l"] = length
        kwargs["w"] = width
