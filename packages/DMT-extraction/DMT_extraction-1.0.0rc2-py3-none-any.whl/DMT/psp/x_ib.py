...
Impact Ionization Parameters (VGS positive):
* A1
* A2
* A3
* A4
GIDL Parameters (VGS negative):
* AGIDL
* BGIDL
* CGIDL
...
class XIb(XStep):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_function = self.extract_ib
        self.model_function_name = "ibe"
        ...
        if to_optimize is None:
            to_optimize = (["A1", "A2", "A3", "A4", "AGIDL", "BGIDL", "CGIDL"],)
        ...
        self.col_ib = specifiers.CURRENT + "B"

    @plot()
    @print_to_documentation()
    def main_plot(self):
        main_plot = super(XIb, self).main_plot(
            r"$ " + self.col_ib.to_tex() + r" \left( " + self.col_vgs.to_tex() + r" \right) $",
            x_specifier=self.col_vgs,
            y_specifier=self.col_ib,
    ...
    def get_tex(self):
        ...
        return r"I_B = f( V_{GS}, V_{DS}, V_{BS})"

    def get_description(self):
        doc = Tex()
        doc.append(
            NoEscape(
                r"This extraction step fits the bulk current model directly, without circuit simulation."
            )
        )
        return doc
    ...
    def ensure_input_correct_per_dataframe(self, dataframe, dut=None, key=None):
        ...
        dataframe.ensure_specifier_column(self.col_ib)
    ...
    def init_data_reference_per_dataframe(self, dataframe, t_meas, dut=None, key=None):
        ...
        for _i_vb, v_bs_f, data_filtered_bs in dataframe.iter_unique_col(
            ...
            for _i_vg, v_ds_f, data_filtered_ds in data_filtered_bs.iter_unique_col(
                ...
                i_b = data_filtered_ds[self.col_ib].to_numpy().real
    ...
    def extract_ib(self, vgs, vds, vbs, vdb, t_dev, width=0.0, length=0.0, **kwargs):
        kwargs["l"] = length
        kwargs["w"] = width
