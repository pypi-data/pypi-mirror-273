...
class XQPoa(XStep):
    ...
    def main_plot(self):
        ...
        main_plot = super(XQPoa, self).main_plot(
    ...
    def set_initial_guess_line(self, composition, line):
        ...
        # assume all quantity of line comes through quantity_perimeter
        val_p = np.mean(line["quantity"] / (2 * line["l_E0"] + 2 * line["b_E0"]))
        para_l = McParameter(
            "quantity_per_perimeter", value=val_p, minval=0, maxval=np.abs(val_p) * 2
    ...
    def plot_quantity_separated(self):
        """Plot quantity as a function of operating point meas vs fit for multiple geometries for all analyzed duts"""
        col_area, col_perimeter = self.get_cols_poa_full()
    ...
    def write_back_results(self):
        ...
        for dut in self.relevant_duts:
            ...
            quantity_area, quantity_perimeter = (
            ...
            for i, composition in enumerate(self.paras_to_optimize_per_line):
                ...
                length = 2 * (dut.length + dlE)
                width = 2 * (dut.width + dbE)
                area = (dut.length + dlE) * (dut.width + dbE)
                ...
                try:
                    quantity_perimeter[i] = perimeter * composition["quantity_per_perimeter"].value
                except KeyError:
                    quantity_perimeter[i] = 0
            ...
            try:
                ...
                f_quantity_perimeter = interpolate.interp1d(
                    voltages_data, quantity_perimeter, kind=3, fill_value="extrapolate"
                )
                ...
                f_quantity_perimeter = interpolate.interp1d(
                    voltages[i_sort],
                    np.asarray(quantity_perimeter)[i_sort],
                    kind=3,
                    fill_value="extrapolate",
                )
            ...
            quantity_perimeter = sign * f_quantity_perimeter(voltages_df) * self.nfinger

            # copy to local dataframe
            col_area, col_perimeter = self.get_cols_poa_full()
            df = copy.deepcopy(dut.data[key_dut])
            df[col_area] = quantity_area
            df[col_perimeter] = quantity_perimeter
