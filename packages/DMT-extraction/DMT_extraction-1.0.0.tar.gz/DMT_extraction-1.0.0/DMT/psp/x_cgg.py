...
* Direct on the C_GG(V_G) characteristics at fixed V_DS=0V ANDS fixed V_BS=0V
...
from DMT.extraction import XStep, plot, IYNormLogOneRange, print_to_documentation
...
class XCgg(XStep):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_function = self.extract_cgg
        self.model_function_info = {
            "independent_vars": ("vgs", "vds", "vbs", "t_dev"),
            "depends": (model.functions["QG"],),
        }

        if to_optimize is None:
            if mcard.get("swgeo").value == 0:
                # if gate_length == "long":
                to_optimize = ["vfb", "neff", "dphib", "np", "cox"]
                # elif gate_length == "short":
                #     to_optimize = ["cox"]
                # else:
    ...
    def main_plot(self):
        main_plot = super(XCgg, self).main_plot(
            r"$ " + r"C_{\mathrm{GG}}" + r" \left( " + self.col_vgs.to_tex() + r" \right) $",
            x_specifier=self.col_vgs,
            y_label=r"$C_{\mathrm{GG}} ( \si{\femto\farad} ) $",
            y_scale=1e15,
    ...
    def get_tex(self):
        ...
        return r"C_{GG} = f( V_{GS} )"

    def get_description(self):
        doc = Tex()
        doc.append(
            NoEscape(
                r"This extraction step fits the CGG model directly, without circuit simulation."
            )
        )
        return doc
    ...
    def init_data_reference_per_dataframe(
        ...
        for _i_vd, v_d, data_filtered_ds in dataframe.iter_unique_col(self.col_vds, decimals=3):
            for _i_vb, v_b, data_filtered_bs in data_filtered_ds.iter_unique_col(
                ...
                # ccg = cgs + cgd
                cgg = data_filtered_bs[self.col_cgg].to_numpy().real
                try:
                ...
                line = {
                    "x": vgs,
                    "y": cgg,
                    self.col_vds: v_d.real,
                    self.col_vbs: v_b.real,
    ...
    def extract_cgg(self, vgs, vds, vbs, t_dev, width=0.0, length=0.0, **kwargs):
        kwargs["l"] = length
        kwargs["w"] = width
