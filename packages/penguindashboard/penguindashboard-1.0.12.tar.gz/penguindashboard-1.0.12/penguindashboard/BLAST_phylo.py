...
def perform_blast_search(sequence):
    ...
    blast_result = NCBIWWW.qblast(program="blastp", database="nr", sequence=sequence, url_base='https://blast.ncbi.nlm.nih.gov/Blast.cgi', hitlist_size=100, format_type='XML', expect=0.001, short_query=False)
...
def write_sequences(df, outdir):
    records = [SeqRecord(Seq(row['seq']), id=row['description'], description='') for index, row in df.iterrows()]
    # make dir if it does not exist
    SeqIO.write(records, f'{outdir}/BLAST/all_sequences.fasta', 'fasta')
    SeqIO.write(records[0], f'{outdir}/BLAST/query.fasta', 'fasta')
    SeqIO.write(records[1:], f'{outdir}/BLAST/targets.fasta', 'fasta')

def perform_msa(outdir):
    input_path = f'{outdir}/BLAST/all_sequences.fasta'
    output_path = f'{outdir}/BLAST/msa.fasta'
...
def calculate_percentages(outdir):
    sequences = [str(record.seq) for record in SeqIO.parse(f'{outdir}/BLAST/msa.fasta', "fasta")]
...
def main(args):
    ...
    write_sequences(df, outdir=args.outdir)
    perform_msa(outdir=args.outdir)
    df = calculate_percentages(outdir=args.outdir)
