...
class EmbeddingsContainer:
    ...
    def get_metadata(self):
        ...
        if "azure_ad_token_provider" in self.arguments:
            arguments.pop("azure_ad_token_provider", None)
    ...
    @staticmethod
    def from_metadata(metadata: dict, credential: Optional[TokenCredential] = None) -> "EmbeddingsContainer":
        """Create an embeddings object from metadata."""
        schema_version = metadata.get("schema_version", "1")
        if schema_version == "1":
            embeddings = EmbeddingsContainer(metadata["kind"], credential, **metadata["arguments"])
            return embeddings
            ...
            embeddings = EmbeddingsContainer(kind, credential, **metadata)
            return embeddings
        else:
    ...
    def save_metadata(self, path, file_format_version="1"):
        ...
        with (Path(path) / "embeddings_metadata.yaml").open("w+") as f:
            ...
            # Restore AD token after saving metadata
            if saved_token:
    ...
    def upload_to_index(
        ...
        try:
            ...
            for doc_id, emb_doc in self._document_embeddings.items():
                ...
                document_to_upload, document_to_upload_size = index_store.create_upload_payload_from_embedded_document(
