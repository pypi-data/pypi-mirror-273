    ...
    if isinstance(connection, dict):
        ...
        if auth_type == "ApiKey":
            ...
            if connection["properties"]["category"] == "CognitiveSearch":
                ...
                return get_obo_or_default_azure_credential()
...
def _token_provider_for_aoai_connection(data_plane: bool = True) -> Callable[[], str]:
    ...
    if obo_endpoint:
        logger.info(f"Getting the token provide with AzureMLOnBehalfOfCredential.OBO_ENDPOINT: {obo_endpoint}.")
        ...
        logger.info(f"Getting the token provide with DefaultAzureCredential.Client id: {client_id} will be used.")
...
def get_obo_or_default_azure_credential():
    ...
    obo_endpoint = os.environ.get("OBO_ENDPOINT", None)
    if obo_endpoint:
        logger.info(f"Using User Identity for authentication. OBO_ENDPOINT: {obo_endpoint}.")
        credential = AzureMLOnBehalfOfCredential()
        os.environ["MSI_ENDPOINT"] = obo_endpoint
    else:
