...
def read_chunks_into_documents(files: Iterator[Path], chunk_format: str = "csv") -> Iterator[Document]:
    ...
    for i, chunk_file in enumerate(files):
        ...
        if file_name.lower().endswith(".jsonl") or chunk_format == "jsonl":
            with Path.open(chunk_file) as f:
                for line in f:
...
def load_embeddings_container(
    ...
    logger,
    embeddings_model_uri: Optional[str] = None,
        ...
        with track_activity(logger, "init.load_embeddings_container") as activity_logger:
            ...
            if snapshot_dir_name:
                embeddings_container = EmbeddingsContainer.load(
                    snapshot_dir_name,
                    Path(embeddings_container_path),
...
def main(args, logger, activity_logger):
    ...
    files = Path(args.chunks_source).rglob("**/*")
    ...
    if connection_id is not None:
        ...
        if args.embeddings_model and "open_ai" in args.embeddings_model:
        ...
        with track_activity(logger, "init.load_embeddings_container") as embeddings_container_activity_logger:
            ...
            try:
                ...
                with rslex_uri_volume_mount(
                    args.embeddings_container, f"{Path.cwd()}/embeddings_container", options=mnt_options
                ) as mount_context:
                    try:
                        create_embeddings(
                            chunks=chunks,
