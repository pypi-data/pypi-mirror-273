...
class GetMeteringPointValuesById(PathBase):
    def get_metering_point_values_by_id(
        ...
        request = usedPath +\
              f"?dateTimeFrom={subRequestFromTime.replace(tzinfo=None).isoformat()+'Z'}" +\
                f"&dateTimeTo={subRequestToTime.replace(tzinfo=None).isoformat()+'Z'}"
        ...
        deserialized_body = parse_raw_as(List[SensorDataExtDto], response.data)
        if as_dataframe:
            deserialized_body = convert_values_to_dataFrame(deserialized_body)
            
        api_response = ClimifyApiResponse[List[SensorDataExtDto]](
            response=response,
            status=response.status,
