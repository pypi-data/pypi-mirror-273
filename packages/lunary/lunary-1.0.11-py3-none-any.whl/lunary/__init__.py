...
def get_raw_template(slug):
    ...
    response = requests.get(f"{api_url}/v1/template_versions/latest?slug={slug}", 
                            headers=headers,  
                            verify=os.environ.get("SSL_DO_NOT_VERIFY", True))
...
def get_dataset(slug: str):
    ...
    try:
        ...
        response = requests.get(url, headers=headers, verify=os.environ.get("SSL_DO_NOT_VERIFY", True))
        if response.ok:
...
def evaluate(checklist, input, output, ideal_output=None, context=None, model=None, duration=None, tags=None):
    ...
    try:
        ...
        response = requests.post(url, headers=headers, json=data, verify=os.environ.get("SSL_DO_NOT_VERIFY", True))
