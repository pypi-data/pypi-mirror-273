...
from aheadworks_core.api.youtrack_manager import YoutrackManager
...
class ReleaseManager:
    ...
    def __init__(self, jira_api_config):
        ...
        self.youtrack_manager = YoutrackManager(jira_api_config.user, jira_api_config.token)
    ...
    def youtrack_release(self, project_id, composer_file, discord_bot_url, path_to_files, assign_to):
        ...
        if not project_id:
            print('project_id is empty, skip youtrack release.')
            return False

        files_to_upload = []
        file_names = os.listdir(path_to_files)
        for file_name in file_names:
           files_to_upload.append(('upload', (file_name, open(f"{path_to_files}/{file_name}", 'rb'))))

        links = self.youtrack_manager.find_issues_by_tags([f"{project_id}-{module_version}"])
        release_tasks = {}
        release_links = []
        for l in links:
            for t in l['tags']:
                ...
                if tag_name in [self.PD_TASK_LABEL, self.TEST_TASK_LABEL, self.RELEASE_PACK_TASK_LABEL]:
                    ...
                    release_links.append(self.youtrack_manager.get_issue_url(l['idReadable']))
        ...
        self.youtrack_manager.add_attachments_to_task(release_issue, files_to_upload)

        # assign release pack to user
        self.youtrack_manager.reassign(release_issue, assign_to)
        ...
        self.youtrack_manager.add_comment(release_issue, f'Composer Package Name:\n{composer_package_name}')

        # Set PD and TEST to Done
        self.youtrack_manager.close_issue(release_tasks[self.PD_TASK_LABEL])
        self.youtrack_manager.close_issue(release_tasks[self.TEST_TASK_LABEL])
