...
class WA:
    ...
    def sql_connection(self, uid, pwd, server, database = 'Vista_Replication_PRD'):
        self.user = quote_plus(uid)
        self.pss = quote_plus(pwd)
        self.sv = quote_plus(server)
        self.db = quote_plus(database)
        engine = create_engine(f"mssql+pyodbc://{self.user}:{self.pss}@{self.sv}/{self.db}?driver=ODBC+Driver+17+for+SQL+Server")
        try:
    ...
    def criar_imagem_SQL(self, consulta, arquivo = 'dist/temp.png'):
        # try:
            cn = self.sql_connection(self.user, self.pss, self.sv)
            if cn == 'Conectado':
                df = read_sql_query(consulta, self.conn)
                export(df, arquivo, max_cols=-1, max_rows=-1)
                return 
            else: self.criar_imagem_SQL(consulta)
        # except Exception as erro:
        #     enviar_email(erro)
        #     return None
    ...
    def main_loop(self, funcs: list):
        if self.cn == 'Conectado':
            ...
            while True:
                ...
                dsp(h, self.cn)
        else: self.cn = self.whats.sql_connection(self.uid, self.pwd, self.server)

if __name__ == '__main__':
    p = Parcial('guilherme.breve','84584608-Gui','10.56.6.56', 8, 23)
    dds = [
        lambda: p.whats.enviar_msg('Guilherme','Teste',p.whats.criar_imagem_SQL('SELECT TOP 1 Id, Nome FROM Tarefa')),
        lambda: print('Fim...')
    ]
    main = []
    main.append (dds)
