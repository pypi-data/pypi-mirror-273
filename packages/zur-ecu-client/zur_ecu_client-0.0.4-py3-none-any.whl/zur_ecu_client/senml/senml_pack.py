...
from zur_ecu_client.senml.senml_base import SenmlBase
...
class SenmlPack(SenmlBase):
    ...
    def _check_value_type(self, value, field_name):
        ...
        if not value == None:
            if not (isinstance(value, int) or isinstance(value, float)):
                raise Exception(
                    "invalid type for " + field_name + ", only numbers allowed"
                )
    ...
    def _process_incomming_data(self, records, naming_map):
        ...
        for item in records:
            if (
                naming_map["bn"] in item
            ):  # ref to a pack element, either this or a child pack.
                if item[naming_map["bn"]] != self.name:
                    pack_el = next(
                        (x for x in self._data if x.name == item[naming_map["bn"]]),
                        None,
                    )
                else:
                    pack_el = self
                ...
                rec_el = next(
                    (x for x in cur_pack_el._data if x.name == item[naming_map["n"]]),
                    None,
                )
                if rec_el:
                ...
                rec_el = next(
                    (x for x in self._data if x.name == item[naming_map["n"]]), None
                )
                if rec_el:
                    rec_el.do_actuate(item, naming_map)
                elif new_pack:
    ...
    def remove(self, item: SenmlRecord):
        """
