# Python Client For Nutanix Lifecycle Versioned APIs

The Python client for Nutanix Lifecycle Versioned APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Infrastructure, Software and Firmware Upgrades.
## Features
...
## Configuration
The python client for Nutanix Lifecycle Versioned APIs can be configured with the following parameters
...
```python
# Initialize the API
entities_api_instance = EntitiesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get details about all LCM entities.
try:
    api_response = entities_api_instance.list_entities(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=lifecycle&version=v4.0.b1&language=python). This documentation is auto-generated, and the location may change.
