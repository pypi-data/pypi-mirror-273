...
class GroupType(enum.Enum):
    ...
    NUMBER = 'integer'
    LETTERS = 'letters'
    LETTERS_UPPER = 'lettersUpper'
    LETTERS_LOWER = 'lettersLower'
    SPECIAL = 'special'
...
def random_integer(size: int | list[int]):
    """生成一个指定长度的随机数字"""
    return random_string(size, groups=['integer'])


def random_string_series(series: int, size=15, groups: list[str] = None):
    """
