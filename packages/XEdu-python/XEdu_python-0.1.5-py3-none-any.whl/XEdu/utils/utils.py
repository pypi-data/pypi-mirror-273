...
def visualize_similarity(similarity, x,y):
    ...
    if isinstance(x[0], str) and os.path.exists(x[0]):
        x_im = True
        images = [plt.imread(image) for image in x]
    else:
        x_im = False
        images = x
    if isinstance(y[0], str) and os.path.exists(y[0]):
        y_im = True
        texts = [plt.imread(image) for image in y]
    else:
        y_im = False
        texts = y
...
def visualize_probability(prob, images, classes, topk=2):
