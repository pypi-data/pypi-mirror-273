...
class Actions:
    ...
    def dataset_create(self, dataset_type, dataset_format, use_for, cloud_details=None):
        """Create a dataset and return the id"""
        request_dict = {"type": dataset_type, "format": dataset_format}
        if cloud_details:
            request_dict["cloud_details"] = json.loads(cloud_details)
        if use_for:
    ...
    def experiment_create(self, network_arch, encryption_key, cloud_details=None):
        """Create an experiment and return the id"""
        request_dict = {"network_arch": network_arch, "encryption_key": encryption_key}
        if cloud_details:
            request_dict["cloud_details"] = json.loads(cloud_details)
        data = json.dumps(request_dict)
