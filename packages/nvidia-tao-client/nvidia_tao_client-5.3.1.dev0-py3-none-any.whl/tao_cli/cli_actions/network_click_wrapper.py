...
def create_click_group(group_name, help_text):
    ...
    def dataset_create(dataset_type, dataset_format, use_for, cloud_details):
        """Create a dataset and return the id"""
        id = click_obj.dataset_create(dataset_type, dataset_format, use_for, cloud_details)
        click.echo(f"{id}")
    ...
    @click.option('--cloud_details', help='Cloud storage details.', required=True, default=None)
    def experiment_create(network_arch, encryption_key, cloud_details):
        """Create an experiment and return the id"""
        id = click_obj.experiment_create(network_arch, encryption_key, cloud_details)
        click.echo(f"{id}")
