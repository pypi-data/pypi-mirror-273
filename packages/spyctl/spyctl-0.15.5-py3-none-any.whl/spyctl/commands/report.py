...
def download(report_id: str):
    ...
    handle_get_report_download(report_id, lib.OUTPUT_RAW)
...
def handle_generate_report(from_file: IO[str], save_input: str):
    ctx = cfg.get_current_context()
    if from_file:
        with from_file:
    ...
    if not response.status_code == 200:
        cli.err_exit(api.response_err_msg(response))
        return
...
def handle_get_report_download(rid: str, output: str):
    ctx = cfg.get_current_context()
    report = api.get_report_download(*ctx.get_api_data(), rid)
    cli.show(report, lib.OUTPUT_RAW)
...
def __collect_report_arg(nr: int, arg: Dict) -> Any:
    ...
    if not required:  # (testing) or arg["name"] == "et":
...
def __report_summary_data(
    report: Dict, wide=False, tag_headers: List = []
) -> List:
    rv = [
        report["id"],
        report["status"],
