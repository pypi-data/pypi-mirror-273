...
class Reporter:
    ...
    def processor(
        ...
        # Cluster metrics
        context["cluster_metrics"] = get_cluster_metrics(
            index, schema_index, format
        )
...
def get_cluster_metrics(
    index: dict, schema_index: dict, format: str
) -> list[dict]:
    metrics = [
        {
        ...
        {"name": "Number of pods", "value": len(schema_index[s_pod])},
        {
        ...
        {"name": "Number of services", "value": len(schema_index[s_service])},
    ]
    return metrics
...
def get_agent_health(index: dict, schema_index: dict, format: str) -> dict:

    rv = {"nano_agent": {}, "cluster_monitor": {}}
