...
class Leaderboard(BaseModel, extra=Extra.allow):
    ...
    def to_display_df(self, includes=None, excludes=None, sort_params=None, header_mapping=None) -> pd.DataFrame:
        ...
        df = self.df
        if len(df) > 0:
            df = _filter_by_stable_elo_required_battles(df)
...
def _add_repo_aggregate_columns(df):
    if len(df) > 0:
        ...
        for column in ["celo_rating", "double_thumbs_up_ratio", "thumbs_down_ratio", "win_ratio", "safety_score", "model_score"]:
            grouped_by = df.groupby("model_repo")[column]
            df[f"{column}_by_repo"] = grouped_by.transform("max") if len(grouped_by) > 0 else None
        for column in ["feedback_count", "num_battles"]:
            grouped_by = df.groupby("model_repo")[column]
            df[f"{column}_by_repo"] = grouped_by.transform("sum") if len(grouped_by) > 0 else None
        for column in ["celo_rating", "double_thumbs_up_ratio", "thumbs_down_ratio", "win_ratio", "safety_score", "model_score"]:
            grouped_by = df.where(df["best_of"] == 1).groupby("model_repo")[column]
            df[f"{column}_by_repo_open"] = grouped_by.transform("max") if len(grouped_by) else None
        for column in ["feedback_count", "num_battles"]:
            grouped_by = df.where(df["best_of"] == 1).groupby("model_repo")[column]
            df[f"{column}_by_repo_open"] = grouped_by.transform("sum") if len(grouped_by) else None
    return df


def _filter_by_stable_elo_required_battles(df):
    df = df[(df.status == 'deployed') | (df.num_battles >= config.LEADERBOARD_STABLE_ELO_REQUIRED_BATTLES)]
    return df
