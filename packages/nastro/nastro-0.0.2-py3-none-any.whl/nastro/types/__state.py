...
from ..frames.frames import Frame, ICRF


# JUST HERE AS REFERENCE FOR FUTURE FRAME IMPLEMENTATION
...
class GenericState[T: (Double, Vector)]:
    ...
    def __init__(
        ...
        _angles: dict[str, tuple[float, float]] | None = None,
    ) -> None:

        # Specify components and angles
        ...
        if _angles is not None:
            ...
            self.angles: dict[str, tuple[float, float]] = {}
        ...
        # Convert angles to radians and wrap them if necessary
        ...
        if wrap:
            for angle in self.angles:
                low, high = self.angles[angle]
                # self.__dict__[angle] = self._wrap(self.__dict__[angle], low, high)
                self.__dict__[angle] %= high
    ...
    def __repr__(self) -> str:
        ...
        out = f"{self.__class__.__name__} in {self.frame.__name__}\n"
        out += "-" * len(out) + "\n"
        for component in self.components:
            out += f"{component}: {getattr(self, component)}\n"
        return out
    ...
    def __mul__(self, other: Self | Double | int) -> Self:
        if isinstance(other, Double) or isinstance(other, int):
            ...
            # CHECK: Do I really want to support this?
            out = type(self)(
                self.q1 * other.q1,
                self.q2 * other.q2,
                self.q3 * other.q3,
    ...
    @staticmethod
    def _wrap(angle: Double, low: Double = 0.0, high: Double = 2.0 * np.pi) -> Double:
        """Wrap angle to a given interval
        ...
        angle -= low
        angle = angle % (high - low)
        angle += low
        return angle

    def transform(self, frame: type[Frame]) -> Self:
        """Transform state to another reference frame"""
        q1, q2, q3 = self.frame.transform(self.q1, self.q2, self.q3, frame)
        q4, q5, q6 = self.frame.transform(self.q4, self.q5, self.q6, frame)
        out: Any = type(self)(q1, q2, q3, q4, q5, q6, frame=frame)
        return out
...
class CartesianState[T: (Double, Vector)](GenericState):
    ...
    Components might be floating point numbers (single state) or numpy arrays
    of them (time series of states).

    Parameters
    -----------
    x, y, z: Double | Vector
        Components of the position vector [m]
    dx, dy, dz: Double | Vector
        Components of the velocity vector [m/s]
    frame: Frame
        Reference frame in which the state vector is defined
    """

    def __init__(
        self, x: T, y: T, z: T, dx: T, dy: T, dz: T, frame: type[Frame] = ICRF
    ) -> None:
        ...
        super().__init__(x, y, z, dx, dy, dz, frame=frame, _components=_components)
        return None
    ...
    # def transform(self, frame: type[Frame]) -> Self:
    #     """Transform cartesian state to another reference frame"""
    #     x, y, z = self.frame.transform(self.x, self.y, self.z, frame)
    #     dx, dy, dz = self.frame.transform(self.dx, self.dy, self.dz, frame)
    #     out: Any = CartesianState(x, y, z, dx, dy, dz, frame=frame)
    #     return out

    def to_spherical(self) -> "SphericalState":
        """Conversion from cartesian to spherical state"""
        rho, theta, phi = self.frame.to_spherical(self.q1, self.q2, self.q3)
        drho, dtheta, dphi = self.frame.to_spherical(self.q4, self.q5, self.q6)
        out: Any = SphericalState(rho, theta, phi, drho, dtheta, dphi, frame=self.frame)
        return out
...
class KeplerianState[T: (Double, Vector)](GenericState):
    ...
    def __sub__(self, other: object) -> Self:
        ...
        if not isinstance(other, KeplerianState):
            raise TypeError("Subtraction is only supported for keplerian state vectors")

        di = np.unwrap(self.q3, period=np.pi) - np.unwrap(other.q3, period=np.pi)
        draan = np.unwrap(self.q4, period=2.0 * np.pi) - np.unwrap(
            other.q4, period=2.0 * np.pi
        )
        daop = np.unwrap(self.q5, period=2.0 * np.pi) - np.unwrap(
            other.q5, period=2.0 * np.pi
        )
        dta = np.unwrap(self.q6, period=2.0 * np.pi) - np.unwrap(
            other.q6, period=2.0 * np.pi
        )
    ...
    def to_cartesian(self, mu: Double) -> "CartesianState":
        ...
        return CartesianState(x, y, z, dx, dy, dz, frame=self.frame)
...
class SphericalState[T: (Double, Vector)](GenericState):
    ...
    def transform(self, frame: type[Frame]) -> Self:
        """Transform spherical state to another reference frame"""
        cstate = self.to_cartesian()
        q1, q2, q3 = self.frame.transform(cstate.q1, cstate.q2, cstate.q3, frame)
        q4, q5, q6 = self.frame.transform(cstate.q4, cstate.q5, cstate.q6, frame)
        out: Any = CartesianState(q1, q2, q3, q4, q5, q6, frame=frame).to_spherical()
        return out

    def to_cartesian(self) -> "CartesianState":
        """Conversion from spherical to cartesian state"""
        x, y, z = self.frame.to_cartesian(self.q1, self.q2, self.q3)
        dx, dy, dz = self.frame.to_cartesian(self.q4, self.q5, self.q6)
        out: Any = CartesianState(x, y, z, dx, dy, dz, frame=self.frame)
        return out

    def to_keplerian(self, mu: Double) -> "KeplerianState":
        """Conversion from spherical to keplerian state"""
        return self.to_cartesian().to_keplerian(mu)
...
class CartesianPosition[T: (Double, Vector)](GenericState):
    ...
    def __init__(self, x: T, y: T, z: T, *_, frame: type[Frame] = ICRF) -> None:

        components = {"x": "q1", "y": "q2", "z": "q3"}
        null = 0.0 * x
        super().__init__(x, y, z, null, null, null, frame=frame, _components=components)
    ...
    def to_spherical(self) -> "SphericalPosition":
        rho, theta, phi = self.frame.to_spherical(self.x, self.y, self.z)
        out: Any = SphericalPosition(rho, theta, phi, frame=self.frame)
        return out
...
class CartesianVelocity[T: (Double, Vector)](GenericState):
    ...
    def __init__(self, dx: T, dy: T, dz: T, *_, frame: type[Frame] = ICRF) -> None:

        components = {"dx": "q1", "dy": "q2", "dz": "q3"}
        null = 0.0 * dx
        super().__init__(
            dx, dy, dz, null, null, null, frame=frame, _components=components
        )
        return None
...
class SphericalPosition[T: (Double, Vector)](GenericState):
    ...
    def __init__(
        ...
        _keys = list(frame.variables.keys())
        components = {_keys[0]: "q1", _keys[1]: "q2", _keys[2]: "q3"}
        ...
        super().__init__(
            ...
            wrap=wrap,
            _components=components,
            _angles=frame.angles,
        )

        return None

    def asarray(self) -> Vector:
        return np.array([self.__getattr__(var) for var in self.frame.variables])

    def to_cartesian(self) -> "CartesianPosition":
        x, y, z = self.frame.to_cartesian(self.q1, self.q2, self.q3)
        out: Any = CartesianPosition(x, y, z, frame=self.frame)
        return out

    def transform(self, frame: type[Frame]) -> Self:
        """Transform spherical position to another reference frame"""
        out: Any = self.to_cartesian().transform(frame).to_spherical()
        return out
