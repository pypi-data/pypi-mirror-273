...
class EventReporter(BaseReporter):
    ...
    def __init__(self, logger, reports_dir, templates_dir, global_fallback, locale, timezone, eventservice, sqlservice, mailer, groups_dict, settings_dict, whoismodule, thresholding = True):
    ...
    def _get_relevant_source_sections(event, ip):
        ...
        for section in event.get("Source", []):
            if (str(ip) in list(map(str, section.get("IP4", []))) or
                    str(ip) in list(map(str, section.get("IP6", [])))):
