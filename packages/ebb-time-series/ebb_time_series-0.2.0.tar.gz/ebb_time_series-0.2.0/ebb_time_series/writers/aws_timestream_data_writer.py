...
class AwsTimestreamDataWriter(BaseDataWriter):
    ...
    def _parse_event_data(self, event_consumer: EventConsumer) -> list[dict]:
        """
        Helper method that parses event message data from an EventConsumer object
        to build the record to be written to time series database. For aws timestream
        writer specifically, a list of records is required so this method returns list[dict].
        ...
        NOTE: must be of form list[] because list is expected by boto3 but it will always be a list of one
        dict entry.
        ...
        Returns:
            list[dict]: list of single measurement_record dict to be written to timestream
