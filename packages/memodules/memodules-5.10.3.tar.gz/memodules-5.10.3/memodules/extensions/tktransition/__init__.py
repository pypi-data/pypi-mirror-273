...
class Transition(tk.Frame):
    ...
    def register_widget(self, widget: tk.Widget | Iterable[tk.Widget], *, id: int | slice = AUTO):
        ...
        if self._initialized:
            self.current.max = len(self._widgets) - 1
...
def debug():
    root = tk.Tk()
    frames = Transition(root)
    frames.pack(side=tk.TOP, anchor=tk.N, fill=tk.BOTH, expand=True)
    frames.prepack(fill=tk.BOTH, expand=True)

    f = tk.Frame(frames)
    frames += f
    tk.Label(f, text='LABEL1', font=font.Font(size=20)).pack(fill=tk.BOTH, expand=True)
    frames.initial_pack()

    f = tk.Frame(frames)
    frames += f
    tk.Label(f, text='LABEL2', font=font.Font(size=20)).pack(fill=tk.BOTH, expand=True)
    tk.Button(f, text='BTN', command=lambda: print('Button Pushed!')).pack()

    footer = tk.Frame(root)
    footer.pack(side=tk.BOTTOM, anchor=tk.S, fill=tk.X, expand=True)
    tk.Button(footer, text='NEXT', command=frames.next).pack(side=tk.RIGHT, anchor=tk.E)
    tk.Button(footer, text='PREV', command=frames.prev).pack(side=tk.RIGHT, anchor=tk.E, padx=5)
    root.mainloop()
