...
def measure_shear(image: Image,
    ...
    if bad_exposures:
        ...
        if image.n_exp == 0:
            raise LensMCError('Left no exposures.')
    ...
    image.merge_mask_with_segmentation(id_, ra, dec)
    ...
    # PSF FWHM
    psf_fwhm = np.array([np.mean([psf_disc[ii].get_fwhm(), psf_bulge[ii].get_fwhm()]) *
                         np.mean(astrometry[ii].pixel_scale) for ii in range(n_objects)])
    ...
    lensmc_results.set_fluxes(snr, flux, flux_bulge, flux_disc, image)
    lensmc_results.set_image_props(image)
    lensmc_results.set_other_props(id_, chi2 / dof, dof, pvalue_bkg, flag, psf_fwhm, seed)
    if cal:
...
class LensMCResults:
    ...
    __slots__ = ('e1', 'e2', 're', 'ra', 'dec', 'e1_err', 'e2_err', 'e_var', 'w', 're_err', 'ra_err', 'dec_err',
                 ...
                 'likl_calls_intcal', 'likl_calls_mcmc', 'likl_calls_optim', 'psf_fwhm', 'seed',
    ...
    def set_fluxes(self, snr, flux, flux_bulge, flux_disc, image):
        ...
        self.magnitude = np.mean(mag, axis=1)
        ...
        if self._has_samples:
            ...
            if np.isfinite(mag).any():
                self.magnitude_err = np.nanstd(mag, axis=1)
            else:
                self.magnitude_err = np.nan
    ...
    def set_other_props(self, id_, chi2, dof, pvalue_bkg, flag, psf_fwhm, seed):
