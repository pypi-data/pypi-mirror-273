...
class Application:
    ...
    def _create_loki_handler(
        ...
        kwargs = {}
        if loki_endpoint:
            kwargs["url"] = loki_endpoint
        else:
            raise Exception("invalid loki_address")
        if loki_user:
            kwargs["auth"] = (loki_user, loki_pass)
        if loki_labels:
            kwargs["tags"] = loki_labels
        handler = logging_loki.LokiQueueHandler(
            multiprocessing.Queue(-1),
            **kwargs,
        )
        handler.formatter = _JsonFormatter()
        return handler
...
class _InlineFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record_tags and isinstance(record_tags, dict):
            tags |= record_tags
        record_lite_tags = getattr(record, "liteTags", None)
        if record_lite_tags and isinstance(record_lite_tags, dict):
            tags |= record_lite_tags
        if record.levelno == logging.ERROR or record.levelno == logging.CRITICAL:
            tags["caller"] = f"{record.module}:{record.lineno}"
        if tags:
            if record.exc_info:
                return f"{time} {record.levelname[0]} | {record.msg} {json.dumps(tags)}\n{self.formatException(record.exc_info)}"
            else:
                return f"{time} {record.levelname[0]} | {record.msg} {json.dumps(tags)}"
        else:
            if record.exc_info:
...
class _JsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record_lite_tags and isinstance(record_lite_tags, dict):
            tags |= record_lite_tags
        record_dict = {}
        record_dict["message"] = record.msg
        record_dict["level"] = record.levelname
        record_dict["time"] = datetime.datetime.fromtimestamp(
            record.created, datetime.datetime.now().astimezone().tzinfo
        ).strftime(_JsonFormatter.default_time_format)
        record_dict["server"] = socket.gethostname()
        record_dict["tags"] = tags
        if record.levelno == logging.ERROR or record.levelno == logging.CRITICAL:
            record_dict["caller"] = f"{record.module}:{record.lineno}"
        if record.exc_info:
            record_dict["exception"] = self.formatException(record.exc_info)
        return json.dumps(record_dict, indent=self._indent)
