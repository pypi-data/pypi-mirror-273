...
class VectorGraphics2D(Rectangle2D):  # Image2D # Rect2D
    ...
    def __init__(self, figure, *args, x=None, y=None, width=None, height=None, opacity=1., stroke=0.65, **kwargs):
        ...
        self.line_style = None # useless just for compat
...
#         return extra_space
    ...
    def draw(self, painter, draw=True):
        ...
        if self.must_update_figure_on_first_paint:
            try:
                if self.figure is not None:
                    cm_width = 0.02646 * float(self.width())
                    cm_height = 0.02646 * float(self.height())
        ...
        painter.setClipRect(self.getRect(scale=True))  # in fact this is needed to get text to work!!!
        ...
        # painter.scale(1./self.scale, 1./self.scale)  # size is ok but position sucks --> how can I fix that
        ...
        if draw and self.renderer is not None:
            ...
            # does not really work
            # neo = QRectF(self)
            neo = QRectF(self.getRect(scale=True))
            ...
            painter.setClipRect(neo)  # , Qt::ClipOperation operation = Qt::ReplaceClip , operation=Qt.ReplaceClip
            # neo.setX(self.x())
            # neo.setY(self.y())
