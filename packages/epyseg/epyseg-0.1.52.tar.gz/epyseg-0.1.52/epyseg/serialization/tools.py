...
from epyseg.files.tools import read_string_from_file
    ...
    if isinstance(obj_dict, dict):
        for key, value in obj_dict.items():
            ...
            #     object_to_xml(value, child)
                ...
                if isinstance(value, str) and not key=='coords_as_list' and not key=='range': # excetptions that should not be written as texts even though they are texts
...
def clone_object(objects, auto_unpack_if_single=True):
    ...
    if isinstance(objects, list) and not objects:
        return clones
    if not isinstance(objects, Image2D) and not objects:
        return clones
    # if objects is None:
    #     return clones
...
if __name__ == '__main__':
    ...
    if False:
        ...
        print(has_custom_script(read_string_from_file('/home/aigouy/Bureau/test.pzf')))
        print(has_custom_script(read_string_from_file('/home/aigouy/Bureau/test_with_custom_script.pzf')))
    ...
    if True:
        ...
        print(type(copy.annotations), copy.annotations)
        print(type(copy.img))
        print(type(copy))
        print(copy.isText)
