...
from .types import _path, _str, _array
...
def load_json(path: str | Path, encoding: str = "utf-8") -> list | dict | None:
    """
    ...
    Returns:
        Union[list, dict, None]: The loaded JSON data as a list, dictionary, or None if an error occurs.
    """
    if not _path(path) or not str(path).endswith(".json"):
        return None
...
def save_json(
    path: str | Path, content: list | dict, encoding: str = "utf-8", indent: int = 4
) -> None:
    """
    ...
    elif not str(path).strip().endswith(".json"):
        path = f"{path}.json"
    create_path(path)
