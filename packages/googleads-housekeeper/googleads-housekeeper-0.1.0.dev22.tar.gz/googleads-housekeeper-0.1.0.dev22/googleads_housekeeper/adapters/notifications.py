    ...
    def _prepare_message(self, payload: MessagePayload) -> mail.Message:
        ...
        if placements_excluded := payload.placements_excluded_sample:
            ...
            body = self._generate_notification_html(
                payload.task_id, payload.task_name, payload.task_formula, placements_excluded)
        else:
        ...
        if body:
            try:
                ...
                logging.error(f'Error sending email. Invalid sender format. Sender: {sender}')
                raise e

        return mail.EmailMessage(
            sender=sender, to=recipient, subject=subject, body=body)

    def _generate_notification_html(self, task_id: str, task_name: str, task_formula: str, placements: gaarf.report.GaarfReport) -> str:
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                <p>The CPR tool has excluded the following placements
                based on your defined thresholds:</p>
                <p><strong>Task Name: </strong>{task_name}</p>
                <p><strong>Task Logic: </strong>{self._to_human_readable(task_formula)}
                (<a href="https://{self.project_id}.uc.r.appspot.com/newtask?task={task_id}"
                class="task-link"></a>)
                </p>"""
        ...
        for placement in placements:
            html_body += f"""
                <ul class="placement-list">
                    <li title={placement.reason}><strong>Placement Name:</strong>
                    {placement["name"]} (<a href="[Placement Link]">
                    {placement["placement"]}</a>)
                    <br/>
                    Reason: {self._to_human_readable(placement.reason)}</li>
                </ul>
            """
        html_body += """
            </div>
            ...
            <div class="logo-placeholder">
                <img src="https://i.sstatic.net/Vc0DxQth.png"
                alt="gPS Logo">
            </div>
