...
class RefuelClient:
    ...
    def label(
        ...
        if explain_fields:
            ...
            subtask_names = [subtask.get("name") for subtask in subtasks]
            for subtask_name in explain_fields:
                if subtask_name not in subtask_names:
                    raise RefuelException(
                        HTTPStatus.NOT_FOUND,
                        f"Subtask {subtask_name} not found for application {application}",
                    )
            explain_subtask_ids = [
                subtask.get("id")
                for subtask in subtasks
                if subtask.get("name") in explain_fields
            ]
        else:
            application_id = self._get_application_id(application)
            ...
            for i, input in enumerate(inputs):
                future_obj = self._async_post(
                    ...
                    params={
                        ...
                        "explain": explain,
                        "explain_subtask_ids": ";".join(explain_subtask_ids)
                        if explain_subtask_ids
                        else None,
                    },
                    json=[input],
                    headers=self._headers,
