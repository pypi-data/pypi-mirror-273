# AUTOGENERATED! DO NOT EDIT! File to edit: ../../../../nbs/agents/rl_agents/algorithms/03_SAC_hybrid_reversed.ipynb.

# %% auto 0
__all__ = ['SAC_hybridPolicy', 'SAC_hybrid_reversed']

# %% ../../../../nbs/agents/rl_agents/algorithms/03_SAC_hybrid_reversed.ipynb 4
# General libraries
import numpy as np
...
# RL_OM
from ..sac_gumbel import GumbelSoftmax

# %% ../../../../nbs/agents/rl_agents/algorithms/03_SAC_hybrid_reversed.ipynb 6
class SAC_hybridPolicy(Policy):
    ...
    def draw_action_mean_and_logits(self, state):
        ...
        disc_action = logits.argmax(dim=1)

        if isinstance(disc_action, np.ndarray):
            if self._mu_approximator.model.use_cuda:
                state = torch.from_numpy(state).cuda()
            else:
                state = torch.from_numpy(state)

        cont_mu_raw = self._mu_approximator.predict(torch.hstack((state, disc_action)), output_tensor=True)
    ...
    def compute_action_and_log_prob_t(self, state, compute_log_prob=True):
        ...
        discrete_dist = self.discrete_distribution(state) # detach to avoid gradients of continuous through here
        a_discrete = discrete_dist.rsample()

        # Continuous
        cont_dist = self.cont_distribution(state, a_discrete.detach())
    ...
    def cont_distribution(self, state, a_discrete):
        """
        ...
        mu = self._mu_approximator.predict(torch.hstack((state, a_discrete)), output_tensor=True)
        log_sigma = self._sigma_approximator.predict(torch.hstack((state, a_discrete)), output_tensor=True)
        # Bound the log_std
    ...
    def entropy(self, state=None):
        ...
        """
        # Discrete
        disc_distribution = self.discrete_distribution(state)
        disc_action = disc_distribution.rsample()

        # Continuous dist and action
        cont_distr = self.cont_distribution(state, disc_action)

        # return sum of cont and discrete entropy
        return torch.mean(cont_distr.entropy()).detach().cpu().numpy().item() + torch.mean(disc_distribution.entropy()).detach().cpu().numpy().item()
...
# %% ../../../../nbs/agents/rl_agents/algorithms/03_SAC_hybrid_reversed.ipynb 8
class SAC_hybrid_reversed(DeepAC):
    """
