...
def global_to_local(
    pts: npt.NDArray[np.floating[T]],
    cam_pose: npt.NDArray[np.floating[T]],  # [x,y,z,qx,qy,qz,qw]
) -> npt.NDArray[np.floating[T]]:
    local_pts = pts - cam_pose[0:3]
    rotation_matrix: npt.NDArray[np.floating[T]] = Rotation.from_quat(
        cam_pose[3:7]
    ).as_matrix()
    return cast(npt.NDArray[np.floating[T]], local_pts @ rotation_matrix)
