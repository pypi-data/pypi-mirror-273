...
class SaxiIcoDataModule_MT(pl.LightningDataModule):
    ...
    def pad_verts_faces(self, batch):
        # Lists to store padded tensors for each attribute
        verts_l_list, faces_l_list, vertex_features_l_list, face_features_l_list = [], [], [], []
        verts_r_list, faces_r_list, vertex_features_r_list, face_features_r_list = [], [], [], []
        nb_timepoints_list, Y_list = [], []

        # Iterate over each subject in the batch
        for subject_data in batch:
            # Pad attributes for the current subject
            verts_l, faces_l, vertex_features_l, face_features_l, verts_r, faces_r, vertex_features_r, face_features_r, nb_timepoints, Y = self.pad_subject_attributes(subject_data)
            
            # Append padded tensors to respective lists
            verts_l_list.append(verts_l)
            faces_l_list.append(faces_l)
            vertex_features_l_list.append(vertex_features_l)
            face_features_l_list.append(face_features_l)
            verts_r_list.append(verts_r)
            faces_r_list.append(faces_r)
            vertex_features_r_list.append(vertex_features_r)
            face_features_r_list.append(face_features_r)
            nb_timepoints_list.append(nb_timepoints)
            Y_list.append(Y)

        # Stack padded tensors along batch dimension
        verts_l = torch.stack(verts_l_list, dim=0)
        faces_l = torch.stack(faces_l_list, dim=0)
        vertex_features_l = torch.stack(vertex_features_l_list, dim=0)
        face_features_l = torch.cat(face_features_l_list, dim=0)
        verts_r = torch.stack(verts_r_list, dim=0)
        faces_r = torch.stack(faces_r_list, dim=0)
        vertex_features_r = torch.stack(vertex_features_r_list, dim=0)
        face_features_r = torch.cat(face_features_r_list, dim=0)
        nb_timepoints = torch.tensor(nb_timepoints_list)
        Y = torch.stack(Y_list, dim=0)

        return verts_l, faces_l, vertex_features_l, face_features_l, verts_r, faces_r, vertex_features_r, face_features_r, nb_timepoints, Y

    def pad_subject_attributes(self, subject_data):
        # Pad attributes for a single subject
        verts_l, faces_l, vertex_features_l, face_features_l, verts_r, faces_r, vertex_features_r, face_features_r, nb_timepoints, Y = subject_data

        # Pad individual attributes
        verts_l = pad_sequence(verts_l, batch_first=True, padding_value=0.0)
        faces_l = pad_sequence(faces_l, batch_first=True, padding_value=-1)
        vertex_features_l = pad_sequence(vertex_features_l, batch_first=True, padding_value=0.0)
        face_features_l = torch.cat(face_features_l)
