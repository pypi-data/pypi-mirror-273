...
class MongoDBModel(BaseMongoDBModel, MongoDBModelEvents):
    ...
    def get_all(self,
        ...
        # ).limit(per_page)
        cursor: CommandCursor = self._collection.aggregate([
            {"$match": filters},
            {"$facet": {
                "data": [
        ...
        # matches: int = cursor.count()
        # Convert cursor to list of dictionaries
        matches = list(cursor)[0]["count"][0]["total"] if cursor.alive else 0
        data = [Helper.standardize_dict(item, underscore=True) for item in list(cursor)[0]["data"]]
