...
class EnumWidget(Widget):
    def __init__(self, enum_class: Any):  # type: ignore
        self.enum_class = enum_class

    def clean(self, value, row=None, *args, **kwargs):  # type: ignore
        val = super().clean(value)
        if val:
            return getattr(self.enum_class, val.upper()).value

    def render(self, value, obj=None):  # type: ignore
        if value:
...
class SCSV(base_formats.CSV):
    ...
    def create_dataset(self, in_stream, **kwargs):  # type: ignore
        kwargs["delimiter"] = ";"
        kwargs["format"] = "csv"
...
class SCSVAdminMixin(ImportExportMixin):
    ...
    def get_import_formats(self):  # type: ignore
...
class ExternalImageResourceMixin(Resource):
    ...
    def before_import(self, dataset, using_transactions, dry_run, **kwargs):  # type: ignore
        self.dry_run = dry_run

    def before_import_row(self, row, row_number=None, **kwargs):  # type: ignore
