...
setup(
    ...
    package_data={'': ['*.yaml', '*.txt', '*.bin', '*.pcd', '*.png', '*.gif', '*.ui']},
    classifiers=[
    ...
    install_requires=['PyQt5', 'PyOpenGL', 'pyqtgraph','opencv-python', 'opencv-contrib-python'],  # 安装所需要的库
    entry_points={
        'console_scripts': [
            'wata.yanhua=wtao.console:fireworks',
        ],
    },  # 设置命令行工具(可不使用就可以注释掉)
)
