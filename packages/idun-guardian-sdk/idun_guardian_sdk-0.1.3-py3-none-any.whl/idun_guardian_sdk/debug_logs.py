...
def logging_device_not_found(break_time):
    logger.debug(f"[BLE]: No IGEB device found, trying again in {break_time} seconds")
...
def logging_device_address(device_address):
    logger.debug("[BLE]: Received address as %s", device_address)
...
def logging_device_description_list(char, value):
    logger.debug("%s : %s", char.description, str(value))
...
def logging_reading_battery_level():
