"""
Modifying the directory.py file based on langchain
...
from pylmkit.utils.data_utils import yield_specify_file
...
class DirectoryLoader(BaseLoader):
    ...
    def __init__(
        ...
        self.path = path
        self.glob = glob
        self.suffixes = set(suffixes or [])  # Modifying
        self.load_hidden = load_hidden
        self.loader_cls = loader_cls
        if self.loader_cls is None:  # Modifying
            self.loader_cls = UnstructuredFileLoader
    ...
    def load(self) -> List[Document]:
        ...
        # Modifying
        if self.suffixes:
            items = list(yield_specify_file(items, suffixes=self.suffixes))
