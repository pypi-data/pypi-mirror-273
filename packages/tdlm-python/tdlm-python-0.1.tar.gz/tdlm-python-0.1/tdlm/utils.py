...
def insert_events(data, insert_data, insert_labels, sequence, n_events, 
    ...
    # Randomly choose starting indices for events based on the calculated probability distribution
    idx_start = np.random.choice(np.arange(len(data)), n_events, replace=False, p=p)
    ...
    for start_idx in replay_start_idxs:
    ...
    return (data_sim, event_onsets) if return_onsets else data_sim
