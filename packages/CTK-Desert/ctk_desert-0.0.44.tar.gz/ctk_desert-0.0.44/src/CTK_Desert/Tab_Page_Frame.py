...
class Frame(ctk.CTkFrame):
    ...
    def update_state_checker(self, event):
        if ((event.width != self.window_width or event.height != self.window_height) and (event.widget == self.window)):
            ...
            if self.listener_thread is None or not self.listener_thread.is_alive():
                self.listener_thread = threading.Thread(target=self.start_pynput_listener, daemon=True)
                self.listener_thread.start()
                self.pack_forget()
                self.check_thread_completion()

    def check_thread_completion(self):
        if self.listener_thread.is_alive():     # if the left click is pressed, then the window is being resized, so i will keep checking the state until the left click is released
            self.after(50, self.check_thread_completion)
        else:
            self.pack(expand = True, fill = "both")
            # print(f"thread state {'Alive' if self.listener_thread.is_alive() else 'dead'}")
            self.update_sizes()

    def start_pynput_listener(self):
        with mouse.Listener(on_click=self.on_pynput_click) as listener:
    ...
    def on_pynput_click(self, x, y, button, pressed):
        if button == mouse.Button.left:
            ...
            if not pressed:
                # print("Left mouse button released at", (x, y))
