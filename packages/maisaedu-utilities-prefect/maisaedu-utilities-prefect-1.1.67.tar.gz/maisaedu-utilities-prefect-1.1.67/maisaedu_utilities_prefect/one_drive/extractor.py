        ...
        for column in columns:
            if column['type_cast'] is not None:
                ...
                if column['type_cast'] == 'string and remove dots':
                    documents_list.iloc[:, column['position']] = documents_list.iloc[:, column['position']].apply(lambda x: f'{x:.0f}' if isinstance(x, (int, float)) else str(x))
                    documents_list.iloc[:, column['position']] = documents_list.iloc[:, column['position']].str.replace('\.0', '', regex=True)
