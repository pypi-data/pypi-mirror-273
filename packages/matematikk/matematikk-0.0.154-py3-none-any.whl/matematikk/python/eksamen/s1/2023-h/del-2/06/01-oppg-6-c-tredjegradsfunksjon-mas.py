...
if _blokk == 1:
    ...
    svar_c_liste.append(f"")
    svar_c_liste.append(f"- Hvis grafen til f har et vendepunkt for x = 3 slik at f'(1) = f'(5)")
    svar_c_liste.append(f"  så betyr dette at f''(3) = 0")
    svar_c_liste.append(f"- Fordi f''(x) er den deriverte av f'(x), så betyr dette igjen at")
    svar_c_liste.append(f"  f'(x) har et ekstremalpunkt for x = 3 (enten topp- eller bunnpunkt)")
    svar_c_liste.append(f"- Videre så vet vi at f(x) er en tredjegradsfunksjon, noe som vil si at")
    svar_c_liste.append(f"  f'(x) er en andregradsfunksjon")
    svar_c_liste.append(f"- Grafen til f'(x) blir altså en parabel")
    svar_c_liste.append(f"- Og, fordi alle parabler er symmeritske om ekstremalpunktene sine,")
    svar_c_liste.append(f"  så kan vi se på avstanen mellom henholdsvis x = 1 og x = 5, og")
    svar_c_liste.append(f"  denne symmetrilinjen gjennom x = 3")
    
    # Symmetrilinje og verdier
    x_symmetrilinje  = 3
    x_1              = 1
    x_2              = 5
    ...
    # Svar-setninger
    svar_c_liste.append(f"")
    svar_c_liste.append(f"- Fordi 3 - 1 = 2 og 5 - 3 = 2 betyr at det er like stor avstand i fra")
    svar_c_liste.append(f"  de to x-verdiene til symmetrilinjen, så må de også ha samme y-verdi,")
    svar_c_liste.append(f"  altså at f'(1) = f'(5)")

    # If-løkke som ser om avstandene er like
    svar_c_liste.append(f"")
    if x_symmetrilinje - x_1 == x_2 - x_symmetrilinje:
        svar_c_liste.append(f"- Påstand 3 er derfor sann")
    if x_symmetrilinje - x_1 != x_2 - x_symmetrilinje:
        svar_c_liste.append(f"- Påstand 3 er usann")
