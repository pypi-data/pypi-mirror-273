...
class ClustEn(object):
    ...
    def extract_conformations(self, loc:str):
        ...
        for i, (frameindex, cluster_id) in enumerate(zip(self.chosen_frames, self.chosen_cluster_ids)):
            pdb_file = self.engen.pdb_files[cluster_id]
            traj_file = f'/lab/hou/vchaudhari/EnGens/pdb_data/accepted/{self.engen.mdtraj_list[cluster_id]}'
            print(f"Extracting conformation from frame {frameindex} of cluster {cluster_id}")
            new_file = os.path.join(loc, f"conf_in_cluster_{cluster_id}_frame_{frameindex}.pdb")

            if os.path.exists(new_file):
                print("File with the same name exists. Deleting the old file.")
                os.remove(new_file)
            command = f"~/anaconda3/envs/conda_engen_py37/bin/mdconvert -t {pdb_file} -o {new_file} -i {0} {traj_file}"
            print("command: ",command)
            ret_val = call(command, shell=True)
            #ret_val = call(["mdconvert -t " +pdb_file  + " -o " + new_file + " -i " + str(frameindex) + " " + traj_file ], shell=True)
            print(ret_val)
            if ret_val != 0:
