...
from MolGen.MolGen.ClustEn import *
from MolGen.MolGen.FeatureSelector import *
from MolGen.MolGen.DimReduction import *
import os
...
class PlotUtils(object):
    ...
    def __init__(self, molgen:MolGen, 
                        clust: ClustEn,
                        file_loc: str = "./", 
                        stride: int = 1,
        ...
        self.x = molgen.dimred_data[::self.stride,0]
        self.y =  molgen.dimred_data[::self.stride,1]
        ...
        self.rep_x = molgen.dimred_data[::,0][self.rep_fnum]
        self.rep_y = molgen.dimred_data[::,1][self.rep_fnum]

    def view_timeline(self, fileloc=None, width=None) -> go.Figure: 
        if self.molgen.crystal_flag == True:
            # plot for crystals
            pfig = go.Figure()
            ...
            x_labs = [x for x in self.molgen.structure_names]
    ...
    def view_PCs(self, fileloc=None) -> go.Figure:
        ...
        for i, elem in enumerate(lablist):
            if self.molgen.crystal_flag == True:
                marker_labels.append("cluster="+str(elem)+" \n pdb file ="+str(self.molgen.structure_names[i]))
            else:
    ...
    def view_movie(self, repr = "cartoon"):
        ...
        views = []
        for traj_file, pdb_file in zip(self.molgen.mdtraj_list, self.molgen.pdb_files):
            file_name = '/lab/hou/vchaudhari/MolGens/pdb_data/accepted/' + traj_file
            traj = pt.load(file_name, top=pdb_file) 
            
            view = nv.show_pytraj(traj)
            view.clear_representations()
            add_func = getattr(view, "add_" + repr)
            add_func()
            update_func = getattr(view, "update_" + repr)
    ...
    def plot_custom_feature_scatter(self, feature, y_title="feature"):
        ...
        for i in range(n_clusters):
            indices = np.argwhere(self.clust.labels[self.clust.chosen_index] == i).flatten()
            y = [feature[index] for index in indices]
            y_list.append(y)
    ...
    def plot_frames_from_cluster(self,
        ...
        for i, frame_idx in enumerate(selected_frames):
            ...
            frame = frame_idx
            #frame = cluster_frames[frame_idx]
            print("Adding frame {}".format(frame))
            filepath = '/lab/hou/vchaudhari/MolGens/pdb_data/accepted/' + molgen.mdtraj_list[frame]
            traj_frame = mdtraj.load_frame(filepath, top=molgen.pdb_files[frame], index=0)
            #traj_frame = mdtraj.load_frame(['/lab/hou/vchaudhari/MolGens/pdb_data/accepted/' + path for path in self.molgen.mdtraj_list], top=molgen.ref, index=frame)
        ...
        if plot_representative:
            ...
            filepath = '/lab/hou/vchaudhari/MolGens/pdb_data/accepted/' + molgen.mdtraj_list[repres_index]
            traj_frame = mdtraj.load_frame(filepath, top=molgen.pdb_files[frame], index=0)
            #traj_frame = mdtraj.load_frame(['/lab/hou/vchaudhari/MolGens/pdb_data/accepted/' + path for path in self.molgen.mdtraj_list], top=molgen.ref, index=repres_index)
            frames_mdtraj.append(traj_frame)
        ...
        mdconvert_path = "~/anaconda3/envs/conda_molgen_py37/bin/mdconvert"
        ...
        if extract_selected:
            ...
            for frame in selected_frames:
                pdb_file = molgen.pdb_files[frame]  # Assuming this indexation is correct
                traj_file = f'/lab/hou/vchaudhari/MolGens/pdb_data/accepted/{self.molgen.mdtraj_list[frame]}'  # Assuming this indexation is correct
                file_name = os.path.join(folder_path, f"cluster_{cluster_index}_frame_{frame}.pdb")
                print(f"Extracting {file_name}")
                
                # Delete the old file if it exists
                if os.path.exists(file_name):
                    print("File with the same name exists, deleting the old file")
                    os.remove(file_name)

                # Call mdconvert with the full path
                ret_val = call(f"{mdconvert_path} -t {pdb_file} -o {file_name} -i {0} {traj_file}", shell=True)
                if ret_val != 0:
                    raise Exception("Error saving conformations for frame {}".format(frame))

            # Extract representative frame
            repres_index = clust.chosen_frames[cluster_index]
            pdb_file = molgen.pdb_files[repres_index]
            traj_file = f'/lab/hou/vchaudhari/MolGens/pdb_data/accepted/{molgen.mdtraj_list[repres_index]}'
            file_name = os.path.join(folder_path, f"cluster_{cluster_index}_representative_frame_{repres_index}.pdb")
            print(f"Extracting {file_name}")
            ...
            # Call mdconvert with the full path for the representative frame
            ret_val = call(f"{mdconvert_path} -t {pdb_file} -o {file_name} -i {0} {traj_file}", shell=True)
            if ret_val != 0:
                raise Exception("Error saving conformations for representative frame {}".format(repres_index))
        '''
        ...
        if extract_selected:
            ...
            for i, frame in enumerate(selected_frames):
                ...
                ret_val = call(["mdconvert -t " + molgen.pdb_files + " -o " + file_name + " -i " + str(frame) + " " + 
                                molgen.mdtraj_list], shell=True)
                if not ret_val == 0:
