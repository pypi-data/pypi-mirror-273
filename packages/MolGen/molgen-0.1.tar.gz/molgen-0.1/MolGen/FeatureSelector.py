...
class UserFeatureSelection(FeatureSelection):
    ...
    def __init__(self, index: int, molgen: MolGen) -> None:
        self.index = index
        self.molgen = molgen
        super().__init__()
    ...
    def select_feature(self) -> None:
        #implement feature selection
        if self.index > len(self.molgen.featurizers):

            raise Exception("Featurizer index out of bounds.\
             Please chose one of the following indexes: \n" +
             self.molgen.describe_featurizers())
        else:
            self.molgen.chosen_feat_index = self.index  
            print("Picked featurized no. "+str(self.index)+": "+self.molgen.featurizer_names[self.index])
            f = self.molgen.featurizers[self.index]
            if self.molgen.crystal_flag: f = f[0]
            print(f.describe())

class VAMP2FeatureSelection(FeatureSelection):

    def __init__(self, lags: List[int], dims: List[int], molgen: MolGen) -> None:
    ...
    def run_vamp(self) -> None:
        ...
        if self.molgen.data == None:
            print("Generating data from featurizations")
            self.molgen.apply_featurizations()
        if len(self.molgen.data) == 0:
            raise Exception("No featurizers provided to MolGen.")
    ...
    def select_feature(self) -> None:
        ...
        print("Picked featurized no. "+str(max_ind)+": "+self.molgen.featurizer_names[max_ind])
        print("With maximum VAMP2 score no. "+str(max_ind))
        print(self.molgen.featurizers[max_ind].describe())
        self.molgen.chosen_feat_index = max_ind
    ...
    def plot_results(self) -> None:
        ...
        for i, d in enumerate(self.dims):
            for j, l in enumerate(self.lags):
                names = self.molgen.featurizer_names
