...
from smol_sae.gated import GatedSAE
...
def sae(config, model):
    return GatedSAE(config, model)


def test_vanilla_sae_forward(model, device, sae: GatedSAE):
    n_batch = 2
    # test forward
...
def test_vanilla_sae_patch_loss(model, device, sae: GatedSAE):
