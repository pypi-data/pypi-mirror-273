...
from smol_sae.anthropic import AnthropicSAE
...
def sae(config, model):
    return AnthropicSAE(config, model)


def test_vanilla_sae_forward(model, device, sae: AnthropicSAE):
    n_batch = 2
    # test forward
...
def test_vanilla_sae_patch_loss(model, device, sae: AnthropicSAE):
