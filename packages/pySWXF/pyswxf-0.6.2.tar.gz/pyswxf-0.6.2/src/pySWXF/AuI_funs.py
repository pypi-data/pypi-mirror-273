...
from pySWXF.fluor_fit import multilayer_ref_new_model
...
def get_offset_fit(th,I,dI,Beam_Energy, fitflag, showfit = True, verbose=True):
    if fitflag == 'NN':
        with importlib.resources.open_text('pySWXF', MULTILAYER_FIT) as f:
            params = Parameters()
            params.load(f)
    ...
    presim = multilayer_ref_new_model.eval(params = params, theta = th_refrac, 
