...
from ._data import AnnDataSplitter, _get_contact_array, _get_cools
from ._module import HiClipModule
from ._train import HiClipTrainingPlan
from ._utils import _logger, mat2array
...
class HiClip(BaseModelClass):
    ...
    def observe(
        ...
        tensors = dict()
        tensors[self.main_loc] = torch.Tensor(main_array[np.newaxis, :])
        tensors[self.sub_loc] = torch.Tensor(sub_array[np.newaxis, :])
