...
class TestLLM(TestCase):
    ...
    def test_local_tgi_api(self):
        ...
        url = "http://hz-model.bigmodel.cn/tgi-12b"
        # set_logger("dev", "")
        messages = [dict(role="user", content="你好呀，你是谁")]
        _system = "请用英语回答我的问题，你的名字是XAgent"
        # 测试tgi api
        logger.info("test sync chat")
        resp = call_llm(messages, url=url, llm_type=LLM_TYPE.TGI,
                        system=_system, temperature=0.7, top_p=0.95, max_tokens=100, stream=False, do_sample=False)
        logger.info(json.dumps(resp.model_dump(), ensure_ascii=False, indent=4))
        self.assertIsNotNone(resp.content)
        self.assertTrue("XAgent" in resp.content)
        ...
        resp = call_llm(messages, url=url, llm_type=LLM_TYPE.TGI,
                        system=_system, temperature=0.7, top_p=0.95, max_tokens=100, stream=True, do_sample=False)
