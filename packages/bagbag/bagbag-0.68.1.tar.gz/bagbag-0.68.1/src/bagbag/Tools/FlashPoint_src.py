...
class FlashPoint():
    ...
    def Search(self, basetypes:list[str]|str, keyword:str=None, pagesize:int=100, page:int=0, fromTime:str|int="2024-05-15T22:09:39Z", toTime:str="now", order:str="desc") -> dict:
        ...
        if type(fromTime) in [int, float]:
            fromTime = Time.Strftime(fromTime, "%Y-%m-%dT%H:%M:%SZ")
    ...
    def FetchDataUntilNow(self, basetypes:list[str]|str, fromTime:str|int="2024-05-15T22:09:39Z") -> typing.Iterable[dict]:
        """
        ...
        while True:
            try:
                res = self.Search(basetypes, pagesize=200, fromTime=fromTime, order="asc")
