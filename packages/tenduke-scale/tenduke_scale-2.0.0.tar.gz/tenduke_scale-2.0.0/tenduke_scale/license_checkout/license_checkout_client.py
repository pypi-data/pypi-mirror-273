...
def _concat_checkout_headers(client_details, license_consumer_id):
    client = client_details.to_api() if client_details else {}
    consumer = (
        {"licenseConsumerId": str(license_consumer_id)} if license_consumer_id else {}
    )
...
class _DescribeQueryStringBuilder:
    ...
    def license_consumer_fragment(self, license_consumer_id):
        ...
        return (
            None
            if license_consumer_id is None
            else f"licenseConsumerId={license_consumer_id}"
        )
    ...
    def describe_license_options_fragment(self, describe_license_options):
        ...
        return (
            None
            if describe_license_options is None
            else describe_license_options.to_query_string()
        )

    def client_binding_id_fragment(self, client_binding_id):
        """Fragment based on client binding id."""
        return (
            "" if client_binding_id is None else f"clientBindingId={client_binding_id}"
        )
