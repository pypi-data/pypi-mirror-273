...
class Outlook(Core):
    ...
    def get_two_factor_code_email(
        self, subject_pattern: str, otp_code_pattern: str = r"\b\d{6}\b", folder_name: str = "Inbox"
    ) -> str:
        """Retrieve two-factor authentication code from email.

        :param:
            subject_pattern (re.Pattern)
            otp_code_pattern (re.Pattern, optional)
            folder_name (str, optional): Name of the folder to retrieve
        ...
        query = folder.new_query().on_attribute("subject").contains(subject_pattern)
        retry_count = 0
