...
def process_heartbeat(client: APIClient | None, **kwargs: Any) -> str:
    try:
        config_dict = kwargs.get("config_dict", {})
        agg_info = config_dict.get("aggregator")
        wml_services_url = agg_info["ip"].split("/")[0]

        hearbeat_resp = requests.get(
        ...
        if service_version.startswith("4.6"):
            return "cpd460"
        if (
            service_version.startswith("4.5")
            or service_version.startswith("45")
            or service_version.startswith("0.1.10000")
        ):
            return "cpd450"
...
def import_diff(module_file_path: str) -> None:
...
def choose_software_version(software_spec: str) -> str:
    ...
    if (
        ...
        or software_spec == "12b83a17-24d8-5082-900f-0ab31fbfd3cb"
    ):
        client_reqs = {
            "py_version": "3.9",
            "tensorflow_version": "2.7",
            "torch_version": "1.10",
            "scikitlearn_version": "1.0",
            "fl_extras": "fl-rt22.1",
        }
        ...
        or software_spec == "b56101f1-309d-549b-a849-eaa63f77b2fb"
    ):
        client_reqs = {
            "py_version": "3.10",
...
class Party(WMLResource):
    ...
    def __init__(self, client: APIClient | None = None, **kwargs: Any) -> None:
        ...
        # process location
        ibmfl_module_location = ibmfl_base_module_location + str(
            self.SUPPORTED_PLATFORMS_MAP.get(platform_env)
        )
        if not ibmfl_module_location.endswith("ibmfl"):
            raise Exception(
                "Please use ibm-watsonx-ai "
                + str(self.SUPPORTED_PLATFORMS_MAP.get(platform_env))
            )
        ...
        module_spec = importlib.util.spec_from_file_location(
            module_name,
            ibmfl_base_module_location + "/py38-tf24-pt17-sk23/ibmfl/__init__.py",
        )
        module = importlib.util.module_from_spec(module_spec)  # type: ignore[arg-type]
        sys.modules[module_name] = module
        module_spec.loader.exec_module(module)  # type: ignore
