...
class AbstractTestAutoAIRemote(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        AbstractTestAutoAIRemote.experiment = AutoAI(credentials=copy(self.credentials),
    ...
    def test_03_initialize_optimizer(self):
        AbstractTestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            t_shirt_size=TShirtSize.S if self.api_client.ICP_PLATFORM_SPACES else TShirtSize.L,
    ...
    def test_19_deployment_setup_and_preparation(self):
        AbstractTestAutoAIRemote.service = WebService(source_instance_credentials=copy(self.credentials),
    ...
    def test_24_batch_deployment_setup_and_preparation(self):
        AbstractTestAutoAIRemote.service_batch = Batch(source_instance_credentials=copy(self.credentials),
                                                       source_project_id=self.project_id,
                                                       target_instance_credentials=self.credentials,
                                                       target_space_id=self.space_id)
