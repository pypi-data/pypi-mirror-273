...
from ibm_watsonx_ai.utils.deployment.errors import ServingNameNotAvailable
...
class AbstractTestWebservice(abc.ABC):
    ...
    def test_31_deployment_setup_and_preparation(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestWebservice.service = WebService(source_instance_credentials=self.credentials,
    ...
    def test_32__deploy__deploy_best_computed_pipeline_from_autoai_on_wml(self):
        ...
        self.assertTrue(self.api_client.deployments.is_serving_name_available(
        ...
        self.assertFalse(self.api_client.deployments.is_serving_name_available(
            serving_name=AbstractTestWebservice.initial_deployment_serving_name))

        with self.assertRaises(ServingNameNotAvailable):
            service = WebService(source_instance_credentials=self.credentials,
                                 source_space_id=self.space_id,
                                 target_instance_credentials=self.credentials,
    ...
    def test_32b_serving_name_test(self):
        ...
        deployment_details = self.api_client.deployments.get_details(serving_name='non_existing')
        self.assertTrue(len(deployment_details['resources']) == 0)

        self.api_client.deployments.update(AbstractTestWebservice.service.deployment_id, {
            self.api_client.deployments.ConfigurationMetaNames.SERVING_NAME: self.deployment_serving_name})

        time.sleep(1)  # wait for deployment update
        deployment_details = self.api_client.deployments.get_details(serving_name=self.deployment_serving_name)
        ...
        self.assertTrue(
            str(self.api_client.deployments.get_serving_href(deployment_details['resources'][0])).endswith(
        ...
        deployment_details = self.api_client.deployments.get_details(
            serving_name=self.deployment_serving_name_patch)
    ...
    def test_32b_get_details_as_generator(self):
        limit = 2
        deployments_details = []
        for entry in self.api_client.deployments.get_details(limit=limit, asynchronous=True):
    ...
    def test_32b_get_details_as_generator_get_all(self):
        limit = 2
        deployments_details = []
        for entry in self.api_client.deployments.get_details(limit=limit, asynchronous=True, get_all=True):
            #self.assertLessEqual(len(entry['resources']), 2)
            deployments_details.extend(entry['resources'])
    ...
    def test_32b_get_details_get_all_with_limit(self):
        limit = 2
        deployments_details = self.api_client.deployments.get_details(limit=limit, get_all=True)
        #self.assertLessEqual(len(entry['resources']), 2)

        self.assertEqual(self.get_all_result, len(deployments_details['resources']))

    def test_32b_get_details_get_all(self):
        deployments_details = self.api_client.deployments.get_details(get_all=True)
        self.assertEqual(self.get_all_result, len(deployments_details['resources']))
    ...
    def test_35_delete_deployment(self):
        ...
        self.api_client.repository.delete(AbstractTestWebservice.service.asset_id)
