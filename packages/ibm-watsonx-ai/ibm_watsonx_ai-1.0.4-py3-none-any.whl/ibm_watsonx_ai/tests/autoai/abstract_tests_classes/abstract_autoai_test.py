...
class AbstractTestAutoAIAsync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        ...
        if self.SPACE_ONLY:
            AbstractTestAutoAIAsync.experiment = AutoAI(credentials=copy(self.credentials),
        ...
        AbstractTestAutoAIAsync.batched = ('include_batched_ensemble_estimators' not in self.experiment_info and self.api_client.ICP_PLATFORM_SPACES) or \
...
class AbstractTestAutoAISync(abc.ABC):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        if self.SPACE_ONLY:
            AbstractTestAutoAISync.experiment = AutoAI(credentials=copy(self.credentials),
                                                       space_id=self.space_id)
        else:
            AbstractTestAutoAISync.experiment = AutoAI(credentials=copy(self.credentials),
                                                       project_id=self.project_id)
    ...
    #     AbstractTestAutoAISync.service = WebService(source_instance_credentials=copy(self.credentials),
