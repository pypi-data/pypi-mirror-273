...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_01_wml_credentials_development_False(self):
        if self.DEV_FIELD_NAME in self.credentials.__dict__.keys():
            self.credentials.__dict__.pop(self.DEV_FIELD_NAME)
        self.assertNotIn(self.DEV_FIELD_NAME, self.credentials.__dict__.keys())
    ...
    def test_06_wml_credentials_add_development_True(self):
        ...
        self.credentials.__dict__[self.DEV_FIELD_NAME] = True
        self.assertIn(self.DEV_FIELD_NAME, self.credentials.__dict__.keys())
