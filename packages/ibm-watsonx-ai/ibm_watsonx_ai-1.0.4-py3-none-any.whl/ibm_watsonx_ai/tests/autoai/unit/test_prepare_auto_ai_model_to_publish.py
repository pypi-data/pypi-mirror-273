...
class TestPrepareAutoaiModelToPublish(unittest.TestCase):
    ...
    @classmethod
    def setUp(cls) -> None:
        cls.credentials_cloud = get_credentials_from_config('CLOUD_PROD_AM', 'wml_credentials')
        cls.credentials_cp4d = get_credentials_from_config('CLOUD_DEV_AM', 'wml_credentials')

        cls.client_cloud = APIClient(cls.credentials_cloud)
        cls.client_cp4d = APIClient(cls.credentials_cp4d, cls.project_id)
