...
class TestAutoAIRemote(AbstractTestAutoAISync, AbstractTestBatch, unittest.TestCase):
    ...
    def test_00d_prepare_connected_data_asset(self):
        asset_details = self.api_client.data_assets.store({
            ...
            self.api_client.data_assets.ConfigurationMetaNames.NAME: "Housing - training asset",
            self.api_client.data_assets.ConfigurationMetaNames.DATA_CONTENT_NAME: f"{self.bucket_name}/{self.data_cos_path}"
        })
