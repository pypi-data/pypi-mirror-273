...
class TestAutoAIRemote(unittest.TestCase):
    ...
    def test_01_initialize_AutoAI_experiment__pass_credentials__object_initialized(self):
        TestAutoAIRemote.experiment = AutoAI(credentials=copy(self.credentials),
    ...
    def test_20_deployment_setup_and_preparation(self):
        ...
        if self.api_client.ICP_PLATFORM_SPACES:
            TestAutoAIRemote.service = WebService(source_instance_credentials=copy(self.credentials),
        ...
        self.api_client.set.default_project(self.project_id) if self.api_client.ICP_PLATFORM_SPACES else None
    ...
    def test_25_deployment_setup_and_preparation_different_wml(self):
        if self.api_client.ICP_PLATFORM_SPACES:
            TestAutoAIRemote.service = WebService(source_instance_credentials=copy(self.credentials),
                                                  source_project_id=self.project_id,
                                                  source_space_id=self.space_id,
                                                  target_instance_credentials=copy(self.credentials),
    ...
    def test_30_batch_deployment_setup_and_preparation(self):
        TestAutoAIRemote.service_batch = Batch(source_instance_credentials=copy(self.credentials),
                                               source_project_id=self.project_id,
                                               target_instance_credentials=self.credentials,
                                               target_space_id=self.space_id)
