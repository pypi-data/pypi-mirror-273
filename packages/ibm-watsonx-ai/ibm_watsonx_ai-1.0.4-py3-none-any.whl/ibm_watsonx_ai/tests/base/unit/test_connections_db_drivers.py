...
class TestDBDrivers(unittest.TestCase):
    ...
    def setUp(cls) -> None:
        ...
        cls.wml_client_cpd = APIClient(credentials=cls.wml_credentials_cp4d)
        ...
        cls.wml_client = APIClient(credentials=cls.wml_credentials)
    ...
    def test_02_sign_db_driver_url_cpd(self):
        jar_uris = self.wml_client_cpd.connections.sign_db_driver_url(
    ...
    def test_04_sign_db_driver_url_cpd(self):
        with self.assertRaises(WMLClientError):
            jar_uris = self.wml_client.connections.sign_db_driver_url(
                self.driver_file_name
            )
            print(jar_uris)
