...
class TestScikitLearnDeployment(AbstractOnlineDeploymentTest, unittest.TestCase):
    ...
    if JENKINS_RUNTIME == "rt23.1":
        ...
        deployment_type = "scikit-learn_1.3"
    else:
    ...
    def create_package_extension(self):
        ...
        pkg_extn_id = self.wml_client.package_extensions.get_id(pkg_extn_details)

        self.wml_client.package_extensions.download(pkg_extn_id, "./test_pkg_extension.zip")
    ...
    def create_software_spec_with_pkg_extension(self, pkg_extn_id):
        ...
        sw_spec_id = self.wml_client.software_specifications.get_id(sw_spec_details)

        return sw_spec_id

    def create_model_props(self):
        pkg_extn_id = self.create_package_extension()
        sw_spec_id = self.create_software_spec_with_pkg_extension(pkg_extn_id)
        ...
        return {
            ...
            self.wml_client.repository.ModelMetaNames.SOFTWARE_SPEC_ID: sw_spec_id,
        }
    ...
    def create_deployment_props(self):
        ...
        if self.wml_client.ICP_PLATFORM_SPACES:
