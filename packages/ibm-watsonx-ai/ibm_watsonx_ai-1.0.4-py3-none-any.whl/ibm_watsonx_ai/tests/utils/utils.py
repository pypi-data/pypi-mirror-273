...
def get_wml_credentials(env=environment):
    wx_credentials = Credentials.from_dict(json.loads(config.get(env, "wx_credentials")))
    wx_credentials._set_env_vars_from_credentials()
...
def setup_nfs_env(wml_credentials, dataset_file_name, remote_directory="datasets"):
    ...
    ssh_host = wml_credentials.__dict__.get("ssh_host")
    ssh_username = wml_credentials.__dict__.get("ssh_username")
    ssh_password = wml_credentials.__dict__.get("ssh_password")

    kube_username = wml_credentials.__dict__.get("kube_username")
    kube_password = None
...
def create_connection_to_cos(
    ...
    connection_id = wml_client.connections.get_id(connection_details)
