...
def next_resource_generator(
    ...
    _all: bool = False,
    _filter_func: Callable | None = None) -> Generator[dict, None, None]:
    """
    ...
    while next_href is not None:
        ...
        response = requests.get(
            ...
            params=(
                params if params is not None else client._params()
            ),
        )
        ...
        yield {
            "resources": (
                _filter_func(resources) if _filter_func else resources
            )
        }
