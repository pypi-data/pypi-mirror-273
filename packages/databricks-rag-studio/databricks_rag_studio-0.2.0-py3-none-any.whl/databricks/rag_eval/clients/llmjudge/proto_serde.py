...
class ChatAssessmentProtoSerde:
    ...
    def construct_assessment_request_json(
        ...
        result.set_repeated_field(
            ...
            [self.construct_assessment_definition(assessment_name, examples)],
        )
        return result
    ...
    def construct_example_assessment_input_json(
        ...
        result = JsonProto()
        result.set_required_field("chat_request", example.variables["request"])
        result.set_optional_field(
            "chat_response", example.variables.get("response", None)
        )
        result.set_optional_field(
            "retrieval_context",
            self.construct_example_retrieval_ctx_json(example.variables.get("context")),
        )
        result.set_optional_field(
            "ground_truth",
    ...
    def construct_example_ground_truth_json(
        ...
        result.set_optional_field(
            ...
            example.variables.get("expected_response", None),
        )
    ...
    def construct_assessment_definition(
        self, assessment_name: str, examples: List[example_config.AssessmentExample]
    ) -> JsonProto:
        result = JsonProto()
    ...
    def construct_example_rating_json(
        ...
        result = JsonProto()
        # Assuming that value can only be "True" or "False"
        result.set_optional_field(
            "bool_value",
            True if example.value.strip().lower() == "true" else False,
        )
        result.set_optional_field(
            "justification", example.variables.get("rationale", None)
        )
        return result
        ...
        if assessment_name in response_assessment_ratings.keys():
            return [
                entities.AnswerAssessmentResult(
                    ...
                    rating=response_assessment_ratings.get(assessment_name),
                )
            ]
    ...
    def construct_assessment_error_result(
        ...
        error_rating = entities.Rating(
            bool_value=None, double_value=None, rationale=None, error_message=error_msg
        )
        # if an error occurs, gracefully return a RatingError based on the type
        match assessment_type:
            case AssessmentType.ANSWER:
                return [entities.AnswerAssessmentResult(assessment_name, error_rating)]
            case AssessmentType.RETRIEVAL:
                return [
                    entities.RetrievalAssessmentResult(
                        assessment_name, {0: error_rating}
                    )
                ]
    ...
    def json_to_rating(self, proto: JsonProto) -> entities.Rating:
        ...
        # the proto is an oneoff so only one of them should be present
        if maybe_value is not None:
            return entities.Rating(
                double_value=JsonProto.get_optional_field(maybe_value, "double_value"),
                bool_value=JsonProto.get_optional_field(maybe_value, "bool_value"),
