...
class LocationSetBase:
    ...
    def fews_name(self) -> str:
        ...
        for fews_name in self.fews_name_candidates:
            if fews_name in possible_fews_name:
                self._fews_name_from_candidates = fews_name
                return self._fews_name_from_candidates
        raise AssertionError(
            f"fews_name_candidates {self.fews_name_candidates} (hardcoded in hdsr-wis-config-reader), but not found "
            f"in FEWS config (LocationSets.xml)"
        )
