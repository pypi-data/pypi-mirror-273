...
class GridAnalyzer:
    ...
    def _eval_grid_state(self):
        ...
        self.stats["health"] = float(
            self.model.grid.get_value("res_bus", attr="vm_pu")
            .values[1:]
            .mean()
        )
    ...
    def eval_voltage_violations(self):
        ...
        num_buses = len(self.model.grid.get_value("res_bus"))

        lower_bounds = np.ones(num_buses) * self.config["undervoltage_pu"]
        upper_bounds = np.ones(num_buses) * self.config["overvoltage_pu"]

        lower_violations = (
            self.model.grid.get_value("res_bus", attr="vm_pu") - lower_bounds
        )
        upper_violations = (
            self.model.grid.get_value("res_bus", attr="vm_pu") - upper_bounds
        )
