...
class Hypergraph_binning:
    ...
    def DL(self,i,j,celldata):
        ...
        """
        past_dls = self.past_dls
        past_nks = self.past_nks
        N = self.N
        T = self.T
        S = self.S
        D = self.D
    ...
    def logOmega(self, rs, cs, swap=True):
    ...
    def MDL_hypergraph_binning(self, X,dt,exact=True):
        """
        inputs:
        ...
        DL = self.DL
        self.N = int(sum([x[2] for x in X]))
        ts = [x[-1] for x in X]
        tmin,tmax = min(ts),max(ts)+1e-10
        self.S = len(set([x[0] for x in X]))
        self.D = len(set([x[1] for x in X]))
        N = self.N
        cellxs,celldata = {},{}
        for i,x in enumerate(X):
            s,d,w,t = x
            self.s, self.d, self.w, self.t = x
        ...
        if exact: 
            ...
            i = np.inf
            MDL_boundaries = []
            print("Checking j value:", j)
            print("Available keys in LMDL_int:", LMDL_int.keys())
            ...
            for K in np.arange(T,1,-1).astype('int'):
                ...
                for k in range(K-1):
                    ...
                    delta_dl = DL(i1,i4,celldata) - DL(i1,i2,celldata) - DL(i3,i4,celldata)
            ...
            best_MDL = sum([DL(b[0],b[1],celldata) for b in MDL_boundaries])
            
            MDLK1 = DL(1,T,celldata)       
            if MDLK1 < best_MDL:
                best_MDL = MDLK1
                MDL_boundaries = [(1,T)]
        ...
        L0 = DL(1,T,celldata) - np.log((N-1)*(T-1)) #subtract off previously ignored constant from description length 
