...
class AdultDataset(Dataset):
    ...
    def __init__(self, data):
        ...
        self.x = torch.from_numpy(data[["AGEP","COW","SCHL","MAR","OCCP","POBP","RELP","WKHP","race","sex"]].to_numpy()).type(torch.float)
...
class AdultNN:
    ...
    def fit(self, dadata):
        ...
        :parameter data: Dataset the will be split for training and hyper parameter tuning. Dataset must contain a column called "member" used as training label.
        :type dadata: fetch_data.Dataset
        """

        data = dadata.load()
        x = data.drop("member",axis=1).to_numpy()
        x = data[["AGEP","COW","SCHL","MAR","OCCP","POBP","RELP","WKHP","race","sex"]].to_numpy()
        self.columns = x.columns
        self.scaler = StandardScaler()
        self.scaler.fit(x)
        x = self.scaler.transform(x)
        x = pd.DataFrame(x,columns=data.columns.drop("member"))
        data[x.columns] = x
