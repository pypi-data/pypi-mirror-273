...
setup(
    ...
    packages=find_packages(include=["kmat_tools", "kmat_tools.*"]),
    package_data={"": ["kmat_tools/*","cython/*.pxd","cython/*.pyx","cython/lib/include/*.h","cython/lib/src/*.c","kmat_tools/cython/km_tools_wrappers/*"]},
    ext_modules = cythonize([ext1,ext2,ext3,ext4]),
    entry_points={
        "console_scripts": [
            "kmat_tools = kmat_tools.__main__:main",
            'km_basic_filter = cython.km_tools_wrappers.km_basic_filter_wrapper:km_basic_filter_main',
            'km_fasta = cython.km_tools_wrappers.km_fasta_wrapper:km_fasta_main',
            'km_unitig = cython.km_tools_wrappers.km_unitig_wrapper:km_unitig_main'
       ]
    },
        install_requires=[
        "colorama",
