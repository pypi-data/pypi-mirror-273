...
class FilterManager:
    ...
    def load_filter(self, df, primary_index=None):
        ...
        df_filter.to_sql(table_name=self.table_name, schema_name=self.schema_name, if_exists='replace', primary_index = primary_index)
        tdml.execute_sql(f'CREATE UNIQUE INDEX (filter_id) ON {self.schema_name}.{self.table_name}')
